...
> - **✏️ 이름·이모지·번역투**: 점토 채취장→**점토굴** · 성벽 🧱→**🏰**(점토와 중복) · 창고 📦→**🛢️** · **은신처→지하창고 🏺**(실제 역할=약탈당해도 자원 일부 보존) · 기계식 `을(를)/이(가)`를 받침 판별 조사 헬퍼 **`jo(단어,'을')`** 로 교체.
> - **🧹 인라인 팁 전부 제거**(설명은 모달로 갈 예정) · 상단바 질감 벽돌→석재.
> ⏭️ **미결**: 도움말 모달 미착수(우선순위=전투 규칙›야만인 습격›지도›집결지›정찰›정복›마을›시장/대장간) · 헥스 타일 바닥색이 지면색과 살짝 안 맞음 · 습격 빈도/강도 실플레이 튜닝 · 지형 타일이 개별 드로우콜(~300+)이라 병합하면 추가 성능 여지.
