...
- 현재 버전: v2.43.272 (브릿지 aram-bridge v1.1.31 패치 대기 — bridge-patch/ 참조)
...
## ⏭️ 다음 작업: 시즌 2 준비 (2026-06-02 논의, 구현 대기)
롤 칼바람 **증강 업데이트(6/10~11, PBE 이미 진행 중)**에 맞춰 앱도 **시즌 2** 시작 예정.

### 시즌 2 정책 (사용자 확정분)
- **리셋**: LP·티어, 골드, 패스, 쓰레기통(동전·무료권·유미 등) → 전부 새로 시작
- **가챠 컬렉션**: **결정 대기 중** (A 완전리셋 / B 그대로유지 / **C 하이브리드=추천**)
  - C안 = 카드/시너지는 리셋(골드싱크 부활·시너지 LP 이월 불공정 해소) + "도감 수집 기록"은 영구 보존(🏅 시즌1 수집 표식) + (선택) 완성자 prestige 보상

### ⚠️ 핵심 기술 과제 — 시즌 시스템 일반화 (가장 큰 작업)
현재 `sField()`는 시즌 인식 O (`seasonSuffix(2)='_s2'` 자동) 이나, **LP·대부분 기능이 `_s1`/`season1`에 하드코딩**돼 시즌2로 토글하면 반쪽 리셋(망가짐):
- **LP 랭킹이 시즌1 고정**: `season1Data`(33곳), `/season1/players` 경로(4곳), `s1ApplyMatchResult`(13곳) → `CURRENT_SEASON` 기준으로 일반화 필요(`/season{N}/players`)
- **하드코딩 `_s1` 리터럴 205개**: champCards_s1, seasonPassClaimed_s1, trashGold_s1, secretYuumi_s1, hasBattlePass_s1 등 → 정책(리셋/유지)에 맞춰 sField화 또는 보존 처리
- **switchSeason/관리자 바**: 현재 S0/S1만, S2 버튼·라벨 추가 필요
- 작업은 6/11 전 완료 목표. **정책(특히 가챠 A/B/C) 확정 후 착수.**
...
### v2.43.265~272 (2026-06-03) — 출석 버그 긴급 수정 + 브릿지 재연결 패치 + 기록 필터 ← 최신
