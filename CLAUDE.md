...
- **강화 3갈래(검키우기식)**: ✅성공(+1)/🟰보류(유지·재시도 쿠션)/💥파괴(수량−1 + 전투력 +0 리셋). 파괴로 7종 깨지면 인형뽑기 재획득해야 복귀. 확률=`ARENA_ODDS`(**v352 재조정**: 파괴를 **초반부터 분배**(+1=1%→+19=12% 점증·0%대 구간 없음) + 초반 성공↑, 실패=대부분 "유지"). 핵심=**파괴율 낮게(≤12%) + 유지가 골드 sink**(실제 검키우기처럼 인형 잘 안 부서지고 골드만 빠짐·이전엔 파괴 최대27%로 가혹했음). 시도비용=`arenaForgeCost`=`round(10+N×4+N²×0.8)`(**초반 싸고 갈수록 비쌈**·+1=15G·+10=130G·+19=375G·**+20까지 최선 누적 ~2,900G**·v352·**sink만**·투기장 골드생성0=인플레무관).
