...
> - **🎭 룬테라 컨셉 리스킨(같은 세션 후속·사장님 지시 "게임성은 그대로, 우리 컨셉을 껍데기처럼")**: 수치·로직 무변, 명칭/로어/표기만 교체. ①자원: 점토→**🔹 마정석**(RES_NM 신설·resbar title에 이름) ②건물: 본진→**💠 넥서스**·공방→**헥스텍 공방**·대장간→**오른의 대장간**(사이트 S2 연결)·대학→**총독부**·시장→**빌지워터 시장**·집결지→**지휘소**(탭/문구 전체)·목재소→벌목장 ③병종: 데마시아 창병/검사·녹서스 도끼병/습격대·아이오니아 궁수·**요들 정찰대**·프렐요드 궁기병·철갑 기사단·**공성 미니언**(ic 🐏→👾)·**헥스텍 투석기**·귀족→**총독** ④월드: 야만인 마을→**버려진 마을**(정보 패널에 전쟁 로어 한 줄)·라이벌=**사이트 팀원 닉 소환사**(울퉁쓰/애긔반달곰/빛나는언즈/맹독/브랜딩프로/나랑듀오)·뉴비 보호→**신규 소환사 보호** ⑤**대륙 K##=룬테라 지역명**(`KMAP`/`regionOf`/`crdR` — 북=프렐요드(산맥)·중앙=데마시아(홈 K11)·남=슈리마(사막)·남동=그림자 군도, 기존 지형 배치와 일치): Map3D에 `opts.regions` 라벨(`.v3-label.rgn3` 반투명 세리프)+2D 폴백 캔버스 지역명+정보패널/씬/사이드바 좌표에 지역 표기 ⑥디버그 노출 `window.onVillageInfo`/`goVillage`. 검증=Playwright(명칭·로어·정찰 게이트·라이벌·모바일 오버플로 전부 통과·에러 0).
