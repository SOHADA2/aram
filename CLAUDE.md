...
- **현재 BUILD 148** (HUD `BUILD N` pill로 확인 — 상단 정보 pill은 숨김, `#build-pill`만 노출). 최근: BUILD143=5번째 플레이 집기 먹통 수정, **BUILD148=코인/기회 차감 시점 START→`tryDrop`(실제 집기)로 이동** — START만 누르고 집기 전 닫기/최소화/iframe리로드/모바일전환/로딩중 START 시 `coins--;playsUsed++`가 즉시 `_saveCoins`→Firebase 저장돼 "하지도 못했는데 코인+기회 날아감"이던 버그. 차감을 `tryDrop`(수동·자동집기 단일 길목, `state!=='READY'` 가드로 1회)으로 옮겨 집기 전 이탈=과금0. `startGame`은 READY 진입만(공짜 조준). 매 수정 BUILD +1 + 인라인 `<script type=module>` 추출해 `node --check`(임시 .mjs) 검증 후에만 커밋. **푸시**: 그 세션의 작업브랜치(이름은 세션마다 다름, 예전엔 `claude/project-overview-skvq0h`·이번 세션은 `claude/work-from-another-computer-dq72u4`)에 커밋 후 `git push origin HEAD:main` + `HEAD:<작업브랜치>` 둘 다(Pages 자동배포 1~3분, 모바일 캐시 강함 → 시크릿/새탭). `claw.html`=캐시우회 사본(매 빌드 `cp` 후 `sed`로 `BUILD N · claw` 태그 — **소스+claw.html 둘 다 수정 필수**). 한글 커밋=heredoc UTF-8.
...
### v2.45.301 + 인형뽑기 BUILD 148 (2026-06-25) — 🕹️ 인형뽑기 코인/기회 헛소모 버그 수정 + 보상 + 🧸 단짝 desc 정정 ← 최신
