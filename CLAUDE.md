...
### v2.45.265~284 (2026-06-24/25) — 🔗 전투준비 칩·🎯 챔피언풀 모달 UI 대수술 + 🧸 단짝 하트유대(♥) + 🔨 강철심장 강화조작 되돌림 ← 최신
...
- **단짝 효과 명세 = "나쁘지 않다, 이대로 진행" 합의** (연승 2~7 / 연패 2~6 / 레벨효과 Lv2 골드·Lv3 안전망·Lv4 덤20%·Lv5 만개). 단 **`BUDDY_REWARDS_LOCKED=false`는 아직 안 풂**(확정 시 flip). ⚠️ **표시 스택=원시카운트인데 보상상한 7/6 불일치** 정리 必.
...
- 🧸 **단짝 보상 잠금 풀기**(`BUDDY_REWARDS_LOCKED` L32003) — 명세 합의됐으나 스택상한 불일치 정리 + 실기기 검증(7/7 완성자 0) 후. 7종 세트효과 구현.
