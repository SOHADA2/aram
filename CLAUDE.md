...
- **강화 3갈래(검키우기식)**: ✅성공(+1)/🟰보류(유지·재시도 쿠션)/💥파괴(수량−1 + 전투력 +0 리셋). 파괴로 7종 깨지면 인형뽑기 재획득해야 복귀. 확률=`ARENA_ODDS`(**v350 검키우기식 재조정**: 🟢+0~4 파괴0%·+0=100%확정 / 🟡+5~9 파괴4~10% / 🟠+10~14 ~10% / 🔴+15~19 **파괴 12% 캡**·실패=대부분 "유지"). 핵심=**파괴율 낮게(≤12%) + 유지가 골드 sink**(실제 검키우기처럼 인형 잘 안 부서지고 골드만 빠짐·이전엔 파괴 최대27%로 가혹했음). 시도비용=**소량 골드** `arenaForgeCost`=50+N×60(v351 ~절반 인하·N↑↑·**sink만**·투기장 골드생성0=인플레무관).
