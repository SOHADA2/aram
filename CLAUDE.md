...
> ⚠️ **index.html(라이브 앱) 무관** — `룬테라원정-프로토타입.html` 전면 재작성(v0.1→**v0.2**·1670줄·`SAVE_VER 7`=구세이브 폐기). 사장님 지시: "부족전쟁과 최대한 동일한 게임성으로 검토 + 비주얼/퀄리티 재구성 + **PC버전과 모바일버전이 다름을 확실히 인지**". 검증=Playwright 헤드리스(`/opt/node22/lib/node_modules/playwright`·chromium `/opt/pw-browsers/chromium-1194`·python http.server 8899·⚠️이 환경 jsdelivr 차단이라 3D는 2D 폴백으로 검증, 3D 실렌더는 실기 확인 필요) — PC 1366px/모바일 390px 스크린샷+전투/정찰/정복/지원 흐름 evaluate 실행 전부 통과·pageerror 0.
