...
- **✅ 1단계 완성(홀덤-목업.html)**: 홀덤 엔진(홀2+커뮤5·프리플롭/플롭/턴/리버 베팅·블라인드5/10·best7 판정·팟/올인 런아웃)+기본 봇AI(핸드강도+팟오즈+약간 블러핑·성격없음)+4인 테이블 UI+**초보 도움(코치=내 패 힘/추천액션·토글)+30초 인트로+족보/용어 도움말**. RTP/역추적/사이드팟은 미구현(단판 동일스택이라 사이드팟 불필요).
- **⏭️ 다음 단계**: 2단계=봇 **성격·블러핑·내 베팅에 반응**(심리전 핵심) → 3단계=참가비/상금·**RTP 시뮬 재튠**·일일캡 → 4단계=TF 3D 딜러 연동·봇 얼굴·모션 → **홈 복권 탭 정식 통합**(진짜 골드·역추적 로그·오버레이 임베드). 시뮬 스크립트=scratchpad `poker_real.mjs`(진짜 포커 RTP)·`poker_sim2.mjs`(구 성급버전).
