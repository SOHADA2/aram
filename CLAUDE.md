...
- 현재 버전: v2.44.49 (시즌2 🔨오른 3D 대장간 + 걸작 시스템 + 걸작 LP 스케일·S2 뉴비 OFF·LP 밸런스 감사 / 브릿지 aram-bridge v1.1.31 릴리즈 완료)
...
### v2.44.48~49 (2026-06-07) — ⚡ 걸작 LP 스케일 + 🌱 S2 뉴비 OFF + LP 밸런스 감사 ← 최신
...
- **⏭️ 미결 권고 (우선순위)**:
  1. **[중·1순위] `s1Resimulate` 재계산 누락 차단** — 운영자 콘솔 LP 재계산이 걸작 LP(winLP/lossLP)·가챠 시너지 LP를 빼먹어, S2에서 돌리면 라이브와 한 경기당 최대 ~28LP 어긋남(과거 LP2x 누락 버그 동종). 조치: S2 활성 중 `s1Resimulate` 실행 차단 OR 재시뮬 루프에 걸작/시너지 LP 포팅. **S2 본오픈 전 필수.**
