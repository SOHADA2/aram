...
### 🗺️ 룬테라 전쟁 — 🖥️ 다른 컴퓨터 이어작업 핸드오프 (2026-08-03 기준·최종 커밋 `c83e7de`·Pages success) ← 최신·必독
...
> - **작업 브랜치**: 세션마다 다름(직전 두 세션 = `claude/continue-update-bnfsry` → **`claude/continuing-work-4qpf2b`**). **배포 = `git push origin HEAD:main` + `HEAD:<작업브랜치>` 둘 다.** `.gitignore`가 `*`+화이트리스트라 **`git add -f`** 필요.
...
> - **시작 배율 = `CAM_START_ZOOM`(1.6)** — 모듈3 상단 상수 하나로 시작·`resetCam` 둘 다 제어(v0.4·2026-08-03). 예전 `8`은 내 성만 화면을 꽉 채워 세계가 안 보였다. 넓게/좁게는 이 한 줄만 고치면 됨. ⚠️`_camPlaced`는 target만 잡고 zoom은 안 건드리니 상수만 보면 된다.
>
> #### 5) ⏭️ 다음 할 일 / 미결
> - 🔤 **용어 변경 — 진행 중(사장님 결정 대기·이어받으면 여기부터)**. 사장님이 "용어 변경할 건데 바꿔야 할 리스트 달라"고 해서 **현재 용어 인벤토리만 뽑아 드린 상태**. **범위(A/B/C)를 아직 안 고르셨다 → 답 듣기 전에 이름을 바꾸지 말 것.** 물어본 선택지 = **(A) 기능명 유지, 어색한 것만**(예: 대학이 귀족 뽑는 게 안 맞음·공방·지하창고) / **(B) 전체를 새 컨셉으로**(⚠️이 경우 진영 안 섞이는 단일 세계관부터) / **(C) 찍어주는 몇 개만**.
>   - **인벤토리(등장 횟수=고칠 자리 규모)** — 전부 부족전쟁 기능명 그대로다(v0.3에서 컨셉 리네이밍을 통째로 되돌린 상태).
>     - **건물 15종 `BLD`(544행)**: 본진2·집결지14·병영2·마구간3·공방2·대장간14·대학2·시장9·목재소1·점토굴2·철광산1·농장5·창고5·지하창고4·성벽26
>     - **병종 11종 `TROOP`(571행)**: 창병3·검사2·도끼병3·궁병11·정찰병10·경기병3·궁기병2·중기병2·파성추3·투석기3·귀족8
>     - **자원 `RES_NM`(535행)**: 목재4·점토4·철3 — 건물명(목재소·점토굴·철광산)과 물려 있어 같이 움직인다
>     - **월드/상태**: 야만인 마을11 · 발전 4단계 `STAGE_NM`(813행) 개척지→마을→소도시→성곽 도시 · 충성도7 · 약탈11 · 수비대5 · 인구10
>     - **화면**: 탭 4개(지도·마을·집결지·보고서) + 상단(시장·대장간) · 행군 모드(공격·지원·정찰·귀환·습격 — 습격23은 야만인 습격 비활성이라 대부분 죽은 문자열)
>   - ⚠️ **병종 이름을 바꾸면 `TROOP_ROLE`(586행) 11줄도 같이 고쳐야 한다** — 「창병=기병을 막는 데 특화」처럼 **이름을 전제로 쓴 설명문**이다.
>   - ⚠️ **「소환사」 10곳이 잔재로 남아 있다** — `소환사 {이름}` 마을 접두어·`적 소환사`·**「신규 소환사 보호」 배너**. 라이벌 AI 삭제 때 안 지워진 것. 지금은 거의 안 보이는 문자열이라 **살릴지 지울지 먼저 정해야** 나머지 이름이 그 톤을 따를지가 결정된다.
>   - ⚠️⚠️ **이름은 임의로 제안하지 말 것** — 진영·지역·롤 컨셉 명칭을 붙였다가 **두 번 되돌렸다**(데마시아 창병+녹서스 도끼병이 한 군대에 섞여 설정 붕괴). 사장님 방침 = 컨셉은 설정 설계부터.
> - **도움말 모달**(사장님 승인됨·용어 변경 뒤가 순서상 유리 — 용어가 바뀌면 설명문을 다시 써야 하므로). 인라인 팁을 전부 걷어낸 상태라 설명할 곳이 없다. 순서=**전투 규칙 → 지도 → 집결지 → 정찰 → 정복 → 마을 → 시장/대장간**. ⚠️전투 규칙엔 **병과 분할 계산 같은 「규칙」만** 넣고 **조합·전술 조언은 넣지 않는다**(사장님 방침 — 조합은 유저가 알아내는 영역).
...
### 🎓 룬테라 전쟁 v0.11 — 튜토리얼(초반 10단계 + 건물 첫 완성 시 후반 콘텐츠 설명) (2026-08-03·원격web·작업브랜치 `claude/update-check-jz6251`·main+브랜치 배포) ← 최신·必독
