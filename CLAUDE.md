...
### 🗺️ 룬테라 전쟁 v0.2 — 부족전쟁 게임성 정합 + 양피지 테마 + PC/모바일 레이아웃 분리 (2026-07-29·원격web·작업브랜치 `claude/computer-work-session-we1guo`·main+브랜치 배포) ← 최신
...
> - **⚠️🎭 리스킨 정정(사장님 피드백 — 다음 작업자 필독)**: 처음에 유닛에 진영명(데마시아 창병·녹서스 도끼병·아이오니아 궁수·프렐요드 궁기병·요들 정찰대), 시장에 빌지워터, 대륙 K##에 룬테라 지역명(KMAP/regionOf/crdR·지도 라벨), 라이벌에 팀원 닉네임까지 붙였다가 **전부 되돌림**. 이유: **"데마시아와 녹서스는 적대 관계 — 한 군대에 섞어 붙이면 설정 붕괴. 진영/지역 설정은 나중에 별도로 제대로 설계해서 넣을 영역"**(사장님). → 유닛=순수 직업명(창병/검사/도끼병/궁병/정찰병/경기병/궁기병/중기병), 시장=시장, 좌표=K##만, 라이벌=소환사 아리/리븐 등 챔피언명. **유지된 것 = 진영 중립 롤 컨셉만**: 넥서스·헥스텍 공방·오른의 대장간·총독(부)·마정석·공성 미니언·헥스텍 투석기·버려진 마을·신규 소환사 보호. 도감 카드의 챔피언 출신 지역(아리=아이오니아 등)은 공식 챔피언 로어라 유지. **앞으로 진영/지역을 붙일 땐 반드시 사장님과 설정 설계부터.**
...
> - ⏭️ **미구현/이월**: 🌫️ 전장의 안개(사장님 승인·미착수) · 라이벌 AI 정찰/총독 없음 · 시장 상인 이동/부족(길드)/프리미엄 미구현(부족전쟁 잔여 격차) · 3D(지도·마을) 실기 확인 · 투석기 목표 건물 선택(현재 발전도 파괴로 단순화) · 가챠 챔피언=특수 지휘관 합류(도감에 예고만).
