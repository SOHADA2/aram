...
- 현재 버전: v2.45.114 (🌌 **시즌2 라이브 중 2026-06-10~** — 최근: 🎟 스크래치 복권 셀 레이아웃을 **목업 구조로 정합**(번호+SCRATCH 라벨 위 / 원형 디스크 아래 = `.scard-cellwrap` 래퍼 방식, 긁기 JS 무변경). 그 전 v110에서 셀/grid 직접 손대 긁기 깨짐 → v109 복원 후 래퍼 방식으로 안전 적용. 브릿지 **v1.1.35**. 시즌2 라이브 `switchSeason(2)` 완료. 상세는 아래 세션 이력 v2.45.110~114 + v88~109)
...
### v2.45.110~114 (2026-06-15) — 🎟 스크래치 복권 셀 레이아웃 목업 정합(번호+SCRATCH 래퍼 구조) + 크기 조절 프리뷰 ← 최신
