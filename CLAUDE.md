...
> - **🌱 야만인 마을 성장 상한(`barbMaxSize`)**: 성장이 `size<6`까지 무제한이라 **평균 2시간마다 +1★ → 약 10시간이면 월드 전체가 6★로 수렴**, 초반에 깔아둔 무방비 마을이 전부 사라져 파밍터가 없어짐(사장님 지적). → 마을마다 **거리 기준 상한 `mx`** 부여(<7칸 2~3★ · <14 3~4 · <24 4~5 · 그 밖 5~6, 생성 크기가 더 크면 그게 하한). `BARB_NEAR` 시작 파밍터는 **`mx:2` 고정**(수비대는 3★부터라 영구 무방비). 구 세이브는 **폐기 없이 `_ensureBarbCaps()`가 거리 기준으로 backfill**(이미 자란 마을은 줄이지 않고 그 자리에서 정지). 검증(전부 만렙까지 성장시킨 뒤): 반경4 13개 평균 2.54★·무방비 6 / 반경8 47개 평균 2.81★·무방비 22 / 반경16 평균 4.02 / 그 밖 평균 5.41.
