...
- 현재 버전: **v2.45.657** · 룬테라 전쟁 **v0.38(정식오픈·행군마커 병종구분)** · 인형뽑기 claw BUILD **172** · 오버레이 **v0.1.54** · 브릿지 **v1.1.38** · 인형뽑기 BUILD 171 (🌌 **시즌2 라이브 중** — 🆕**최신(v2.45.657·2026-08-04·이 PC·main 직접·Pages success)**: 🎬**룬테라 전쟁 행군 마커 병종별 구분**. `marchKind(m)`(mode+units→정찰/지원/귀환/습격/공격/**공성**(ram·cata)/**영웅**(noble)) 도입. 2D 지도=`MARCH_ICO` 아이콘+`.army-mk.{siege/noble/scout}` 색발광(공성 주황·영웅 금색·정찰 청록). 3D=Map3D에 `_mKind` + MKIT/MMDL/MCOL 확장(실모델: `siege-catapult.glb`·`flag-wide.glb`(금색·큼)·`flag-pennant.glb`), `mkMarchMesh` **bbox 정규화**(원본크기 무관 균일). 단일GLB 몽타주로 5종 시각구분 확인(투석기/금깃발/페넌트/배너/수레). MM3ARR `.mode`→`.kind`(프레임루프는 `.obj`만 참조=안전). ★**직전(v656)**: ⚖️**운영개선 #3·#4(사기 반그리핑·정복 온보딩)**. #3 **사기 반그리핑 실효화**: `moraleVs`를 옛 `defP=size×150`(홈 size6 고정→건물합321초과 강자에게만 페널티)에서 **공격자 `devPoints(originV)` vs 방어자 `w.pts`(발행 발전도)** 비교로 재설계 → `defP/atkP+0.3` clamp 0.4~1.0(대등1.0·2배0.8·4배0.55·10배↑0.4바닥). `_opAttack`은 `moraleVs(originV,o)`(스냅샷 pts)·`openAttack` ally 합성 w에 `pts:o.pts` 추가. #4 **정복 온보딩**: 신전(academy) req `main20→10·smithy20→8·market10→4`로 도달가능화(2차마을=정복뿐인데 게이트 너무 높았음)+튜토리얼 완료 메시지에 정복 목표 안내(BLD_TIP.academy 설명은 기존). ⚠️ 사기는 헤드리스로 수식만 검증(모듈내부 함수). **운영 잔여(여전히·사장님 판단)**: 🔴유령 마을/claims 정리(TTL) · 🟡감사로그 전투미기록·운영도구부재·RESET에폭 서버world 미삭제 · [비용]야만인 통저장은 **현 10명 규모선 무시가능**(실측: 구독 221B·저장 330KB/인·월500MB<무료10GB 확인). ★**직전(v655)**: 🔎운영감사(2갈래)+수정2건(뉴비보호막 야만인약탈유지·보고서캡). 수정: 🛡️**뉴비 보호막이 야만인 약탈로 풀리던 문제**(`openAttack` confirmRelease를 `w.owner==='player'||ally` 공격 때만·야만인 약탈은 보호 유지 — 튜토리얼 9단계 약탈이 15일 보호를 강제 해제하던 자기모순) · **중요 보고서(conquered) 캡 유실 방지**(`pushReport` 정복당함 별도보존20+일반60). **운영 잔여(미수정·사장님 판단 대기)**: 🔴[비용]서버저장이 **S 전체(야만인1800+안개`revealed`) 20s 통저장**→RTDB 대역폭(야만인은 `genWorld(778899)` 시드 재생성 가능한데 매번 저장=최대 낭비·구독은 플레이어 마을만이라 작음) · 🔴[데이터]**유령 마을/claims 무한누적**(이탈·정복 후 정리 주체 없음→안 들어온 유저 마을 계속 털림·registry 오염·`remove` 코드 없음) · 🟠[밸런스]**사기 반그리핑 무력**(`moraleVs` defP=size×150인데 홈 size=6 고정→건물합 321초과 강자에게만 페널티) · 🟠[온보딩]**2차 마을=정복뿐인데 튜토리얼서 정복 미안내**(신전 영웅 후반게이트→중반 콘텐츠 소진→그리핑) · 🟡감사로그가 전투/정복 미기록(분쟁 재구성 불가) · 🟡운영도구부재(유저1명 리셋/추방/롤백/재환불) · 🟡RESET에폭이 서버 world 미삭제(수기 REST 필수) · Low(FB장애 조용한 실패·버전공존·스폰인접·프리미엄가치). 상세=서브에이전트 리포트(대화). ★**직전(v654)**: 🧹**정식오픈 정리 — 테스트도구 숨김**. 사장님 "정식 오픈인데 테스트 버튼 남음" 지적 → 게임 우하단 🧪 devpanel(`initDevPanel`·즉시건설/무한자원/단계미리보기)과 콘솔치트(`runeGold`/`runeArmy`)를 **`_RUNE_DEV` 플래그**(=`?dev` 또는 `localStorage.rune_dev='1'`) 뒤로 숨김 — 정식선 안뜨고 치트 무효, 관리자만 소환. 상단 「베타 v0.34」→「v0.35」(베타 워딩 제거·버전은 진단용 유지). 헤드리스 검증=devpanel 없음·runeGold 무효·상단바 "v0.35 · 월드 1". ★**직전(v653)**: 🔄**오픈 리셋 실행 완료**. 코드=`const RUNE_RESET=1`(다음 리셋 시 +1) — `fresh()`에 `resetEpoch` 넣고 `load()`/`migrate()`/`_runeAdoptServer`가 **에폭 불일치 로컬/서버 세이브 폐기→전원 신규 시작**(서버만 지우면 클라 localStorage가 옛상태 재업로드하므로 에폭 필수). **REST 실행(이 PC·curl/Python urllib UTF-8)**: ①서버 게임데이터 삭제=`runewar/players`·`runewar/world/w1/villages`·`runewar/world/w1/claims` DELETE(전부 null 확인, runewar/log는 유지). ②**프리미엄 골드 환불**=`gold/<key>`의 `goldSpendLog_s2`서 `k:'runewar'` 합만큼 `goldSpent_s2` 차감 + 해당 로그 제거. 2계정 환불: **애긔반달곰 -10000(30일권1)·럼블홀릭 -20000(30일권2)**, 검증=runewar 로그 잔여0·중복환불 없음(1차 실행이 ✅이모지 print서 cp949 인코딩크래시로 A만 처리→멱등 재실행으로 B 완료, `PYTHONIOENCODING=utf-8`+ASCII print로 해결). 프리미엄은 players 삭제로 함께 리셋=완전 환불. ★**직전(v651~652)**: 🚀**팀 오픈 + 오픈전 3갈래 감사·수정 5건**(치명 시작게이트잠금·inbox 멱등성영속화 등). 3갈래 병렬 서브에이전트 감사(멀티정합성/보안·경제/상태·엣지) 후 수정: 🔴**치명 블로커=새기기·캐시삭제 시 시작게이트 안닫혀 게임잠김**(`_runeAdoptServer` started분기서 `#start-gate` 제거) · `bindActive` S.active 존재가드 · 정산 즉시 `_runeSync` 발행(반복약탈 창축소) · `_runeConquer` set→**runTransaction**(prev확인 동시정복 선착1인) · **inbox 멱등성 영속화**(`S._inboxDone[coord][pid]`+prune → 새로고침 타이밍 자원/병력 복제 차단·`_inboxSeen` 세션메모리 불충분 보완). CHANGELOG_MAJOR 「룬테라 전쟁 오픈!」 갱신(멀티 반영)로 팀 공지. **감사 잔여(미수정)**: [A그룹 신뢰팀원 권장]정복후 남은 상대inbox 오정산·감사로그 부모자율기록·롤백스냅샷·큰 저장payload(1800야만+안개 20s통저장) [B그룹 공개필수]🔴**Firebase 규칙 완전공개**(REST로 마을삭제/소유탈취/무한자원/프리미엄위조/골드무한 가능)=Auth+보안규칙(P5·공개오픈 절대전제)·프리미엄 amount↔days 검증·자식 postMessage source검증·villages 크기제한. **감사 확인 OK**: RTDB빈배열 먹통=정규화로 닫힘·오토세이브20s 정상·멀티덮어쓰기 `_ctrlGuard`차단·브릿지 견고(골드 차감전용·노드격리·iframe격리). ★**직전(v650)**: ⚔️**멀티 P4(정복) 완성** — 멀티 마지막 페이즈. 팀원 **2차 마을**만 정복 가능(홈=정복불가·전멸 엣지케이스 회피). 구현: `_runePub`에 **loyalty+home** 발행 · `_opAttack`에서 승리+생존 영웅(noble)→충성도 `-20~35/명`, 0이면 `_opConquer`(마을 편입=로컬 conquerVillage 크로스판·주둔=생존병력·noble 1소모·`_myConquered` 마킹). 수비자 상실=**2중**: ①inbox `{kind:conquered}` ②**등록부 소유권 감지**(`_onRuneWorld`서 내 좌표를 남이 소유→`_lostVillage`, `_myConquered` 가드로 정복직후 자멸 방지). 소유권 이전 안전: 부모 `_runeConquer`가 `claims/<xy>` 강제 set + **발행 시 claim검사**(남이 선점한 좌표는 발행 skip→플립플롭/되살아나기 차단). `attacked`/`scouted` 핸들러에 **자기발신 가드**(정복마을 남은 내 attacked 자기정산 방지). UI: 정보카드 충성도·공격창 영웅 정복힌트·보고서 「마을 상실/정복!」. ⚠️**단일계정 로직만 검증**(Windows엔 Playwright 없어 그쪽 `_p3test.cjs` 못 돌림 → 동일오리진 단일프레임+목부모로 정복 Y·주둔정확·수비상실·홈유지·에러0 확인). **2인 실접속 검증 권장**(P3처럼). 멀티 P1~P4 완료 → 남은 로드맵=금광(보류)·P5 안티치트(신뢰팀원이면 정식 직전). ★**직전(v649)**: 🗺️상단 UI 이모지 배제 리디자인(사장님 "이모지 최대한 배제·깔끔하게"). 부모 나가기바=중립 다크(`#10141c`)+얇은 **chevron SVG**+「나가기」 텍스트·타이틀 자간(`.06em`)·가운데정렬. 게임 topstrip=이모지(🗺️🏅❓) 전량 제거→「룬테라 전쟁」「상점」「도움말」 텍스트버튼(투명 아웃라인·`.shop-btn`만 골드 강조)·스톤텍스처 제거. 프로토 **v0.30**. ⚠️**다기기 충돌 병합**: 이 PC가 UI 리디자인하는 동안 **다른 기기가 P3(v648)를 main에 먼저 push** → 둘 다 v648/v0.29 번호충돌 → `git reset --hard origin/main`으로 **그쪽 P3를 base 삼고 내 UI 4곳(rune-topbar CSS+마크업·topstrip CSS+마크업) 재적용**·번호는 v649/v0.30으로. 그쪽 P3(공격/약탈/정찰·오토세이브 수정) **전부 보존 확인**(grep). ★**직전(v2.45.648·원격web·작업브랜치 `claude/continue-update-1rk0hy`)**: ⚔️**멀티 P3 — 팀원 공격·약탈·정찰 완성 + 🐛서버 오토세이브 미발화 치명버그 수정**. **P3 설계=비동기·공격자 클라 해소**: `_runePub`가 이제 **전투 스냅샷**(`dstack`=주둔+지원 합산·`loot`=숨은곳간 제외 약탈가능분·`def` 실계산·`prot`)까지 발행 → 공격자 `processMarches`의 op 분기가 **`_opAttack`/`_opScout`**로 `twBattle` 해소 → 결과를 상대 inbox `{kind:'attacked'|'scouted'}`로, 내 병력은 loot 싣고 return. 방어자 `_processVilInbox`가 접속 시 **보유량 클램프**로 병력(주둔→지원 순)·자원·성벽 차감 + 📜보고서. UI: `_openOtherInfo`에 지원/정찰/공격 3버튼(보호중이면 공격 disabled)·`openAttack` ally 탭 3종(`allyProt`면 지원만)·`reportCard`에 「정찰당함/정찰 격퇴」. 🛡️**반복 약탈(자원 복사) 차단 = `_opPendingApply`** — 상대가 오프라인이면 서버 스냅샷이 안 깎여 같은 스냅샷으로 무한 약탈 가능 → `_onRuneWorld`가 **남의 마을 미처리 inbox를 스냅샷에 선반영**(attacked=병력·자원·성벽 차감 / support=도착한 지원군을 수비에 합산). 공격자 로컬은 `_opSnapApply`로 즉시 반영. 🐛**치명버그(P3보다 영향 큼)**: `loop()`가 **매 1초 `save()`** 를 부르는데 `_runeSyncDebounced`가 「마지막 호출 후 2.5초」 디바운스라 타이머가 매초 리셋 → **오토세이브가 영원히 발화 안 함**(서버엔 start/conquer/premium 같은 즉시동기화 시점만 저장). 다른 기기 이어하기가 옛 상태였고, P2/P3에선 내 수비 스냅샷이 「빈 마을」로 굳어 무한 약탈 대상이 되는 문제. → **최대지연 보장 스로틀**(`RUNE_SYNC_MS=20000`·`_runeSyncLast/_runeSyncPend`)로 교체. ✅**2인 크로스계정 실검증 완료(P2부터 남아있던 숙제)** — **하네스 `_p3harness.html` + `_p3test.cjs`/`_p3test2.cjs` 리포에 커밋**(gitignore 화이트리스트). 방식=**포트 2개(8907/8908)로 오리진 분리**(같은 오리진이면 localStorage 공유돼 두 계정이 서로 덮어씀)·부모 브릿지(Firebase runewar)를 메모리로 흉내(⚠️RTDB 재현: update 병합으로 inbox 보존 + **빈 dstack은 삭제**). 검증결과: 정찰 왕복·공격 승리(atk13664 vs def1598·수비전멸·loot 2453×3)·방어자 정산(병력 전멸·자원 정확차감·보고서)·**반복약탈 1차15264→2차11817→3차0**·재접속 후 inbox 0 정산·`pageerror` 0. ⚠️**남은 것**: **P4 정복**(팀원 마을 함락=claims 이전·현재 영웅 보내도 정복 안 됨)·실기기 2인(사장님·팀원) 확인·`prot` 발행은 이번에 추가됐지만 기존 저장된 마을은 다음 발행 때 채워짐. ★**직전(v2.45.647·이 PC·main 직접·Pages success)**: 🗺️**룬테라 전쟁 상단 UI 정리** — 「룬테라 전쟁」 2번 나오던 중복 제거. 부모 나가기바=타이틀 **가운데 절대정렬**(rt-spacer 제거), 게임 topstrip=`document.body.classList.add('rune-embed')` + CSS `.rune-embed .logo{display:none}`로 임베드 시 중복 로고 숨김·버전(.world)/🏅상점/❓도움말 재배치. **⚠️다기기 인수인계**: 전부 main push 완료(working tree clean·HEAD=origin). **메모리는 기기-로컬이라 이 CLAUDE.md가 다른 기기의 유일한 기준** → 새 기기 새 세션은 `git fetch && git pull` 먼저. ★**직전(v646)**: 🐛**치명버그 — 서버 상태서 마을 「건물·병력·행군」 박스 통째로 빔**. 사장님 반복 보고("건물 아이콘 사라짐")의 진짜 원인=**아이콘 아니라 박스 전체가 빈 것**. 근본원인=**Firebase RTDB가 빈 배열/객체(`buildQ:[]`·`recQ:[]`·`troops:{}`·`support:[]`)를 저장 안 함** → 서버채택(`_runeAdoptServer`) 상태서 undefined → `updateVillageDynamic`의 `v.buildQ.length` 예외 → **v640 loop try/catch가 삼켜서 조용히 빔**. ⚠️**임베드/서버 전용 — 로컬(localStorage는 빈 배열 보존)선 재현 안 됨**(그래서 여러 번 못 잡음). 수정: `bindActive`에 **전 마을·상태 필드 정규화**(res/blds/troops/buildQ/recQ/support + marches/reports/outSupport). **빈필드 시드로 재현(tiles 0·"Cannot read length of undefined")→수정 후 tiles 15·errs 0** 검증. 교훈: **RTDB 왕복 상태는 빈 컬렉션이 사라지니 로드 후 정규화 필수**. v645=`.bt2 .i` 아이콘색 명시(부수), 상단바 버전 상시표시(캐시확인용). ★**직전(v644)**: 🗺️**멀티 P2(팀원 지원·협동)** — 크로스계정 inbox 기반(`runewar/world/w1/villages/<xy>/inbox`·발행 set→**update**로 inbox보존)·`sendCommand` op타깃·`_processVilInbox`·집결지 회수 UI. ⚠️2인 실접속 검증 필요. **다음: P3 공격/약탈**(신뢰팀원 대상이면 보안은 뒤로·정식직전). 크로스계정 **inbox 기반**: `runewar/world/w1/villages/<xy>/inbox`(부모 `_runeInboxPush`/`_runeInboxClear`, ⚠️발행은 `set`→**`update`**로 바꿔 inbox 보존). 게임: `sendCommand` op타깃(OTHERS서 좌표/키)·`processMarches` op도착 시 상대 inbox로 `{kind:support}` + `S.outSupport` 기록·`_processVilInbox`(내 마을 inbox 처리: support 합류/return 복귀/recall 회수)·집결지 「내가 보낸 지원」 `recallOutSupport`·`openAttack`/`_openOtherInfo` **ally(팀원)=지원 전용**. 회수 왕복: recall→상대가 return 발신→내 홈 inbox. REST왕복+스모크(errs 0) 검증. ⚠️**2인 실접속 검증 필요**(솔로 불가). **다음: P3 공격/약탈**(공격자 클라가 상대 def스냅샷으로 resolveAttack→상대 inbox `{kind:attacked,result}` + 내 병력 return+loot·`_runePub`의 def 실제계산 필요)→P4 정복(충성도0·claims 이전)→**P5 안티치트(정식 게이트)**. **⚠️정식 blocker: Firebase 규칙 완전공개**(부모 `.write:true`면 자식 못막음→`runewar`만 잠그려면 루트 변경=사이트 전체 깨짐. Auth 도입 필요=대공사·정식 직전 별도 프로젝트). 금광/경제=사장님 지시로 보류. ★**직전(v641~643)**: 건물 클릭 씹힘 수정(매초 사이드바 재생성)+상단바 버전표시+기록버튼 제거. **v643 근본원인**: `renderSidebar()`가 **매초 `sb.innerHTML=` 사이드바 전체 재생성**→팔레트 노드가 매초 갈려 탭이 씹힘(특히 모바일). 수정: 구조는 `sb.dataset.k=S.active|보유수` 바뀔 때만 재생성, 자원/인구/점수는 `_sbUpdateStats`(텍스트만), 팔레트 `vil-blds`는 `_bldPaletteSig` 시그니처 가드로 변경 시만. 헤드리스 검증=노드 STABLE·클릭 시트 정상. ⚠️**사장님 보고한 "3D지도 폴백·건물아이콘 없음·건물 반응X"는 제 환경(PC/모바일/임베드/컨텍스트고갈 전부)서 재현 안 됨** → 캐시된 옛 버전 의심 → **상단바에 버전 상시 표시(`.world`="베타 vX · 월드1")** 로 확인 가능케. v641=「📋기록」 버튼 제거(감사로그는 서버 계속 적재)+지도 폴백 배너에 「🔄3D 다시시도」+원인메시지. **⚠️정식 출시 blocker(사장님 문의)**: ①**Firebase 규칙 완전공개**(누구나 read/write·REST확인)=Auth+보안규칙 필수(P5·최대) ②멀티 전투 미완(P1가시성만·P2지원/P3공격/P4정복 남음) ③금광/경제루프 보류 ④3D 안정성. 완료=서버저장·마이그레이션·감사로그·뉴비보호막(15일). ★**직전(v640)**: 🗺️**회귀방어(렌더 예외격리)+뉴비보호막+부족전쟁 워딩정리**. 사장님 보고 "3D지도 로드실패 배너 뜸+마을 건물팔레트 아이콘 사라짐" → 원인=**렌더 예외가 `loop()`로 전파돼 뒤따르는 `renderSidebar`(건물팔레트) 등이 안 돎**. 수정: `renderMap`의 Map3D.build를 **try/catch**로 감싸 실패 시 2D 폴백+`_mapFbErr`에 실제 에러 노출, `loop()`의 tick/뷰렌더/UI렌더 **각각 try/catch 격리**(하나 실패해도 나머지·save 계속). ⚠️**표준 실행(헤드리스 검증)에선 3D·건물팔레트·보호막 전부 정상 = 미재현** → 사용자 환경(임베드 WebGL 컨텍스트 고갈 의심/특정 세이브)일 가능성. **다음 재발 시 배너에 뜨는 에러메시지 확인이 핵심**. 🛡️**뉴비 보호막 실제 부여**(`_doStart`서 `protectUntil=+PROTECT_MS` 15일·기존엔 시스템만 있고 미부여였음·헤드리스서 "신규 보호 중 14일23시간" 배너 확인)·먼저 공격 시 즉시 해제(confirmRelease 기존). 「부족전쟁」→「영지 전략」/「세력」 전량 교체(gs-sub·시작게이트·changelog·major·주석). ★**직전(v639)**: 🗺️**멀티 P1(가시성) 완료**. 확정된 멀티 규칙(사장님): **월드1개·~10명·팀원 서로 공격(PvP)·약탈=게임자원만(골드 무관, 골드는 채굴/상점 전용)·집좌표 최초1회 무작위→서버고정**. 구현: 부모 `RUNE_WORLD='runewar/world/w1'` — **발행**(`_runeSaveState`가 `d.pub`[내 마을 요약]을 `villages/<x,y>`에 set, owner=골드키)·**구독**(`_runeWorldSub` onValue→`runeWorld`)·**선점**(`_runeClaim`: `villages` 점유검사 후 `claims/<x,y>` runTransaction). 게임: `OTHERS`(팀원 마을)·`_mergedWorld()`(owner:'player' 병합·`_OPPTS` 점수)·**안개무시 렌더전용 reveal**(`MREV`, 저장X)·`_runePub()`(발행payload)·팀원 클릭 `_openOtherInfo` 카드·`startGame`→`_claimHomeThenStart`(겹치면 `_rerollHome` 재추첨)·`_runeAdoptServer`서 접속 시 'login' 발행(좌표보호)·`runeInit.key`→`MY_GKEY`. ⚠️**P1은 실유저 다중접속 실검증 필요**(솔로/헤드리스 불가) — Firebase 경로/데이터 shape는 REST 왕복검증 완료. **다음 Phase**: P2 지원(우군 주둔)→P3 공격/약탈(비동기·공격자 클라 해소·상대 mail 리포트)→P4 정복(충성도0·claims 이전)→P5 안티치트(Auth+보안규칙=정식오픈 게이트). 설계 `룬테라-멀티-설계.md`(로컬). ★**직전(v638)**: 🗺️**모바일 진입점+진행안정성 3종**. ①**모바일 진입점**=대문 헤더 안 `.rune-hero-entry` 골드알약(≤540px만 표시·코너 `.rune-badge`는 모바일서 숨김). ②**세이브 마이그레이션** `migrate()`/`_saveOk()`(SAVE_VER 불일치여도 구조 온전하면 승급·폐기X, `load()`+`_runeAdoptServer` 양쪽·서버세이브는 승급후 'migrate' 로그). ③**감사로그 캡**(parent `_runeSaveState`가 push 후 `get`→300개 초과분 `remove`). ④**「📋 기록」 뷰어**(게임 top `openRuneLog`→`runeLogReq`→parent `_runeLogFetch` `get(runewar/log)` 최근100 최신순→`runeLog`→`_renderRuneLog` `.rl-ov` 모달). **📌 로드맵 결정(2026-08-04)**: 🚫**금광=보류**(사장님 — 공유경제 인플레 위험). 🤝**멀티=설계안부터 확정 후 착수**(추천 채택) → 설계문서 `룬테라-멀티-설계.md`(로컬·gitignore로 미추적) 작성됨. **멀티 설계 핵심**: `genWorld(778899)` 고정시드라 **야만인 월드·좌표계 이미 공유**·집만 무작위(멀티 토대 완비, 라이벌AI 이미 제거). 스키마 `runewar/world/w1/villages/<x,y>`(소유자만 씀)·`claims/<좌표>`(runTransaction 선점)·`mail/<키>`(전투리포트 비대칭쓰기)·`presence`. **비동기 전투**=공격자 클라가 도착시 상대 def스냅샷 읽어 `resolveAttack`→공격자세이브+상대mail+상대노드 갱신(닫혀있으면 다음접속 밀린행군 처리). Phase **P1가시성→P2지원→P3공격/약탈→P4정복(충성도0·claims)→P5안티치트(Auth+보안규칙, 정식오픈게이트·현재 규칙 완전공개)**. **⚠️확정 대기 4항목**: ①월드1개 공유(팀규모?) ②팀원 서로공격 허용 vs 협동(지원만) ③약탈=게임자원만(사이트골드 무관) ④P1부터 착수 OK? — **사장님 답 받고 P1 구현 시작**. ★**직전(v637)**: 🗺️**진행상황 서버 저장(계정 귀속)+감사 로그**. 브릿지 확장: 게임이 `runeLoad` 요청→부모 `_runeLoadState`가 `runewar/players/<골드키>` get→`runeState` 회신, 게임 `save()`→`_runeSyncDebounced`(2.5s 오토세이브)/`_runeSync(action,log)`(즉시)→부모 `_runeSaveState`가 `set(runewar/players/<키>)`={schemaVer:1,updatedAt,name,pts,vills,gold,data:S클론} + `d.log`면 `push(runewar/log/<키>)` append(진행검토·롤백 근거). **서버=진실원**(`_runeAdoptServer`: 서버 있으면 채택=다른기기 이어하기, ver===SAVE_VER·home 검증, 없으면 seed 업로드). `_runeLoaded` 플래그로 로드 전 업로드 금지(서버 클로버 방지). `_ctrlGuard` 멀티기기 잠금(false면 서버 저장 skip·로컬 유지). 중요이벤트=startGame('start')·conquer('conquer')·_applyPremium('premium'). ⚠️**Firebase 규칙 공개(read/write 열림)** 확인됨→`runewar` 신규노드 OK. REST 왕복검증 통과. **미구현(다음)**: 스냅샷 전체복원 UI·롤백 도구·감사로그 뷰어·log 무한증가 캡·SAVE_VER 마이그레이션(현재 불일치=fresh 폐기). **직전(v635·같은 날)**: 🗺️**월드맵 3종** ①맵 원점=내 마을(`off=hex(home)`)로 시작·리셋 시 내 마을 중앙 ②「내 성」→내 닉네임(`runeInit.name`→`MY_NAME`→`_mkMarkHtml`·`_vsig` 반영) ③내 마을 6단계 성장모델(`wood_4`오두막→`hide`→`hide2`→`workshop_3/5/6`성·주변 오두막 ring5→0, 사슴조형 main_2/3/5/6 배제). **헤드리스 렌더 팁**: file://는 GLB 바이너리 fetch 막혀 빈화면 → **`python -m http.server`+http://localhost로 우회**해야 swiftshader 렌더 성공(단일/소수 모델만·15모델은 반복 행). 🗺️**대문 진입점(베타)** — 좌상단 corner-badges-left에 「🗺️ 룬테라 전쟁 NEW」 배지(`openRuneWar()`) → `rune.html?embed` **전체화면 전용 뷰**(`#runewar-modal` fixed inset0 z100000·`_injectRuneCss`·상단 얇은 바 「◀ 나가기」만·`.rune-frame` iframe 꽉참). ⚠️v634는 인형뽑기 claw 떠있는 창(`.claw-win`)이었으나 "사이트 안 사이트" 어색 지적 → v635서 전체화면 전환. **"프로토타입/mock" 표기 전부 걷어내고 "베타"로 통일**(title=「룬테라 전쟁」·proto-tag=「베타 v0.16」·임베드면 「상점=공유 골드」·시작게이트/상점 문구 RUNE_EMBED 조건부). 게임=**별도 파일 `룬테라원정-프로토타입.html`**(APP_VERSION/CHANGELOG 무관·독립 진행). 골드 브릿지=인형뽑기 패턴 이식: 자식이 `runeReady` 포스트→부모가 `{type:'runeInit',gold:getMyGold()}` 응답, 프리미엄 결제 시 자식이 `{type:'runeSpendGold',amount,days,reason}`→부모 `_runeSpendGold`가 key·`_ctrlGuard()`·잔액 검증 후 `goldSpent+=amount`(+`_spendLogUpd(data,'runewar',...)`)·`_applySpendLocal`·`update(gold/key)`·`{type:'runeGoldResult',ok,gold,days}` 회신. 프로토타입 v0.15=시작게이트(started·랜덤 시작좌표·「시작하시겠습니까」)+골드상점 프리미엄(1일500·7일3천·30일1만·`RUNE_EMBED`면 부모 골드 차감)+실시간 낮/노을/밤(KST)+PC 건물창 UI+devPoints. ⚠️**멀티(팀원 공유월드 공격/지원/정복)·금광 방치채굴·프리미엄 계정 서버영속=미구현(로드맵)**. ⚠️프로토타입 물리/3D 실기 미검증(이 PC 풀 3D 헤드리스 타임아웃). 상세=세션이력 룬테라 항목. ★**직전(v633·2026-07-28·원격web·작업브랜치 `claude/continue-work-gq5zvj`·main+브랜치 배포·Pages success)**: 🐱**쓰레기통 유미 눈 사라짐 수정**(티모 눈버그와 동일 원인=`Face_Eye_*` unlit BLEND 데칼이 머리에 가려짐 → claw.html 패턴 이식: `ensureYuumi3d`에 BLEND 투명→`alphaTest` 컷아웃+`depthWrite` 복원, 눈/표정 데칼(`eye|mouth|face|brow|explode|shock`) `polygonOffset`) + 🎓**교장 유미 3D 반영**(변신[`yuumiUnique`] 후 쓰레기통 뒤지기/수령대기가 일반 유미로 나오던 것 → `yuumi-principal.glb`(7MB·이미 배포됨) 1회 로드 후 SkeletonUtils 상태별 클론·클립 대기`Idle01`/뒤지기`Dance_Loop`/휴식`Recall`·교장↔일반 전환 시 `_yuumi3dGen` 카운터로 옛 루프 정지+렌더러 폐기 후 재빌드). ⚠️**교장 프레이밍·dig 클립 실기 미검증**(3D 렌더 못 하는 환경 — 어색하면 `YUUMI_POS`/클립명만 조정). 상세=세션이력 v633. ★**직전(v609~615·claw BUILD164~171·2026-07-24~27·원격web·작업브랜치 `claude/continue-update-other-computer-tqqfpt`·main+브랜치 배포)**: 🕹️**인형뽑기 UI 대개편**(치명타 보너스집게 위치=남은인형 겨냥·속도↑·무지개발광·발동 한박자 BONUS_WAIT / 브라우저창 느낌제거[가짜주소창·최대화버튼 삭제]·돋보기 라벨알약[toggleCloseUp이 「🔭물러서기」로 innerHTML 바꿔 원형버튼서 글자넘침이 원인]·치명타「⚡치명타」라벨칩·BGM 클릭수정[#hud가 pointer-events:none이라 슬라이더 막혀있던 게 근본원인]+팝오버·컬렉션 7/7완성후 버튼접기+수량팝오버·코인 상단 골드칩·톤 투기장화·BUILD 좌상단 고정라벨) · 🐾**포로 위로코인 10→50** · 🛡️**자동로봇 강철심장 당첨금보너스 누락 수정**(예약 시점 장착기준 `t.eb`)+**영수증 투여금액 항목별**(`q.spent/free`)+기본선택 실버1→전부0+수량 **+/- 꾹눌러 가속**(window pointerup 정지). ⚠️⚠️**배포 주의(중요·핸드오프)**: 이 세션 중 **GitHub Pages 클래식 자동 트리거가 멈춤**(2시간+ 빌드 안 생성) → **`.github/workflows/pages-rebuild.yml` 신설**(on: push[paths: index.html/claw.html/워크플로우]+workflow_dispatch · `pages:write` 권한으로 `POST /repos/{repo}/pages/builds` 호출=클래식 재빌드 요청=Settings 재저장과 동일·배포방식 안 바꿈). **삭제 금지** — 자동 트리거 또 멈추면 이게 배포 처리. ⚠️MCP github `run_workflow`(dispatch)는 **403**(integration 권한 없음)이라 push로 트리거해야 함. ⚠️**물리·hold 실기 미검증**. 상세=아래 세션이력 v609~615. ★**직전(v593~608·2026-07-24·이 PC·main 직접·라이브 v2.45.608)**: 🤖자동로봇 골드·프리즘 혼합예약(티어별 pity 연쇄) · 🕹️아케이드 대개편(단짝카드 분리→홈 단짝창 관리·진입버튼 3개) · 🗡️투기장 UX(강화 2단·스킨 2단·탭 상단·배틀부터 진입·「랭킹 보러가기」·강화 충전→폭발 연출) · 🕹️⚡인형뽑기 **「치명타 집게」 장비 신설**(claw.html: grabQ≥0.7=치명타 번개 파박 → 랜덤위치 보너스집게 연쇄 ≤4·창 상단 장착/구매칩·claw_tool 타입) · 🖤블랙팟 교차적립(실버/골드/프리즘 유료구매 5%) · 🗡️배틀코인 ~+55%(승45→70). ⚠️**투기장/인형뽑기 물리·연출 전부 실기 미검증**(개발환경서 물리 못 돌림) · ⚠️**claw.html 미러 `인형뽑기-물리-목업.html` 미동기화**(claw.html 기준 동일 편집 재적용 필요) · ⏭️투기장 엔드게임(+20 이후) 초월/코인sink 설계 대기(아이디어 브레인스톰 완료). 상세=아래 세션이력 v593~608. **(이전 v592)** 🖤**블랙 복권 당첨률 24%→31%(프리즘급) + 팟 적립 30%→20% 상쇄** — v591에서 미결로 남았던 「블랙 당첨률 vs 팟 크기」 결정을 사장님이 **옵션 ①(당첨31%+팟20%)** 로 확정. `BLACK_TIER.winChance 0.24→0.31`·`potRate 210→140`. **체감 당첨률=0.9×31%+0.1×1%≈28.0%**(프리즘 30.7%에 근접) — ⚠️체감 31%를 맞추려 winChance를 0.345로 올리면 **최악 회수율 101.4%=돈복사**라 불가(시뮬로 확인). 시뮬(`블랙복권-시뮬.mjs`·300만장) 기본83.5%·불운89.3%·강철심장88.8%·**최악94.1% <100%** ✅. **소급 마이그레이션 재작성**(`_blackPotMigrate30`→**`_blackPotMigrate20`**·`_blackPotEnsureSub`서 호출): 최종 목표=원본(70G/장)의 ×2인데 **v591 mig30(×3)이 이미 라이브 적용됨**(배포 전 REST 실측 `season2/blackPot={amount:420,mig30:true}`) → `node.mig30 ? cur*2/3 : cur*2`로 **420→280 보정**, `mig20`·`mig30` 플래그를 **둘 다** 세워 아직 v591 캐시를 쓰는 stale 클라의 mig30 재적용을 차단(트랜잭션 순서·중복 실행 무관 280 수렴 검산 완료). UI/설명 "30%"→"20%" 갱신(허브 카드 potsub·`?`도움말·구매확인·모달 팟바)·과거 CHANGELOG 값은 이력이라 유지. CHANGELOG_MAJOR 1줄(v591 공지를 팀원이 이미 봤으므로 정정 공지 필요). 7블록 node --check 7/7. ⚠️**실기 미검증**: 소급은 라이브 첫 유저가 복권 Hub 열 때 실행 → **팟이 420→280으로 바뀌었는지 사장님 확인 권장**(`season2/blackPot.json` REST로도 확인 가능). **직전(v591·2026-07-16·원격web·작업브랜치 `claude/resume-work-another-computer-1rnw9x`·배포완료)**: 🖤**블랙 팟 적립률 10%→30% + 소급 적용**(사장님 요청). **경제 원리(중요·재확인)**: 팟은 닫힌 순환(쌓인 만큼 전액 지급)이라 **적립률이 회수율에 그대로 +로 더해짐**(당첨 확률 0.1%가 낮아도 그만큼 팟이 커져 평균 지급=적립률과 동일·1장당 팟지급=potRate 정확히 검산됨). → 10%→30%는 +20%p라 그냥 올리면 인플레(최악 106%). **일반 당첨률 32%→24%로 낮춰 상쇄**(시뮬 `블랙복권-시뮬.mjs` 300만장: 기본77.7%·불운86.6%·강철심장82.7%·최악90.0% <100% ✅). `BLACK_TIER.potRate 70→210`·`winChance 0.32→0.24`. **소급 적용**: 아직 아무도 팟 미당첨이라, 지금까지 70G/장로 쌓인 팟을 ×3=30G 상당으로 보정. Firebase egress 차단 환경이라 **앱 내 1회성 마이그레이션**(`_blackPotMigrate30`·`_blackPotEnsureSub`서 호출·`runTransaction`으로 `season{N}/blackPot` 노드에 `{amount:cur×3, mig30:true}` 원자 적용·`mig30` 플래그로 전 팀원 통틀어 1회만·팟 0/없으면 flag만 세팅해 이후 30% 적립 정상). UI/설명 "10%"→"30%" 문구 갱신(변신 확률 10%·changelog 과거값은 유지). ⚠️소급은 라이브 첫 유저가 복권 Hub 열 때 실행(그 전 curl 확인 불가·egress 차단)—**실제 팟이 3배로 올랐는지 사장님 확인 권장**. ✅**미결 결정 → v592에서 해소(옵션 ① 채택)**. 이하는 그때 경위 기록: 사장님이 v591 배포 후 "블랙 당첨률이 프리즘 정도는 나와야 사람들이 함(돈 다 사라지면 안 함)" 요청. 측정: **프리즘 당첨률=30.7%·골드32%**(scratchpad `prism_sim.mjs`=실 rollScratch 추출), 현재 블랙=24%(팟30% 상쇄로 낮춤). **핵심 충돌**: "프리즘급 당첨률(31%) + 팟30%"는 동시 불가(회수율>100% 돈복사). 시뮬(`블랙복권-시뮬.mjs`) 옵션: ①당첨31%+팟20%(140G·원래2배·최악93.8%) ②당첨31%+팟10%(70G·최악84.5%·가장안전) ③팟30%유지+당첨27%(최악~97%·빠듯·프리즘보다낮음). 사장님이 옵션 선택 거부하고 "핸드오프 먼저" 요청 → **미선택 상태**. 반영 시: `BLACK_TIER.winChance`/`potRate` 변경 + UI "30%"문구 + 시뮬파일 동기화 + **⚠️소급 마이그레이션 재작성 주의**: v591 `_blackPotMigrate30`(×3·`mig30`플래그)이 이미 배포돼 라이브서 실행됐을 수 있음. 팟%를 20%로 바꾸면 목표=원본×2인데, mig30이 이미 ×3 했으면 ×2/3 보정 필요 → 새 `mig20` 마이그레이션은 `node.mig30 ? cur*2/3 : cur*2` + **mig20·mig30 둘 다 세워** stale v591 클라의 mig30 재적용 차단(트랜잭션 순서 무관 수렴 검증함). **직전(v590)**: 🖤**블랙 복권 모달 팟 표시 iOS 사파리 깨짐 수정**(사장님 제보·아이폰 스샷) — 모달 팟 금액이 `_blkOdometer`(자릿수 `0<br>1<br>…9` 세로 스트립+`.blk-od-col{overflow:hidden}` 클리핑+`translateY(-Nem)` 롤링) 방식인데 **iOS 사파리가 이 클리핑을 제대로 못 해 숫자칸이 10em로 늘어나 "5,460"과 "G" 사이 거대한 빈 공간 + 게임판이 아래로 밀림**. → **`_blkPotSimple`(단순 텍스트+카운트업+drain 카운트다운)로 교체**(`#blk-pot-big`·class `blk-pot-live`라 `_blackPotEnsureSub` onValue가 허브와 동일하게 count-up 갱신·드레인 중 `_blkPotDraining && id==='blk-pot-big'` 제외). `_blkOdometer`는 死코드로 잔존(무해). CSS `.blk-pot-drainfx`(지급 시 레드→골드 플래시)·`.blk-od>b{font-weight:800}`. 헤드리스 390px: 팟 밴드=한 줄(odH 23px·빈 공간 0). ⚠️실 iOS 확인 권장(헤드리스=Chromium이라 iOS 버그 자체는 재현 불가·구조상 클리핑 트릭 제거로 해결). **직전(v589)**: 🖤**블랙 복권 허브 카드 실기기 정렬 수정 + 팟 실시간 카운트업 + 팟 획득 순위 + 설명 정정**(사장님 제보 스샷 5건). ①**빈 공간·가격 밀림 수정**: v588의 flex-wrap(`.lh-black-card{flex-flow:row wrap}`+order 1~5)이 좁은 실기기(360px)서 가격이 둘째 줄로 밀리고 줄 간격 벌어짐 → **블록 레이아웃 재설계**(`.lh-black-card{display:block}` + `.lh-black-hdr`[flex·이름/가격]·`.lh-black-hdr-l`[flex-wrap·이름+배지] + 세로 스택). 360/412px 헤드리스 검증 sameRow=true. TIER4 칩 제거(타 카드도 티어칩 없음=더 일관). ②🏆**블랙 팟 획득 순위**: `season{N}/blackPotLog`(`{at,name,amount}`) 구독 `_blackPotLogSub`/`_renderBlackPotWinners` → 카드 하단 `.lh-black-winners`(#blk-pot-winners)에 최고 획득 TOP3(🥇🥈🥉·금액순). ③**실시간 카운트업**: `_blackPotEnsureSub` onValue가 `.blk-pot-live`를 textContent→`_blkCountUp`(600ms ease-out 카운트업+`blk-pot-bump` 그린 플래시)로 → 팀원 구매 시 스르륵 오름. ④**동기화 빨간 점(`blk-live-dot`) 제거**(허브·모달 둘 다·사장님 "안 들어가도 됨"). ⑤**설명 정정**(현 규칙과 어긋난 부분): "낮은확률(1.5%) 해피블랙"→"가끔 블랙 팟 복권 변신"·"8칸 전부 다른 문양/스피또식" 군더더기 제거·불운스택 %수치 제거하고 간결화(happyChance=10%라 "낮은 확률" 부정확했음). ⚠️카운트업/순위/실시간은 헤드리스(Firebase egress 차단)로 실동작 미검증—실기기 확인 권장. **직전(v588)**: 🖤**블랙 복권 허브 카드를 다른 티어 카드(실버/골드/프리즘)와 동일 형태로 통일**(사장님 제보 "다른 복권처럼 모달로 설명 넣고 레이아웃 동일하게") — 블랙 카드가 별도 `.lh-black-card`에 인라인 설명(`.lh-black-desc`)이 박혀 형태가 달랐음. **수정**: ①`_lhBlackCardHtml`을 `.lh-tier-card` 구조로 재작성(`.lh-tier-badges`[TIER4칩+무료권+불운]·`.lh-tier-nameline`[이름+`?`]·`.lh-tier-price`·`.lh-tier-attempts`·`.lh-tier-help`) — 인라인 `.lh-black-desc` 제거하고 그 설명을 `?` 도움말 패널(`.lh-tier-help`)로 이동 ②블랙 통계(내가/팀/최다) 표시 위해 `_lhTierAttemptStats`/`_lhSyncAttempts`를 `[0,1,2]`→`[0,1,2,3]`로 확장(블랙 이력은 `scratch:true·tierIdx:3`으로 이미 기록됨) ③블랙 CSS를 티어 카드와 동일한 가로 행 레이아웃으로(`display:contents` 배지·order 1~5·가격 `margin-left:auto` 우측)+다크테마/블랙팟만 고유 유지. 이름 🖤 이모지 제거(타 카드도 이모지 없음)·불운/TIER4 배지 축소로 [이름·배지·가격] 한 줄 정렬. 헤드리스(실 CSS 추출 하네스·390px) 검증: 블랙 카드 폭=티어 카드 동일·이름+가격 한 줄(sameRow=true)·블랙팟/통계/도움말 세로 스택·`?` 클릭 시 도움말 표시. 7블록 node --check 7/7. **직전(v587)**: 🖤**블랙 복권 광역 긁개 시 가장 왼쪽 「행운 문양」 안 긁히던 버그 수정**(사장님 제보) — v586 광역 이식이 `#blk-grid`의 8칸(`scr[]`)+배경만 긁고, 그리드 밖 좌측 열(`blk-target-col`)의 **행운문양 target 카드(`targetScr`)는 제외**돼 있었음(target 자체 pointerdown은 `_bkCrossOn`이라 스킵·`_bkCrossAt`은 `scr.forEach`만 호출·광역 드래그 시작이 `#blk-grid` 안에서만 발생). **3곳 수정**: ①`_bkCrossAt`에 `targetScr.cross(e)` 추가 ②`_winUp` crossEnd 루프에 `targetScr.crossEnd()` 추가 ③드래그 시작 pointerdown을 `#blk-grid`→**`.blk-play`**(행운문양 열+그리드 모두 포함)로 확장. 헤드리스 최소하네스(실소스 attachScratch/`_bkCrossAt`/`.blk-play` 재현) 검증: 좌측 행운문양 커버 0%→40% 긁힘·공개·onReveal 발동·그리드 8칸 회귀 없음. 7블록 node --check 7/7. **직전(v586)**: 🖤**블랙 복권 광역 긁개 배경 은박 이식**(사장님 제보 "칸만 긁히고 배경 안 긁힘") — 기존 복권 `scard-bg-canvas` 방식을 블랙에 이식: `blk-bg-canvas`(그리드 inset:0·z1·기본 display:none·`.blk-cell` z2·`.blk-grid` position:relative) + `attachScratch`가 `cross(e)`/`crossEnd()`/`canvas` 노출 → `_bkCrossAt`이 배경(`_bkBgStroke`)+지나가는 칸 함께 긁음. `#blk-grid` pointerdown이 광역 드래그 시작(`_bkCrossDrag`·칸별 pointerdown은 `if(_bkCrossOn)return`로 스킵)·`_winMove`서 cross 분기·`_winUp`서 `crossEnd`. 장착/해제·구매 토글이 `_bkSetupBg()`/`_bkHideBg()` 실시간 호출. 프리뷰 `blackScratchPreview({cross:true})`=`forceCross`. 헤드리스 검증(광역=배경 은박이 칸 사이 덮어 한 장). ⚠️드래그 실동작은 헤드리스 미검증(포트는 기존 검증된 구현 1:1). **직전(v585)**: 🖤**블랙 복권 「이만 확인」 버튼 제거**(사장님 제보 "버리기랑 중복") — 블랙은 매치=자동 확정(남은 해골 안 긁힘)이라 「이만 확인」은 항상 꽝 정산(노출 해골 감점)=버리기(0G)보다 손해라 존재 이유 없음. `#blk-early` 버튼·핸들러·`onCellReveal` disabled해제 3곳 제거·긁는중 버튼=「🗑 버리기」만·게임방법 문구 "나오는 순간 자동 확정" 명확화. **직전(v584)**: 🖤**블랙 복권 개선**(사장님 요청 5건) — ①**블랙팟 변신 확률 1.5%→10%**(구매 10%가 블랙팟 복권으로)·**변신시 당첨확률=1%**(`happyChance:0.10`·`happyWinChance:0.01`→팟 실지급 0.1%=1/1000장·평균팟 ↑72k~최대54만·시뮬 **리포 `블랙복권-시뮬.mjs`**(현 파라미터 일치·`node`로 재실행) 기본75.4%·최악86% <100%·EV 재조정 시 이 파일 상수만 고쳐 재검증) ②**블랙팟 티켓은 각 칸에 개별 당첨금 대신 「🖤 팟 금액」 표시**(`potcell`·핑크·`_potLabel=_blackPotAmt`) ③**변신 연출 화려**(`_blkTransformFx`=플래시 골드/핑크 라디얼+방사 파티클 46개+「🍀 블랙팟!」 스탬프 `blkStamp`+상승 차임+햅틱) ④**긁는 느낌 기존 복권과 완전 통일**(별도 arc erase→기존 `scratchLine`(destination-out 완전불투명+끝점청크)+`spawnDust`격 블랙/골드 팔레트 `_bkDust`+`_bkGlint`반짝임+**모듈레벨 `_lscPlaySound('scratch')`/`_lotVib`**+동전커서 `_injectScardCoinCss`/`scard-coin`+**가운데 52% 공개판정**·디바이스px 직접드로우[setTransform 제거]) ⑤**긁개 도구(만능·광역) 블랙에도 적용+모달 내 장착/해제/구매**(`blk-equip-acc`·기존 `scard-equip-*` CSS 재사용·delegation 핸들러·`item.active` 플립→라이브 brushMult/threshold). ⚠️블랙팟(해피) 티켓=불운 스택 별도트랙(`_rollBlackPity` early return). 헤드리스 실사이트 검증(정상/블랙팟/리빌/모바일 OK·긁개 ID 정합·7블록 node --check 7/7). **직전(v583)**: 🖤**블랙 복권 정식 통합(TIER 4·스피또식)+블랙 팟(공유 잭팟·해피 블랙)** — 목업(`블랙복권-목업.html`)→index.html 통합. 프리즘 위 700G 등급, 행운문양 1+나의문양 8칸(4×2·전부 유니크·💀2칸-20G), 매치 1개면 그 칸 당첨(최고 5만G·매치 즉시 확정·이후 구경용). **🖤 블랙 팟**=구매금 10%(70G)를 `season{N}/blackPot.amount`에 `runTransaction` 적립(전 팀원 공유·onValue 실시간·오도미터 롤링), 🍀**해피 블랙**(1.5%) 변신 티켓 당첨 시 칸+팟 전액(지급도 runTransaction=동시당첨 이중수령 불가·`blackPotLog` 감사·쭈르륵 0 드레인 연출). EV(scratchpad `black_sim2.mjs`·3백만장): 당첨32%·기본83.5%·불운89.2%·강철심장max89.6%·**최악94.2% <100%=인플레불가**(팟=낸돈에서만·시드0·취소 시 적립원복). 기존 시스템 완전 연동: 불운 스택(`lotteryPity.black`·`LOTTERY_PITY_KEY[3]='black'`)·강철심장 해골감소(해골칸→문양 치환)+당첨보너스·무료권`freeScratch[3]`·보류/이어긁기(`pendingScratch` black:true 분기)·버리기·긁기도구·`lotteryHistory`(happy/potWin/potAdd/pity 필드). 코드맵(index.html·`window._lhResumeScratch` 뒤 ~L29627+): `BLACK_TIER`·`rollBlackScratch`·`_rollBlackPity`·`_blackPot*`(Ref/Add/Claim/EnsureSub)·`_blkOdometer`·`_lhBlackCardHtml`(허브 카드·`openLotteryHub`서 3티어 그리드 뒤 호출)·`_lhBuyBlack`/`_buyBlackScratch`·`_blackHandlers`·`_lhResumeBlack`·`_injectBlackScratchCss`(`blk-*`/`lh-black-*`·모바일 ≤480 축소)·`showBlackScratchModal`. `_lhResumeScratch`/pendingCard/지출로그(`[70,200,400,700]`)/버디통계/우편라벨에 블랙 추가. 프리뷰=콘솔 `blackScratchPreview({happy,win})`(실데이터·팟 무수정). 헤드리스 실사이트 검증(iframe 390px): PC/모바일 허브·모달·해피 OK·7블록 node --check 7/7. ⚠️**실기 미검증**: 실구매→보류→이어긁기→해피 당첨(팟 지급) 1사이클은 실계정 확인 권장(개발환경에 Firebase 실흐름 없음). ★**직전(v579~582·2026-07-15·이 PC·홈페이지)**: 🖥️**v582=복권 Hub PC 카드 줄맞춤**(사장님 제보) — 프리즘 카드에만 있는 🍀불운 배지가 세 카드 제목/무료/통계 줄을 밀던 것 → 배지 2개(무료권+불운)를 `.lh-tier-badges` 래퍼로 묶고 `min-height:36px` 예약해 배지 0/1/2개 무관 같은 Y에서 시작(CSS ~L2255·렌더 ~L29420·배지 `line-height:1` 높이안정). 모바일(≤480px)은 `.lh-tier-grid .lh-tier-badges{display:contents}`로 래퍼 무력화→가로 행 그대로(변경전후 카드높이 동일). Edge 헤드리스 실측=PC 제목/가격/통계 편차 0px·모바일 회귀 없음. · 🤖**v581=복권 자동 로봇, 무료 복권 먼저 소모**(사장님 요청) — `autoBotReserve`가 예약 시 보유 무료 실버 복권 `freeScratch[0]`을 `useFree=min(n,보유)` 먼저 소모하고 나머지 `payN`장만 골드 결제(`cost=payN×70`·`cost>0`일 때만 goldSpent/spendLog·`useFree>0`면 ft[0] 차감). 예약 패널(`_autoBotRefresh` else-branch ~L28480)에 "무료 복권 N장이 먼저 사용돼요" 안내+`costLabel`, 완료 토스트에 무료/골드 사용내역. 코드 ~L28530. node --check 7블록·결제로직 시뮬 검증. ⚠️실기 미검증. · 🧠**오늘의 롤 퀴즈(출석 연동)** — 사장님 요청. 출석(오전/오후) 시 슬롯당 롤 퀴즈 1문제 열림·4지선다·정답=복권 3종(`freeScratch{0,1,2}` 각 1)·오답=해설만(재시도X). 문제=**밸런스 패치 무관 불변 사실만**(챔피언 정체성·세계관 로어·롤 역사 **30문제** `QUIZ_Q`)이라 오답 위험 없음(⚠️애초에 "칼바람/아수라장 밸런스·증강" 방향이었으나 변동성 커서 사장님과 로어·역사로 선회). 보기 순서 `_bingoShuffle`로 매번 무작위(원본 정답 전부 idx0). **v580=가용 판정 일일 리팩터**(사장님 제보 "오늘 이미 출석한 사람도 바로"): 기존 `quizAvail` 열람권누적(새 출석 시만 +1) **폐기** → **오늘 풀 수=(오늘 출석 슬롯 수 `lastAttendance`/`lastAttendance2`)−(오늘 푼 수)** 파생(`_quizAvailToday`)이라 이미 출석 끝낸 사람도 즉시 풀림. 코드맵(index.html·`_scratchTierName` 뒤 ~L22040+): `QUIZ_Q`/`QUIZ_REWARD`·`_quizAvailToday`/`_quizState`·`openDailyQuiz`/`_quizRenderQuestion`/`_quizAnswer`/`_quizResult`/`_quizInjectCss` · 진행상태 `quizIndex_s2`(총 푼수·상한30)+`quizDate_s2`+`quizAnsweredToday_s2`(일일리셋) · `showBingoReward` **5번째 인자 `onClose`** 추가(출석팝업 닫으면 자동오픈) · `renderBingoBoardHtml` bg-actions에 「🧠 오늘의 퀴즈」버튼+진행도. 검증: node --check 7블록·데이터무결·상태머신(2/일·미답누적·오답 보상없음·30상한)·일일모델(이미 출석자 즉시열림)·렌더(보기4·정답위치 분산). ⚠️**실기 미검증**(개발환경에 Firebase 출석 흐름/실계정 없음 — 로직·문법·데이터만 검증). 배포 완료(Pages success·라이브 v2.45.580). ★**직전(v576~578·2026-07-09·이 PC·오버레이 연동)**: ⚔️**승급전 표시 명확화**(v578·내 정보바·오버레이 「⚔️ 승급전 N승 N패」 금빛 펄스 배지) + 🎮**오버레이 안내 팝업**(v577·계정당 1회) + 🥊**막고라 결과 오버레이 주입**(v576·`overlayMagollaResult`·숨은 liveWin서 `mgSettleMatch`). ⚠️**오버레이(별도 리포 aram-overlay·v0.1.50)가 이 세션 주 작업** — 헤더 지갑 1열·최소화 버튼·클라종료 다이얼로그·자동로그인 제거 등은 그 리포 CLAUDE.md. ★**직전(v573~575·2026-07-08·다른 PC·오버레이/계정 관련·투기장 무관)**: 🖥️**오버레이 정산 발행**(v575·`overlayForceSettle`=전원 투표 안 돼도 오버레이 방장이 지금 표로 `autoConfirmMvp`→저장→`finalizeVotes`→`lastSettlement` 발행·미투표자 있으면 '건너뛰기'가 숨은 라이브계정에만 있어 정산창이 영영 안 뜨던 문제) + 🔒**계정 조작 잠금 B안**(v574·`accountSession/{fbKey}={deviceId,app,label,at,hb}`·보기는 자유·조작만 한 기기·`_ctrlGuard()`[비어있음/stale75s/내기기=자동클레임·다른기기=확인후 takeover]를 개인 경제 뮤테이터 15곳 게이트[buyItem·toggleItemActive·quick*·doGachaPull/Merge·toggleGachaSynergy·emblem 7종·claimS2PassLevel]·하트비트25s·onDisconnect 반납·liveMode 제외·강제력없는 사고방지용=last-write-wins 증발 차단) + 🪟**시즌 안내 팝업 작은화면 스크롤**(v573·오버레이 홈창서 잘림 수정). ★**직전(v572·2026-07-08·원격web·작업브랜치 `claude/continued-update-1srahn`·배포=`main`+작업브랜치 둘 다)**: 🛡️**투기장 「내 방어 순서」를 강화창(openArenaForge)에도 노출** — 그동안 배틀 화면(`arena-defset`)에만 있던 것을 강화창 하단(인형 목록 아래·탭 위)에도 추가. **전투(장착) 인형 기준**(`eqF=_arenaActiveFighter`)이라 그 인형 이름+스킬명 표시. 슬롯 탭=Q→W→E 순환(`_arenaSetDefOrder`), **전체 재렌더 없이 제자리 갱신**(`b.querySelector('b')`/`span` textContent만)해 3D 인형 안 깜빡임. `.arena-defset/.ads-*` CSS 재사용·SB(샌드박스) 제외. ⏭️프로필 노출은 미적용(원하면 추가). ★**직전(v571·2026-07-08·Pages run#2012 success)**: 🗡️**투기장 배틀 중도 이탈=패배 처리**(사장님 제보 "상대 전투 안 보고 배틀만 돌리고 나가는 것") — 대결 시작 시 이미 배틀이 커밋(v570 정찰 어뷰즈 방지)되므로, 경합 연출이 끝나기(결과화면 showFinish) 전에 X/바깥클릭으로 닫으려 하면 **「지금 나가면 패배로 처리돼요 · 결과를 끝까지 볼까요?」 확인창**(인라인 스타일·`#arena-forfeit`). [계속 보기]=유지 / [나가기(패배)]=`_arenaBattleForfeit(opp,res)`로 그 배틀을 패배 정정(승리 커밋이면 코인·승패·로그 하향·`forfeit:true`·방어자 코인은 드문 자발기권이라 유지)+토스트. 상태 `_pendingBattle`(startClashes 커밋 후 set·showFinish서 null)·`attemptClose()`가 X 3곳+바깥클릭 통합. 확인창 열린 새 결과 뜨면 `_pendingBattle` 가드로 이미 본 결과는 하향 안 함. node --check 7/7·Pages 확인예정. ★**직전(v570·2026-07-08·Pages run#2010 success)**: 🗡️**투기장 배틀 정찰 어뷰즈 차단**(사장님 제보) — 배틀 결과 커밋(`_arenaBattle`: 일일횟수·쿨다운·정찰·코인)이 애니 종료 후 `finishBattle`에서 일어나는데, `finishBattle`의 `if(finished||!document.body.contains(ov))return` 가드 때문에 **상대 방어 스킬이 공개되는 경합 연출(~5초) 도중 X로 나가면 소비가 스킵**됐음 → 상대 방어 순서만 정찰 후 카운터로 재도전하는 어뷰즈. **수정**: `startClashes`에서 `clashRes`/`totalBonus` 계산 직후 `doBattle` **즉시 호출**해 결과·소비·쿨다운·정찰을 「대결 시작=상대 스킬 공개 시점」에 커밋. 애니는 연출 전용(`bonus` 누적 제거·`finished` 가드로 중복 방지), 종료 시 `finishBattle(res)`가 `showFinish`로 확정 결과만 노출(닫혀 있으면 스킵·이미 소비됨). 정찰 기억(`arenaScout`)은 유지·무한 정찰만 차단. 샌드박스(`_arenaBattleSB`)도 동일(무해). node --check 7/7. ★**직전(v569·2026-07-07·원격web·작업브랜치 `claude/continued-update-1srahn`·배포=`main`+작업브랜치 둘 다·Pages success)**: 🗡️**투기장 강화 확률 단조 정상화 + 초반 난이도 하향**(사장님 제보 "12성보다 13성이 더 쉬움") — 이전 `ARENA_ODDS`(~L34697)는 v395가 +13부터 **파괴율만** 확 낮춰 +12(성공26·파괴9)보다 +13(성공28·파괴5)이 오히려 모든 면에서 쉬운 역전이 있었음. → **성공률=강화할수록 강한감소·파괴율=강화할수록 비감소(둘 다 단조)**로 재설계 + 전반 난이도↓(성공↑·파괴↓·파괴 캡 6%)로 초반 키워서 팔기 원활. 새 곡선 예: +0[97,0]·+5[71,2]·+7[59,2]·+10[43,3]·+12[35,4]·+15[26,5]·+19[18,6]. **판매가는 `_arenaExpCost`(R[N])가 페이지 로드마다 이 확률에서 자동 재계산**(코드 무변경)→평균 손해(환급률 `ARENA_SELL_REFUND`0.72<1) 유지=무한증식 방지·손익분기 +7 그대로·고강 럭키 잭팟(+16~19=2.8x 상한) 유지(scratchpad `arena_verify.mjs` node 검산: 단조 O·모든 레벨 판매<기대투입 O·손익분기 +7 O). 도움말/UI는 `${odds.success}` 템플릿이라 자동반영. 상세=세션이력 맨 위. ★**직전(v533~544·2026-07-03·원격web·작업브랜치 `claude/resume-work-in-progress-bdxxwh`·배포=`main`+작업브랜치 둘 다·⚠️Firebase egress 차단[프록시403]→러너우회)**: 🗡️**투기장 코인 병목완화**(v542·강화비용 2차항0.8→0.62[+10 130→112·+15 250→210]·배틀 승38→45/패15→18·출석100→130·훈련봇20/8→28/10·판매환급0.6→0.72·상한2.5→2.8·검산=일일수급 PvP445/봇320·판매손익분기 여전히+7·평균손해 유지=무한증식 방지) + 🗡️**강화창 미보유 챔프 노출**(v543·`arena-pick`이 owned만→전7종·미보유 회색+🔒"미보유"·탭=3D미리보기만·`getStock>0`일때만 `setFighter`) + 🧸**아케이드 단짝 3D 세로늘어남/뜸 수정**(v544·공유 3D캔버스 190×212버퍼가 CSS width/height:100%로 무대[.bsp-stage 138×180=0.77·모바일108×142=0.76]에 늘려그려짐→`object-fit:contain`+`object-position:50% 100%` 바닥정렬·`_BUDDY_MODEL_ADJ` 튜닝 유지·아바타bust 0.89라 영향미미) + 🧸**프로필바 S2 리뉴얼**(v535~540·헥스워터마크·재화칩[골드강조/뽑기·투기장 라벨+흐리게]·모바일 3D→2D 아바타[알림 위치때문]·닉네임 안잘림[`word-break:keep-all`]·만렙LP강조[왕관 crown·LP체력바 10단위눈금]) + ⚒️**강철심장 애칭**(v534·최대3글자·`emblemNick`/`emblemDoNick`·✏️버튼·`emblems_s2[].nick`)·**보유 6→15**(v541·`EMBLEM_MAX_OWN`) + 🎟**복권**(v533 무료권배지 in-flow칩[모바일 이름가림 수정]·v540 PC재배치+매칭 금실선 제거+마지막 긁기 두근두근[`_updateReach` 최상단 return] 제거). ★**직전(v513~522·2026-07-01·원격web·작업브랜치 `claude/resume-work-in-progress-bdxxwh`·배포=`main`+작업브랜치 둘 다·⚠️이 환경 Firebase egress 차단[프록시 403]→읽기/쓰기는 GitHub Actions 러너 우회만 가능[`.github/workflows/<임시>.yml`을 작업브랜치 push로 트리거→`get_job_logs`→파일제거·`.gitignore`가 `*`라 `git add -f`·RTDB 읽기공개 무인증])**: 🤖**복권 자동 로봇 신규**(v522·실버복권만 자동긁기·영구소유 아이템·복권Hub 진입카드서 구매/조작·등급 실버/골드/프리즘=한번에 5/10/20장·장당 10/5/1분·단계업글 300→700→1000G[`autoBotTier_s2`]·예약 시 장수×70G 선불차감+`rollScratch(0)` 결과 미리 굴려 저장[`autoBotQueue_s2{startAt,perMin,count,claimedCount,tickets}`]·실제시각 기준 **순차** 완성=오프라인OK·조작불가·완성분 영수증[당첨/꽝]+「수령」→`passGold`·CSS로봇 `.abot-*`/`_injectAutoBotCss`·`openAutoBotModal`/`autoBotReserve`/`autoBotClaim`/`autoBotBuy`·해골 있는 골드/프리즘 복권 자동화는 미지원). 🪙**상점 구매 즉시반영**(v521·엠블럼 buy 함수[`emblemBuyBase/Ticket/Essence`]가 `goldData` 로컬 미갱신→`renderShop`만 불러 상단바가 onValue 왕복 후에야 갱신되던 문제 → `_applySpendLocal`[낙관적 `Object.assign`+`updateMyInfoBar`]·일반아이템/비밀퀘도) + 💸**골드 지출 로그**(그동안 무기록이던 엠블럼 구매를 `goldSpendLog_s2`에 기록[`_spendLogUpd`]·콘솔 `goldSpendLog("닉")`=엠블럼+상점(purchaseLog)+복권(lotteryHistory) 시간순 통합). 🎰**막고라 배당 걸작 성능비례 상승**(v517~518·`perCap:10`이 성능14서 막던 것 제거→줄당=`round(5×(1+성능×0.1))` 성능0=+5%~30=+20% 계속 상승·`base 4→5`·총 `cap 30→60`=3줄 만강화 60%). 🎯**협력 빙고 출석 대개편**(v516·`CHANGELOG_MAJOR`)=①🔥연속출석 스트릭(3·7·14·30일 마일스톤 자동지급 `ATTEND_STREAK_REWARDS`·attendanceHistory 날짜계산 `_attendStreak`) ②📦주간상자(주5일 `BINGO_WEEKLY_GOAL`·`bingoClaimWeekly`) ③🌟황금칸(첫발견 보너스)·💀함정칸(그 줄 보상 절반 `BINGO_TRAP_MULT`)=`cells[].sp` 블라인드 ④🤝라운드(3줄) 전원완성 보너스(`clears[]`·`bingoClaimClears`) ⑤🏅기여명예(`bingoCells/bingoLines_s2`·TOP3). 데이터 개인 `bingoCells/bingoLines/attendStreakClaimed/weeklyBoxClaimed/roundClearClaimed_s2`·공유판 `season2_bingo.clears`/`cells[].sp`. 🎟**복권 긁기 UX**(v515 「다시하기」 깜빡임=옛 모달 먼저제거→여러 await→새 모달 순서라 Hub 비침 → 새 모달 먼저 생성 후 옛것 제거 / v519~520 긁는중 결과·설명·버튼 뜨며 모달 커져 격자 밀림→오긁힘 → `.scard-progress/actions/result`를 `display`→`visibility` 토글로 **공간 예약**해 높이고정 + `.scard-overlay` `align-items:safe center`). ⚡**첫 진입 속도**(v513~514·폰트 preconnect·기록/랭킹 탭은 진입 시 렌더[`_dirtyRecord/_dirtyRank`]·헥스배경 idle 생성·**모바일은 배경 헥스 펄스 생략** 정적격자만[`_buildHexfield` `matchMedia(max-width:768px)`]). 🔍**애긔반달곰 골드감소 진단(코드무관)**: 버그아님 — 잔액이 획득≈지출로 바닥근처 + 엠블럼 강화권 대량구매(비복권 지출 ~68k G)가 원인. 복권은 순이익(+15,565). 공용 테스트계정이라 타인 소비 가능성도. 상세=세션이력 맨 위. · **직전(v489~512·이 PC[`C:\Users\sbs_n\Desktop\aram`]·`main` 직접 푸시·Firebase REST 직접 가능)**: 🎟️**스크래치 복권 대개편** — ①**🖌️광역 긁개 근본버그 수정**(`_lhBuyTier` activeTool 배열에 `scratch_cross` 누락돼 새로 산 복권 전티어 광역 미적용이었음·v494) ②**복권 전체를 하나의 은박으로**(원 사이 배경 `scard-bg-canvas` z1·원=z2·번호헤드=z0흐림, 번호도 은박에 인쇄돼 함께 긁힘·v493·507) ③**상단=복권카드/하단=긁개 아코디언**(`scard-equip-acc`·긁기도구 인벤토리서 숨김·v495·497) ④**동전 커서/터치 팔로워**(`_injectScardCoinCss`·`scard-coin`·v504) ⑤**복권 「🔄 다시하기」**(버리기+같은티어 재구매·v506) ⑥**Hub 재디자인**(?이름옆·이모지제거·골드 우상단칩·X 좌측·설명→?·가격 크게 우측·모바일 가로행·v511~512) ⑦**Hub 모달 뒷배경 스크롤 잠금**(v510) ⑧모바일 실버 4칸 1열(v505). + 🧸**프로필바 단짝** — **bust 값 7종 튜닝**(목업 `프로필바-3D-위치조정.html`·Firebase `config/buddyModelAdj` bust도 갱신 필수·v499)·**기본 idle 정정 4종**(티모/벡스/에코/니코 CHAMP_CLIPS.idle이 전환클립→looping·v500). + 🔨**대장간** — **⚡빠른 강화 토글**(`forgeSkipAnim`·오른 모션 생략·마지막 망치만 `ornn3d.playForgeTail`·v501~502)·걸작정수 릴 ~30%↑(v503). ⚠️**CSS 소스순서 함정**: 기본 `.lh-tier-card{flex-direction:column}`이 `@media(max-width:480px)`보다 **뒤**라 모바일 오버라이드는 `.lh-tier-grid` 스코프(우선순위↑)로 해야 먹힘(안 그러면 flex-basis:100%가 column서 세로 100%로 터짐=v508 대참사). 상세=세션이력 맨 위. ★**직전(v485·이 PC·작업브랜치 `claude/resume-work-in-progress-bdxxwh`)**: 🏘️**광장에 💎억제기·🛒상점지기 배치 추가** — 다운로드 GLB 4개를 리포로 복사(`inhibitor_blue.glb`/`inhibitor_red.glb`·`shopkeeper_order.glb`[블루/오더]/`shopkeeper_chaos.glb`[레드/카오스])+`.gitignore` 화이트리스트. **prop 시스템 일반화**(nexus/turret 이분 하드코딩 제거): `_PROP_GLB`에 4키 추가(`inhibitorB/R`·`shopkeeperB/R`)·`_PROP_SIZE`{inhibitor:3·shopkeeper:2.4}·`_PROP_LABEL`·헬퍼 `_propType(k)=String(k).replace(/[BR]$/,'')`로 `_addProp` 크기·토스트·`sz()`·`_propSize` 필터 전부 일반화. 배치 팔레트 `sels`에 🔵🔴 억제기·상점 4버튼 추가(8개). 🛒**캐릭터 idle 애니**=`_loadProp`이 `{scene,anims}` 캐시(이전 scene만)→`_addProp`서 idle(없으면 첫)클립 mixer 재생·렌더루프(`loop`)서 `_placed[].mixer` 업데이트(`_SkelClone`로 스킨 클론). 미러는 키 끝 B↔R 그대로. ⚠️상점지기 idle 없으면 바인드포즈(정지). 🔧**[데이터 정정·코드무관·2026-06-30밤] 가짜 매치 되돌리기**: 안 한 경기가 기록됨(매치 `-OwNlSsihGQL1xQUoDCB`·22:44 KST·S2·teamB승) → 익명auth REST로 직접 정정(scratchpad `revert_match.mjs`·DB공개read+익명auth write·apiKey/DB URL=index.html firebaseConfig). ①매치삭제(골드·MVP·전적은 매치 재계산이라 자동복원) ②S2 LP 역산(`season2/players/{name}`): ap렉사이서폿 골드·LP100·승급전1승1패[잘못된 플래승급 취소]·나랑듀오해듀오 골드·LP95[승급전진입 취소]·맹독 실버53·신규회원임 플래88·애긔반달곰/울퉁쓰 LP0유지 ③아이템 환급(`gold/{key}/items_s2` append): 도박권 맹독·애긔반달곰·승급전방어권 ap렉사이서폿. ⚠️**미처리=단짝 연승/연패 스택**(`_buddyProcessMatches` 기기별 watermark처리라 어긋나면 수동보정)·골드 자동재계산이라 그새 소비분 있으면 잔액 오차 가능. 📌**되돌리기 패턴 재사용**: 공개 REST로 matches/season2·players/gold 읽어 분석→익명auth(`accounts:signUp`)로 PATCH/DELETE/PUT, ⚠️LP는 상한100/바닥0/승급경계서 pre값 소실되니 당사자에게 원래값 확인 필수. ★**직전(v480~484·원격web·작업브랜치 `claude/resume-work-in-progress-bdxxwh`)**: 🏰**광장 배치 다듬기**(v484·이동스텝 세밀화 가로0.15/높이0.1·**Firebase `plaza_props` 로드 추가**=저장 실제 반영[`get`→`_addProp`·전원공유·패널없어도 보임]·배치패널 좌하단 컴팩트 박스[`width:198·max-height:46vh·overflow auto·반투명`]로 이동해 화면 덜 가림) + 🏰**포탑 개별 선택 편집**(v483·`_selIdx`/`_selProp`/`_selectProp`·노란 선택링 `_placeRing`[빌드패널 열렸을 때만·depthTest off]·`◀선택/선택▶`로 1개만 이동/회전/크기·설치 시 자동선택·`🗑삭제`=선택분·스텝 세밀화 가로0.3/높이0.2·회전 ↺↻ π/24·미러는 placement에만·sz/nudge/rot 모두 `_selProp` 1개 대상) ← v482 "다 같이 움직임"·"너무 큼"·"방향 회전" 3건 해결. + 🏰**X·Y·Z 자유 이동**(v482·절대 `y` 저장·절대 `y` 저장[yOff 폐기]·`nudgeLast(dx,dy,dz)`/`rotLast`/`_targetProps`/`_groundAlign`/`_setPos`·축별 넛지버튼 r3행[◀X/X▶/Z−/Z＋/높이−/높이＋]+↻회전·미러는 x/z 반대·y동일·리사이즈 베이스고정·저장에 절대y) ← 이전 ▼박기/▲올리기(yOff)는 폐기·각축 자유이동으로 대체. + 🏰**포탑 배치 버그 2종 수정**(v480: 크기조정 사라짐 h0·여기로 이동) — ①크기 조정 시 사라짐(스케일된 모델서 높이 재측정→0수렴 → `_addProp`서 원본높이 `h0` 저장·`sz`/`_propSize`서 `p.h0`로 재계산) ②설치 포탑 이동수단 없음(모바일 바닥터치 막힘) → 「🚶이동」을 **「📍여기로 이동」**으로(마지막 설치 포탑+미러를 캐릭터 위치로·`moveLastHere`/`_moveProp`/`_mirKey`). · 직전 **최신 세션(v447~479·2026-06-30~·로컬·🏘️단짝 광장[접속 팀원과 3D 소환사협곡을 함께 걸어다니는 공유 마을] 대장정 — 아래 세션이력 맨 위 必)** · 직전(v442~443·원격web·`claude/resume-work-in-progress-bdxxwh`): 🧸**아케이드 단짝 패널 좌(3D)/우(정보+버튼) 가로 레이아웃 + 🗡️투기장 버튼(강화/배틀/스킨) 단짝 우측에 통합**(v443·`_arenaActionsHtml`→`_buddySpaceHtml` 4번째 인자·`.bsp-arena*`·`.bsp-3d` 너비고정으로 캡션 늘어남 방지·`@media`서도 가로유지·`_prismClawHtml`서 `_arenaEntryHtml` 제거) + 🧸**단짝 패널 프로필→아케이드 탭 이동**(`_buddyArcadeHtml`=`_prismClawHtml`서 `_buddySpaceHtml` 인라인·renderGachaTab gachaStar===4서 `.wm-card.in`+`_buddyHomeAttach`·프로필 isMe는 이동안내만·남읽기전용/프리뷰 유지) + 🎁**단짝 보상 알림 2버튼화**(`buddyClaimPrompt`=confirm→`brc-*` 모달: 불꽃게이지·지금/다음보상·스택표·「📈더쌓기(=스누즈 `buddyRewardSnooze_s2{type,count}`·`_coachActions`서 같은타입 count≤스누즈면 숨김·`buddyClaim`서 null리셋)」/「🎁지금받기」 — 무심코 수령 방지+도파민). · **직전 v438~441(다른 PC)**: 🔨**강화창 3D 인형 다리만 보이던 문제 수정** — `_forge3dShow`가 풀모델(`_full.glb`/스킨)을 자체 fit-to-sphere로만 맞춰 모델이 크고 위로 떠 **다리만 노출**되던 것을, 스킨샵 미리보기(`_skinPreview3d`)와 **동일한 카메라 수식**(`baseDist=R/sin(fov/2)*1.18`·`baseY=-R*0.05`)+`CHAMP_PREVIEW[glbFile]`의 `{zoom,yOff}` 보정 적용으로 전신이 프레임에 들어오게 통일(스킨키 `*_skin1.glb`도 같은 테이블이라 동일 적용·줌은 camera distance로 처리해 강화펄스 group.scale과 무충돌·`CHAMP_PREVIEW`는 `_forge3dShow`와 같은 `<script>` 블록이라 참조 OK). ⚠️챔프별 프레이밍 미세조정 필요 시 `CHAMP_PREVIEW` 값만 바꾸면 강화창·스킨샵 양쪽 동시 반영. 🎨**스킨/강화 위치 조정 도구**(같은 세션 후속·버전 미상승=플레이어 무변화): `CHAMP_PREVIEW`에 **`xOff`(좌우)** 필드 신설+`_skinPreview3d`·`_forge3dShow`·인앱 `skinPrevTune()` 모두 좌우 적용(기본 0이라 기존 무변화). 독립 목업 **`스킨-위치조정-목업.html`**(`.gitignore` 화이트리스트)=로컬 `node _clawserve.mjs "C:\aram경로"` 후 `http://localhost:8731/스킨-위치조정-목업.html`(크기=zoom·좌우=xOff·상하=yOff 슬라이더+회전멈춤+붙여넣기용 `CHAMP_PREVIEW` 출력·`_skinPreview3d`와 동일 카메라 수식이라 값 1:1 전이). 사장님이 값 주면 `CHAMP_PREVIEW`에 박으면 됨. ✅**v439=값 반영**: 목업 튜닝 결과 적용(티모 0.70→0.50·에코 0.55→0.45·니코 full 1.00→0.75/skin 0.55→0.40·요네skin 0.75→0.85·**럭스 yOff −0.60→−1.20 추가 하강**(v440)·슬라이더 범위도 크기0.1~3/좌우±1.5/상하±2로 확대). 🧹**v441 아케이드 탭 정리**: 평평하게 나열되던 진입점을 **① 🕹️인형뽑기(+단짝) ② 🗡️인형 투기장(강화·배틀·스킨 묶음)** 2섹션 카드로 재구성(`_prismClawHtml`/`_arenaEntryHtml`·CSS `.arcade-sec/.arcade-grid/.arcade-sec-h/.ac-stat`·줄글 note 대폭 축소·"프리즘 18종 컴플리트" 헤더 제거·스킨샵 버튼을 투기장 섹션 안으로 이동·비자격자엔 `🔒 N/7` 잠금 카드). 투기장 헤더 `_arenaHd`(강화·배틀 창)의 **✕/? 좌우 스왑**(`.arena-x2` left→right=✕우측·`.arena-q` right→left=?좌측). ⏭️**다음/미해결(v437에서 이월)**: 방어 순서 설정이 **배틀 화면에만** 노출(강화창/프로필에도 노출 검토)·#2 방어자 본인 코인 절대값 update가 공격 트랜잭션 덮을 수 있음(down-leak)·"누가 내 캐릭터" 전투화면 구분(v401 미해결)·소규모 상대풀(자격자 5명). ★**직전 세션(v432~437·2026-06-29·원격web·작업브랜치 `claude/resume-work-in-progress-bdxxwh`)**: 🗡️**인형 투기장 정식 오픈**(`ARENA_LIVE_ENABLED=true`·v436 — 되돌리려면 false 한 줄) · ⚔️**배틀 수싸움화**(방어 순서 미리 설정·1전투=정찰 기억·같은 상대 15분 쿨다운·읽고 바꾸는 고양이-쥐 RPS·`arenaDefenseOrder/arenaScout/arenaAtkCd_s2`·`_arenaDefOrder/_arenaTargetCdLeft`·배틀화면 하단 `arena-defset` 슬롯·v437) · 🛡️**방어 알림**(공격당함→프로필 코치마크 1건 묶음+클릭 상세 `openArenaDefenseLog`·**append-only inbox `arenaDefendInbox_s2`**로 동시공격 유실 수정·`_arenaDefenseSummary/Consume`·v432·434) · 🏆**투기장 랭킹**(랭킹탭 S2 토글 `#rk-board-seg`·강화(최고전투력)›스킨›순승·`arenaWins/Losses_s2`·`renderArenaRanking`·v433) · 🎨**스킨 장착 시 강화 성공률 +1%p**(외형은 챔프 단위 공유·`ARENA_SKIN_FORGE_BONUS`/`_arenaForgeOddsEff`·v435) · 🔬**경제 재시뮬**(방어코인 포함·무한증식/데드락 없음·⚠️**엔드게임 코인 인플레**=+20+스킨 후 sink 고갈, 중장기 지속 sink 추가 권장: 반복 코스메틱/방어코인 일일캡). **배포=`main`+작업브랜치 둘 다**·매 변경 인라인 `<script>` 7블록 `node --check`. ⏭️**다음/미해결**: 방어 순서 설정이 **배틀 화면에만** 노출(강화창/프로필에도 노출 검토 제안함)·#2 방어자 본인 코인 절대값 update가 공격 트랜잭션 덮을 수 있음(down-leak)·"누가 내 캐릭터" 전투화면 구분(v401 미해결)·소규모 상대풀(자격자 5명). 상세=세션이력 맨 위. ★**직전 세션(v405~431·2026-06-29·로컬)**: 🎨**스킨 상점 완성**(챔피언당 1스킨·400코인·3D미리보기·아케이드진입·`CHAMP_SKINS`/`openSkinShop`/`_appGlb`풀모델하이브리드·상대는 상대본인스킨) · 🗡️**투기장 UI 전면 리디자인**(검정+골드 통일·헤더✕/제목/?·`_arenaHelp`도움말모달·하단탭`_arenaGoTab`·이름3종`별칭(챔피언)`·강화창 확률3칸+상단지갑칩+인형줄한줄·경합문구 적중성공/실패/막힘) · 🔨**강화창 3D**(`_forge3dShow`·등급색림라이트·강화펄스·자체크기독립) · 🧸**단짝 idle 공격모션버그 수정**+**티모 아바타 축소**(`_BUDDY_MODEL_ADJ.teemo.bust.zoom`) · 🔍**코인 역추적완비**(스킨/웰컴/콘솔 로그) · 🔌**브릿지 v1.1.36~37**(진행배너 명단 gameflow폴백+버전노출→구버전 진행자 업데이트안내·앱 `LATEST_BRIDGE_VER`). **`ARENA_LIVE_ENABLED=false` 유지**·테스트=`arenaForgePreview()`/`arenaBattlePreview()`. 상세=세션이력 맨 위. ★**직전 세션(v391~404·2026-06-28)**: 🔮**오늘의 운세**(빙고 출석·49종) · 🕹️**가챠 「아케이드」 탭**(인형뽑기·투기장 진입 이동) · 🪙**내 정보바 3통화**(골드/뽑기/투기장 코인) · 🗡️**투기장 밸런스**(배틀수익↑·고강 파괴완화)+**7종 수집=영구해금**(파괴로 수량<7이어도 입장유지)+**비주얼**(스테이지 배경·상대카드·HP박스 헥스텍톤·워딩 통일)+**랜덤 매치업 데모** · 🔥**메인 프로필 연승/연패 제거** · 🔨**소비 강화권 탭→대장간 이동 후 망치질 연출**. ⚠️**투기장 "누가 내 캐릭터" 구분=미해결(v401 되돌림)** · **브릿지 inGame.players 안 옴=aram-bridge 측 수정 필요**. `ARENA_LIVE_ENABLED=false` 잠금 유지. 상세=세션이력 맨 위. ★**직전 세션(v337~369·2026-06-26~27)**: 🗡️**인형 투기장 대장정** — 강화(코인경제·판매)·비동기 배틀·🔮상성/⚔️QWE스킬·🎬3D 포켓몬식 전투(❤️체력바 3라운드·내 캐릭터 뒷모습·네임플레이트·순차 콜아웃)·🔬경제 시뮬 검증(무한증식/데드락 없음)·데이터 복원/환급 전부 구축. **`ARENA_LIVE_ENABLED=false` 잠금 유지**(샌드박스 `?arenabattle`/`?arena3d`로만 확인). 상세=«🗡️ 인형 투기장» 전용 섹션 + 세션이력 맨 위. ★**직전 세션(v318~336·2026-06-26)**: 🗡️**인형 투기장(검키우기 접목) 설계 확정 + Phase1 강화시스템**(아래 전용 섹션 必) · 📻**메인 라디오**(연결바 새로고침↻→BGM 9곡 패널·`_radioToggle`/`_radioPanelToggle`·인형뽑기 열면 자동정지) · 🎮**진행중 게임배너** 참가팀원 전원표시+**참여자 본인엔 숨김**(`updateNormalGameBanner` meIn) · 🧸**단짝3D 모션 다양화**(`_buddyScheduleIdleBreak` 4.5~10초 랜덤 GLB클립·탭 랜덤) + 단짝패널 빛무리제거+공중부양수정(`.buddy3d-mount` 그림자·`_panelDrop`) + 프로필공간(`.bsp-3d`) 노란글로우 제거 · 🐾**정산 로딩 3D 포로 마스코트**(`_settlePoro3d`·poro.glb·콘솔 `_settleLoadPreview()`·위치 `_poroTune` yOff=-0.4) · 🔔**아바타 알림 스크롤 들썩 수정**(`#attend-coach` fixed→absolute·scroll핸들러 제거·문서좌표) · 🕹️인형뽑기 BUILD152~153=BGM 컨트롤 HUD줄로 이동+가게 BGM 9곡(bgm-steel/onemore/augment/sad 추가). 최근(v307~317·이 세션 후반·아래 세션이력 必): 🧸 **단짝 변경 즉시반영+영속성 근본수정(v316~317)**=`buddySelect`가 `await update` 후에야 UI갱신하던 구조 → **동기 로컬반영+`_buddyAfterPick` 즉시 + 저장은 `_buddyPersist` 백그라운드 4회 재시도**(쓰기 지연/실패에도 화면 즉시·새로고침 원복 해결) + 렌더루프 **자가복구**(`_buddyPickMount`로 보이는 마운트 재탐색·`_buddy3dShow` 엘리먼트 수용 — 공유 3D 캔버스가 재렌더로 사라진 mount에 묶여 멈추던 근본문제). · 🧸 **단짝 3D '하늘보기' 보정**=카메라 아닌 GLB 아이들 포즈 고개들림 → `_buddy3dLoad`서 `group.rotation.x=_buddyHeadTilt`(0.20rad≈11.5°·콘솔 `_buddyTuneTilt(도)`). · 📱 **모바일 정산창/프로필/내정보바 잘림 수정**=정산창 `@media(max-width:560px)` 글자·숫자칸 축소 / 프로필 닉네임=칭호·광채 배지를 `.pm-s1-meta`로 분리(이름 단독 한줄) / 내 정보바=grid로 [아바타|이름·LV2단|골드] 재배치(`max-width:42%` 캡 해제·골드 확대). · 🧾 **정산창 프리뷰**(`settlementPreview()`/URL `?settlement`·샘플 데이터). · 🔨 **강철심장 걸작 정산창 표시 누락 수정(v306)**=효과(LP/골드)는 원래 적용되고 있었으나(`s1ApplyMatchResult` winLP/lossLP·`giveMatchGold` matchG/winG·MVP mvpG) **정산창 `showMatchSummary`에만 빠져** 안 되는 것처럼 보였음 → 매치 스냅샷 `emblemEffects`(발동결과 박제) 읽어 LP/골드 숫자 합산 + "⚡ 이번 판 발동 효과"에 🔨 발동라인(실제 강철심장 아이템 아이콘 `_forgeItemImg`·시너지와 동일 방식). · 🧸 **단짝 변경 흐름 정리(고르면 선택창 즉시닫힘·confirm제거 + 고른뒤 애칭창·변경시도 + 애칭 저장 후 3D 사라짐 수정=`_buddyReflect` 공용헬퍼로 재렌더후 `_buddyHomeAttach` 재부착, v305)** · 🧸 **단짝 변경 즉시반영(3D 재부착) + 인형뽑기 2단계 '꽝' 제거(BUILD151·v304)** · 🕹️ **인형뽑기 역추적 로그(BUILD150·v303) + 코인/기회 헛소모 방지(BUILD149·사장님 설계) + 🧸 단짝 desc 정정(v301)**(아래 세션이력 必): **단짝 변경 즉시반영**=`_buddyAfterPick`이 재렌더 후 60ms에 `_buddyHomeAttach()` 재호출(캔버스가 옛 제거된 mount에 묶여 안 바뀌던 문제 — 새 챔피언이면 `_buddy3dShow` ctx.champ 불일치로 GLB 재로드). **2단계 '꽝' 제거**=7종 완성 후 이미 수집 챔피언 뽑아도 `_collComplete()`면 강화 목적이라 긍정 메시지(단짝 일치="단짝과 친해지는 중!"·앱 init `buddy` 전달). **역추적 로그** `clawLog_s2`(결과 스냅샷 배열·최근150건)=open/start(코인소모)/drop(집기)/result(champion·dup·poro·miss)/leave(진행중 이탈) 기록, 콘솔 `clawLog('닉')`/`clawLog()`로 조회(start↔drop↔result 짝 안 맞으면 헛소모 정황). ⚠️BUILD148(집기 시 차감)은 **되돌림**(공짜 조준 후 재입장 어뷰즈) → 차감=**기존 START 시점 복원** + ① **3D 로딩 게이트**(`_modelsReady` false면 START "게임 로딩 중" 비활성·로딩 중 START로 코인만 날리던 핵심 경로 차단) + ② **진행 중 이탈 확인창**(START 후 닫기/새로고침=`_postParent('gamestate')`→앱 `_clawInProgress`→confirm). BGM 이후 플레이어 11명 코인+3·기회+3 보상(러너 우회). 직전: 🧸 **단짝 보상 밸런스/추적/위치보정 + 🕹️ 인형뽑기 가게 BGM**(v288~300·BUILD144~147·아래 세션이력 必): 연승보상 상향(스택↑=확실히 커지게)·추적도구 `buddyStats()`+`buddyGold_s2` 별도필드·표시스택=연속-1(1승=없음·2승=1스택)·별칭저장 후 전체갱신 버그수정·**단짝3D 위치보정 모드별(full=프로필/bust=아바타) `_BUDDY_MODEL_ADJ`**(7종 코드 영구백업 + Firebase `config/buddyModelAdj` merge 오버라이드 · `buddyTunePreview()`로 캐릭터별 조정·저장 · ⚠️렌더루프 bob이 group.y 덮어쓰던 버그 수정) · 인형뽑기 mp3 **가게 BGM 5곡 라디오**(`bgm-*.mp3`·랜덤시작 순환)+음량/끄기(컬렉션바 안 우측·localStorage). 직전: 🧸 **단짝 연승/연패 보상 정식 오픈**(v285~287·★): `BUDDY_REWARDS_LOCKED=false`로 **수령·지급·안전망·효과표시 전부 라이브**(되돌리려면 true 한 줄·~L32056). 표시 스택 정리=**연속-1**(1승=스택없음·2승=1스택·내부 count는 연속 그대로·보상표 키는 연속 2~7/2~6 유지) + `_buddyAtCap`(승7/패6 상한 '최대치·최고 보상 도달' 표기). 직전: 🔗 **전투준비 칩·챔피언풀 모달 UI 대수술 + 🧸 단짝 하트유대(♥) 전환 + 강철심장 강화조작 되돌림**(v265~284·아래 세션이력 必). 직전: 🧸 **단짝 2차 대장정(아바타3D·LP하단레이아웃·변경가능·S2보상검증·보상잠금→오픈)**(v234~264·아래 세션이력 必): 단짝=좌상단 내 아바타에 움직이는 3D(`buddy-bar-mount`·뒷배경 제거·상반신줌), LP카드 하단 레이아웃 [최근10경기 한줄(flex·정사각 해제) / ⚒️전투준비 2열=강철심장칩(실아이템아이콘+성능 우상단배지+걸작효과 초소형 세로)|시너지], **단짝 언제든 변경**(`buddySelect` 재선택 허용·챔피언별 레벨/별칭 `buddyLevels_s2`에 보존·「🔄 다른 단짝으로 변경」), S2보상검증(**LP2배권=정상**·**방어권insurance=S2 무의미**→연패보상 교체), **보상/효과 설계 미정이라 `BUDDY_REWARDS_LOCKED=true`(L31891)로 수령·효과 표시만 잠금**(선정·강화·스택누적은 유지·확정 시 false로 flip하면 한번에 활성). 직전: 🔨 **강철심장 UX 대수술 + 정산/카운트다운/로비 손질**(v224~233): 강철심장 관리 시트(내아이템 슬롯 탭→장착/강화/판매·인라인💰제거)·소비탭 강화권 탭=관리시트로(실수강화 차단)·성능N 표기·강화성공 0~5 테두리색·그리드 넘침 `minmax(0,1fr)` / ⏱️ 카운트다운 시계스큐 수정·🛠️ 팀 결성 로비 배너(`/lobby`)·⚡ 정산창 지연단축(골드 Promise.all 병렬+"정산 계산 중" 로딩). **LP 병렬화(C)=검증완료(안전)·미구현**(아래 세션이력 v224~233 참조). 직전: 🧸 **단짝 메인 통합 대장정**(v210~223): 홈 대시보드에 단짝 3D 세로패널 + 레이아웃 `[단짝 190px | 매치정보(LP+내전/막고라) 1fr]` + 패스 전체폭 아래로(`wm-home-row`/`wm-home-right`/`wm-pass-full`), **별칭(nickname)** 병행표기·편집(`buddySetNickname`/`_buddyOpenName`/`_buddyNameHtml`, 한글명=`champNamesKR`), **빈상태=챔피언7종 컬렉션→컴팩트 배너**(`_buddyHomePromptHtml` 겹친카드 부채꼴, 미보유 시 자리축소·`_hasBuddy`분기), **단짝3D 홈↔패널 공유캔버스**(`_buddy3dShow(mountId)`·가시성/30fps게이트·offsetParent), **전체모션 GLB 교체**(Idle_Base 첫애니 유지→인형뽑기 무영향, 연승=Dance·탭=Joke/Laugh), 📱**모바일**(단짝 하단배치 `display:contents`+order·스크롤 검은가림 `translateZ(0)` GPU고정), 프리뷰링크 `?buddypreview`. · 🔒 **보안 감사**(미결정): 규칙 읽기공개+쓰기`auth!=null`인데 익명인증 무료라 **사실상 누구나 전체 read/write/delete**·계정 비번없음(이름매칭)·콘솔 파괴함수 노출(`deletePlayer`/`gachaTestGrant`/`clawCompleteAll` 등). 진짜 방어=구글로그인+허용목록, 1차=입장코드+콘솔가드 — 사용자 결정 대기. 직전: 🃏 가챠 시너지 재설계+밸런스, 🔨 강철심장/시너지 대기화면 아코디언, 📖 가챠 컬렉션 북, 🪟 프로필 모달 중앙 창화. ⏭️ **성능 최적화(헥스필드 등) 일부 적용**(복권창 한정·v209) — 메인 배경 상시 헥스 최적화는 미적용. ⚠️**복권 해골감소 인플레 미해결**. 🕹️ **3D 인형뽑기 정식 오픈**(v194) + 🧸 **단짝인형 구현**(v195~201), 🎁 **일정 보상**(v202~204), ⚔️ **팀원 배너 시너지/강철심장 변경**(v205~207). 브릿지 **v1.1.35**. 상세는 아래 세션 이력 v2.45.177~223)
...
> 📌 **지금 진행 중인 작업 = 🗺️ 룬테라 전쟁 프로토타입**(`룬테라원정-프로토타입.html`·index.html과 무관). **바로 아래 「다른 컴퓨터 이어작업 핸드오프」부터 읽을 것** — 검증 레시피·함정·현재 상태·다음 할 일이 다 거기에 있다.

### 🗺️ 룬테라 전쟁 — 🖥️ 다른 컴퓨터 이어작업 핸드오프 (2026-08-04 기준·파일 v0.12·이 PC 최종 커밋·Pages success) ← 최신·必독
> ⚠️ **index.html(라이브 앱)과 완전 무관.** 이 작업은 단일 파일 **`룬테라원정-프로토타입.html`** 하나만 건드린다. `APP_VERSION`·`CHANGELOG`는 손대지 않는다.
>
> #### 1) 5분 안에 이어받기
> - **작업 파일**: `룬테라원정-프로토타입.html` (단일 HTML·빌드 없음·저장은 `localStorage rune_proto` mock). 에셋=`assets/{rts,hex,town,castle,survival,alley}/*.glb`(Kenney/Quaternius CC0·전부 커밋됨) + **`vendor/three/`(자체 호스팅 1.5MB·CDN 안 씀)**.
> - **확인 주소**: **https://sohada2.github.io/aram/rune.html** ⚠️캐시 매우 강함 → `?v=11`처럼 숫자를 바꾸거나 시크릿창. 제목이 「프로토타입 v0.11」이면 최신.
> - **작업 브랜치**: 세션마다 다름(최근 = `claude/update-check-jz6251`). **배포 = `git push origin HEAD:main` + `HEAD:<작업브랜치>` 둘 다.** `.gitignore`가 `*`+화이트리스트라 **`git add -f`** 필요.
> - **배포 확인**: `mcp__github__actions_list`(workflow_runs·branch main·event `dynamic`). ⚠️결과가 토큰 초과로 파일 저장됨 → `python3 -c "import json; d=json.load(open('<파일>')); ..."`로 파싱. 새 push가 이전 in-progress 빌드를 cancelled로 만드는 건 정상. 클래식 빌드가 큐에서 몇 분 머무는 것도 정상(`Pages Rebuild` 워크플로우가 빌드 요청은 이미 보냄).
>
> #### 2) 검증 레시피 (매번 이 순서로)
> 1. **문법**: 인라인 `<script>` **3블록**(classic 1 + module 2) 추출 → `node --check`(module은 `.mjs`). importmap/json 블록 제외.
> 2. **런타임**: 로컬 서버 + Playwright 헤드리스. `node --check`로는 **TDZ/미선언 식별자를 못 잡는다** — 반드시 브라우저로 띄워 `window.rune`이 함수인지 확인할 것.
>    - 서버: `(setsid nohup python3 -m http.server 8907 --bind 127.0.0.1 &)` — ⚠️**세션 중 자주 죽는다.** `ERR_CONNECTION_REFUSED` 나면 그냥 재기동(이번 세션에도 3번 재기동함).
...
>    - ✅ **이 원격 환경에서 마을·월드맵 전체 3D가 헤드리스로 렌더된다**(예전 「전체 3D 불가」 메모는 win32 PC 한정이었음). 로드 후 **5~9초 대기**(GLB 다수)면 스샷이 나온다.
>    - 상태 접근: **`window.rune()`** (classic top-level `let S`는 evaluate에서 직접 못 봄). 카메라는 `window.Map3D.dbg()`.
>    - 테스트 헬퍼: `runeArmy(n)`·`runeGrantNoble(n)`·`runeReset()`·`runeSaveCheck()`·`onVillageInfo(id)`/`goVillage(id)`·`go('map'|'village'|'rally'|'reports')`·`openBldSheet(b)`·`openHelp(key)`.
> 3. **PC(1366)·모바일(390)** 양쪽 스샷 + `pageerror` 0 확인.
>
> #### 3) ⚠️ 이 파일에서 반복해서 밟은 함정 (다음 작업자 必독)
> - **TDZ**: `let S = load()`가 파일 상단에서 즉시 `genWorld()`를 부른다. 그 경로가 참조하는 값을 **아래쪽 모듈 최상위 `const`로 빼면 스크립트 전체가 ReferenceError로 죽는다.** 상수는 함수 안이나 `let S` 위로. (`node --check` 통과함)
> - **마을 3D는 `_vilBuilt` 가드 때문에 재방문으로 다시 안 그려진다** — 건물 레벨을 바꿔 확인하려면 `window.rune()`으로 `S.v.blds` 수정 → 3초 대기(자동 저장) → **페이지 리로드** → `go('village')`.
> - **`RES`/`BLD.ic`/`TROOP.ic`는 이모지가 아니라 아이콘 키** — 그대로 출력하면 화면에 `wood` 같은 글자가 샌다. 반드시 `I(key)`로 감싸고, **toast엔 아이콘을 넣지 말 것**(`textContent`라 마크업이 글자로 찍힘). 실제로 시장 교환 토스트가 「wood1,000 → clay900」으로 새고 있었다(v0.9에서 `RES_NM`으로 수정).
> - **모듈(Town3D/Map3D)은 `I()` 스코프 밖** — `build({icon:I, ...})`로 주입받아 `_icon`/`_mIcon` 사용.
> - **`instFit`는 홀더 그룹 안에서 모델 바닥 중심을 원점에 맞춘다** — GLB 원점이 한쪽에 치우친 모델이 많아 이 보정이 없으면 건물이 슬롯에서 밀려나 이름표와 어긋난다. 반환값을 쓰는 호출부는 없다(전부 fire-and-forget).
> - **헥스 키트 건물은 육각 잔디 받침+나무가 본체와 한 메시·한 프리미티브** — 노드로 못 지운다. `_trimGeo(root,minY,maxR)`(삼각형 무게중심 필터)로 잘라낸다. 대장간=`{y:0.21,r:0.34}`. 다른 모델에 쓰려면 그 모델 bbox를 보고 값을 다시 잡을 것.
> - **마을 링 반지름은 서로 물려 있다**(건물 폭 2.05·본진 3.05·안쪽 3.45·바깥 6.15·성벽 7.4). 하나만 바꾸면 겹치거나 성벽을 뚫는다.
> - **지도 마커는 `pointer-events:none` 유지** — `auto`로 되돌리면 손가락이 마커에서 시작할 때 팬/핀치가 통째로 씹힌다. 탭은 `bindTap()` 히트테스트가 처리.
> - **`proj()`에 `camMoved` 같은 게이트 넣지 말 것** — 초기 1회도 스킵돼 라벨이 전부 원점에 뭉친다.
> - **`resize()`는 크기 0이면 즉시 return** — 뷰가 `display:none`일 때 폴백값으로 캔버스가 굳는다.
> - **`wsig`(월드 시그니처)에 `size` 넣지 말 것** — 마을이 자랄 때마다 전체 재구성이 돌고 카메라가 튄다.
> - **도감 카드(`.lor`)는 자식이 절대배치** — 안에 뭘 끼우면 레이아웃이 깨진다. 추가 패널은 카드 **아래**에.
> - **`runeArmy(n)`은 정찰병 10·파성추 6 고정**(n과 무관) — 모르고 연속 명령을 돌리면 3번째부터 '병력 부족'으로 실패한다(코드 버그로 오인했었음).
> - **줄 끝 주석으로 문장을 삼키지 말 것** — 한 줄에 여러 statement가 있을 때 끝에 `// 주석`을 붙였다가 뒤 문장이 사라진 적이 두 번 있다(`node --check`로 못 잡음).
> - **네트워크**: jsdelivr 등 CDN 차단(HTTP 000). **`registry.npmjs.org`는 열려 있어** `npm pack`으로 라이브러리를 받아 `vendor/`에 넣는 식으로 우회한다(three.js가 그렇게 들어옴). github.io도 차단이라 **라이브 curl 확인 불가** → 사장님이 브라우저로 확인.
>
> #### 4) 현재 상태 (무엇이 돌아가는가 · v0.11)
> - **월드**: 79×98 헥스(7,742칸)·야만인 마을 1,800개·전장의 안개(공개된 칸만 렌더)·3D 구름층. `SAVE_VER 10`. 시작 배율 `CAM_START_ZOOM=1.6`.
> - **마을 성장**: 마을마다 고유 무작위 상한(2★30%·3★22%·4★20%·5★16%·6★12%). 2★까지는 수비대가 없어 영구 파밍터. 집 근처 12개는 상한 2★ 고정.
> - **⛔ 야만인은 먼저 공격하지 않는다**(원작과 동일). 습격 스폰(`spawnRaid`/`maybeRaid`)과 방어 전투(`resolveDefense`)는 **지우지 않고 보존** — 멀티를 붙이면 그대로 되살아난다.
> - **마을 3D**: 링별 균등 배치(겹침 없음·최소 중심 간 2.6+), 빈 부지는 터(흙+밧줄+말뚝)로 표시되고 **부지를 눌러도 건물 창이 열린다**. 대장간=헥스 제련소(받침 잘라냄).
> - **UI**: 이모지 폐지·인라인 SVG 38종 · 건물 팔레트 **3열**(자원6·군사6·중심3이라 줄이 딱 맞음) · 자원바는 좁은 화면에서 내용에 맞춰 폭을 나눠 인구 칩이 안 잘림 · 지도는 `zoom<1.10`이면 이름표가 점으로.
> - **도움말**(`openHelp(key)`): 제목바 「? 도움말」·7탭(전투·지도·집결지·정찰·정복·마을·시장/대장간). **규칙만** 적고 조합·전술 조언은 넣지 않는다(사장님 방침).
> - **튜토리얼**(v0.11): 초반 10단계 「다음 할 일」 배너(`TUT_STEPS`/`_tutRefresh`·`S.tut={i,off}`·매 `loop()` 판정) + **건물 첫 완성(0→1) 시 후반 콘텐츠 설명**(`BLD_TIP`/`_bldTip`·`S.tips[b]`로 1회·신전=정복, 공성 작업장=공성전). 팁의 「? 자세히」 → `openHelp(그 탭)`.
> - **정복**: 신전에서 **영웅**(구 귀족)을 일반 훈련으로 뽑아 계속 보내 충성도를 20~35씩 깎아 0이면 함락. ⚠️**카드로 영웅을 소환하던 v0.7은 사장님이 「안 어울린다」고 해서 v0.8에서 제거** — 되살리려면 커밋 `c95d2a8`.
>
> #### 5) ⏭️ 다음 할 일 / 미결
> - 🤖 **적 AI 부재(가장 큼)** — 아무도 공격하지 않아 성벽·숨은 곳간·지원군·방어 전투가 놀고 있다. **실팀원 멀티(Firebase)** 또는 재설계된 습격이 붙어야 살아난다. ⚠️사장님이 "가짜 라이벌은 의미 없다"고 한 적 있어 **설계 합의 먼저**. (사장님이 직전 라운드에서 이 항목만 빼고 나머지를 진행시킴)
> - 멀티 붙일 때: 시작 파밍터(`BARB_NEAR`) 배치가 지금은 내 집 주변에만 돈다 → **스폰 지점마다** 돌려야 공평하다.
> - 🎨 대장간 지붕이 파란색이라 RTS 목조 건물 사이에서 살짝 튄다(원하면 tint).
> - 🎁 튜토리얼 완주 보상 없음(안내만) — 사장님께 물어봐 둔 상태.
> - 📖 도움말 문구에 수치가 일부 하드코딩 — 밸런스 상수를 바꾸면 `HELP_SEC`도 같이 고칠 것.
> - Town3D 라벨도 `pointer-events:auto`라 건물 명패 위에서 드래그 시작 시 마을 씬 조작이 씹힌다(지도와 같은 패턴 이식하면 됨).
> - 지형 타일이 개별 드로우콜(~300+)이라 병합 여지 · 3D 실기(폰) 체감 확인 · **영웅 카드 도감(`HEROES`/`REGIONS`)은 아직 이모지**(진입 버튼이 없어 현재 도달 불가 화면이라 후순위).
>
> #### 6) ⚠️ 컨셉·명칭 규칙 (되돌린 전례 있음)
> - **진영·지역·롤 컨셉 명칭을 임의로 붙이지 말 것.** 데마시아 창병+녹서스 도끼병이 한 군대에 섞여 설정이 무너져 **두 번 되돌렸다**. 컨셉은 사장님과 설정 설계부터.
> - **부족전쟁 원본 그래픽 복제 금지**(InnoGames 저작권). 수치·메커니즘은 사실이라 재현 OK, 아트는 CC0 키트로만.
> - 현재 용어는 **기능명 그대로**(범위 A로 어색한 것 3건만 개명: 공성 작업장·신전·숨은 곳간). 「소환사」 잔재는 전부 제거됨(0).

### 🚪 룬테라 전쟁 v0.13~0.14 — 시작 게이트(랜덤 시작) + 골드 상점·프리미엄 계정(사이트 붙이기 준비) (2026-08-04·이 PC win32·main 직접 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님 방향: 「이 게임을 사이트에 붙일 때 = **바로 시작 + 「시작하시겠습니까」로 실제 진입해야 시작(멀티)**, 시작지점 랜덤 · 경제=**금광(방치형 골드 채굴·나중) + 사이트 골드 sink용 게임내 골드 상점 + 부족전쟁식 날짜단위 프리미엄 계정**」. 이번엔 그중 **시작 게이트 + 골드 상점/프리미엄** 프로토타입 구현(금광·실제 멀티는 다음).
> #### 🚪 v0.13 시작 게이트 + 무작위 시작(멀티 진입 모델)
> - `SAVE_VER 10→11`(구 세이브 폐기). `fresh()`에 `started:false`·`genWorld`의 집 위치를 **중앙 고정→무작위**(`6+rnd*(HCOLS-12)`, 가장자리 여유 6). ⚠️**월드(야만인)는 여전히 고정 시드**(`mkrng(778899)`)라 공유 가능·**집만 `Math.random`으로 랜덤** = 멀티에서 각자 다른 빈 칸 클레임하는 구조.
> - 부팅(`DOMContentLoaded`): `S.started`면 `showView()`, 아니면 **`showStartGate()`**(전체화면 오버레이 `#start-gate`·「시작하기」). **loop() 최상단 `if(!S.started) return`** 라 시작 전엔 tick·렌더 안 함(→게이트 화면은 3D를 안 올려 **헤드리스로도 가볍게 뜸**). `window.startGame()`=started=true+save+`view='village'`+showView. `onMQ`도 started일 때만 showView.
> - 검증: 헤드리스 dump-dom — 게이트 표시·started=false·집 랜덤(예 24,14≠중앙 39,49)·startGame 함수 존재 확인.
> #### 🏅 v0.14 골드 상점 + 프리미엄 계정(부족전쟁식)
> - **mock 골드** `S.gold`(시작 500)·`S.premiumUntil`(ts) — `fresh`/`bindActive` 기본값. `isPremium()`=`premiumUntil>now`. **골드는 추후 실제 사이트 골드와 연동 예정**(지금은 이 브라우저 mock). 테스트 `runeGold(n)`.
> - **상단바 「🏅 상점」 버튼**(도움말 옆)→`openGoldShop()` 모달: 보유 골드·프리미엄 상태(남은 일수)·**프리미엄 7일 120골드 / 30일 400골드** 구매(`buyPremium(days,cost)`=골드 차감+`premiumUntil=max(now,기존)+days*86400000`, 연장식)·혜택 목록·금광 예고. `.gs-*` CSS.
> - **프리미엄 혜택**: `window.build`에 **건설 대기열 캡 = 무료 2 / 프리미엄 5**(부족전쟁식 QoL). (추가 혜택은 나중에)
> - 검증: 헤드리스 — 모달 열림·7일 구매 시 골드 500→380·isPremium true·7일 확인.
> #### ⏭️ 사이트 붙이기 로드맵(사장님 합의 방향)
> - **붙는 위치 = 아케이드 섹션**(인형뽑기·투기장·광장 옆)·iframe+postMessage+Firebase(인형뽑기 패턴). **경제 분리**(자원은 게임 내, 골드만 sink로 연동). 
> - **다음**: ① **금광**(방치형 골드 채굴소) ② **실제 팀원 멀티**(Firebase 공유 월드·서로 공격/지원/정복 — `spawnRaid`/`resolveDefense` 보존됨·BARB_NEAR는 스폰마다 돌려야 공평) ③ mock 골드→실제 사이트 골드 브릿지 ④ 해금 게이팅 결정. ⚠️멀티/골드 연동은 사이트(index.html) 통합 단계라 프로토타입 단독으론 못 함.

### 🌅 룬테라 전쟁 v0.12 — 마을 낮/밤을 실제 시간(KST) 기반 낮·노을·밤 자동 전환 (2026-08-04·이 PC win32·main 직접 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님: 「밤/낮 토글이 있는데 실제 시간에 맞춰 낮·노을·밤으로」.
> - **Town3D**: 구 `setNight`(수동 토글) 제거 → `_kstHour()`(기기 TZ 무관 KST=`getTime()+(getTimezoneOffset()+540)*6e4`)·`_timePhase()`(밤 <5.5·≥19.5 / 노을 5.5~7·17.5~19.5 / 낮 7~17.5)·`applyDN()`(phase 바뀔 때만 배경·hemi·sun 색/강도 적용, `_dnPh` 캐시). 낮=CSS 하늘(bg null)·노을=따뜻한 주황(bg 0x8a5238·sun 0xff8038)·밤=남색(bg 0x141b2c·달빛).
> - **자동**: `initScene`서 `applyDN()` 1회 + 루프서 15초마다 `applyDN()`(시간 흐르면 자동 전환). ⚠️그림자 프러스텀 유지 위해 **sun.position은 안 건드림**(색/강도만).
> - **버튼**(`rgn-daynight`): `Town3D.cycleDN()` = 자동↔낮↔노을↔밤 강제 순환(테스트/취향)+토스트. 기본=자동. 익스포트 `setNight`→`cycleDN`.
> - `_dnMode/_dnPh/_dnT` 모듈 var 추가. 문법 3/3·`setNight` 잔여 0·경계 로직 node 검증. ⚠️이 PC(win32)는 마을 전체 3D 헤드리스 렌더 불가 → 실기 확인 필요. ⏭️ 원하면 지도(Map3D)도 같은 처리 / 시간대 연속 보간(현재 3단계 즉시전환).

### 🎓 룬테라 전쟁 v0.11 — 튜토리얼(초반 10단계 + 건물 첫 완성 시 후반 콘텐츠 설명) (2026-08-03·원격web·작업브랜치 `claude/update-check-jz6251`·main+브랜치 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님 방침: 「튜토리얼은 보통 초반에 하는데 **뒷부분도 알려줘야 하는 경우**가 생긴다 → 초반은 튜토링, **후반 콘텐츠(영웅 정복·공성전)는 그 건물을 지었을 때** 설명」.
> #### 🎓 초반 튜토리얼 = 「다음 할 일」 한 줄 배너(`TUT_STEPS`·`_tutRefresh`)
> - 자원바 아래 `#tut-bar`에 **번호 + 할 일 + 보러가기 + ✕(끄기)**. 조건을 채우면 **자동으로 다음 단계**(매 `loop()` 마다 판정).
> - 10단계: 자원 Lv3 → 창고 Lv3 → 농장 Lv3 → 집결지 → 마구간 → 정찰병 3 → 첫 정찰 → 병영+도끼병 10 → **첫 약탈 성공** → 성벽. 마지막엔 완료 배너(초록)로 바뀌고 ✕로 닫는다.
> - 상태 `S.tut={i,off}`. `ok(v)`는 **상태만 보고** 판정하므로(플래그 심기 없음) 어떤 경로로 달성해도 인정된다.
> #### 🏛️ 후반 콘텐츠 = 그 건물 **처음 완성(0→1)** 시 팝업(`BLD_TIP`·`_bldTip`)
> - 훅: `tickVillage`의 buildQ 완성부에서 `if(v.blds[q.b]===1) setTimeout(()=>_bldTip(q.b),400)`. `S.tips[b]`로 **건물당 1회만**.
> - 9종: 집결지·마구간·병영·대장간·시장·숨은 곳간·성벽 + **공성 작업장(파성추/투석기가 이겼을 때 하는 일)** · **신전(충성도 20~35씩 깎아 정복·호위 필수)**.
> - 팝업의 **「? 자세히」** → `openHelp(key)`로 도움말 해당 탭 직행(`openHelp`가 인자로 탭을 받도록 확장).
> #### 검증
> 문법 3/3 · `pageerror` 0 · 초기화 후 1단계 표시 → 조건 10개를 차례로 채워 **1→10→완료** 전부 자동 진행 확인 · 신전 첫 완성 시 「👑 마을을 빼앗는 법」 팝업 · `openHelp('conquer')`가 정복 탭으로 열림 · PC/모바일 회귀 없음.
> ⏭️ 미결: 적 AI/멀티 · 대장간 지붕 파란색 · 튜토리얼 보상(현재 안내만·보상 없음)은 원하면 추가.

### 📱 룬테라 전쟁 v0.10 — 모바일 자원바 잘림·건물 그리드 줄맞춤·마을 건물 간격 (2026-08-03·원격web·작업브랜치 `claude/update-check-jz6251`·main+브랜치 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님 실기기 스샷 제보 3건.
> - **인구 칩 잘림**: 자원바 4칸이 `flex:1` 균등분배라, 값이 6자리(62,211)·인구가 `2,898/4,177`처럼 길어지면 **인구 칩만 잘렸다**. → ≤430px에서 `flex:1 1 auto`(내용에 맞춰 나눠 갖기)+폰트 축소, ≤400px 추가 축소. 실측 390/360px에서 숫자 우측 < 칩 우측 = 안 잘림.
> - **건물 그리드 빈칸**: `.bgrid2`가 모바일 **5열**이라 6개짜리 카테고리가 5+1로 갈라져 빈칸 4개가 남았다. → **3열**(`BLD_CATS`가 자원6·군사6·중심3이라 딱 떨어짐·남는 칸 0). 타일이 넓어져 글자도 커짐.
> - **마을 건물 오밀조밀**: 건물 폭 `2.5→2.05` · 본진 `3.4→3.05` · 안쪽 링 `3.55→3.45`(바깥 6.15·성벽 7.4 유지) → 안쪽 호 간격 여유 1.22→1.56, 링 사이 2.6→2.70, 바깥 끝 7.18<7.4. 데코(나무·바위) 밀도도 `[30,46,64,82]→[24,36,50,64]`.
> ⏭️ 미결: 적 AI/멀티 · 대장간 지붕 파란색 · 도움말 문구는 상수 변경 시 동기화.

### 📖 룬테라 전쟁 v0.9 — 도움말 모달 + 대장간 모델 교체 + 시장/대장간 피드백 + 지도 축소 이름표 (2026-08-03·원격web·작업브랜치 `claude/update-check-jz6251`·main+브랜치 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님: 「(적 AI 빼고) 나머지 바로 진행」 → 미결 4건 처리.
> #### 📖 도움말 모달(`openHelp`)
> - 제목바 우측 **「? 도움말」** 버튼. 탭 7개 = **전투 · 지도 · 집결지 · 정찰 · 정복 · 마을 · 시장·대장간**(사장님이 정한 순서).
> - **규칙만 적었다** — 「어떤 조합이 좋다」 같은 전술 조언은 넣지 않음(사장님 방침: 조합은 유저가 알아내는 영역).
> - 수치는 전부 코드에서 그대로 가져옴(운 ±25%·성벽 ×1.037·손실 ^1.5·연구 +12%/Lv·수수료 10%·상인 적재 `MERCH_CAP`·연구 상한 `RES_SMITHY_REQ`). 상수를 바꾸면 **`HELP_SEC` 문구도 같이 고칠 것**(일부는 상수 참조, 일부는 하드코딩).
> #### ⚒️ 대장간 = 헥스 키트 제련소로 교체(팩 한계 해소)
> - RTS 팩엔 대장간다운 모델이 없어 그동안 **광장형 monument(main_N)** 를 쓰고 있었다(사장님 「어색」 지적).
> - 헥스 키트 `building-smelter`(용광로+굴뚝)가 유일하게 맞는데 **육각 잔디 받침+나무가 본체와 한 메시·한 프리미티브**(노드로 못 지움). 클리핑 평면도 시도했으나 **받침 윗면이 그대로 보여** 실패.
> - → **`_trimGeo(root,minY,maxR)` 신설** — 삼각형 무게중심이 `y>=minY`·중심에서 거리 `<=maxR`인 것만 남겨 지오메트리를 재구성. `instFit`에 `o.trim` 옵션. 대장간은 **`{y:0.21, r:0.34}`**(모델 로컬 단위·bbox y 0~0.6·반경 0.577에서 잡은 값) → 받침·나무 제거, 용광로+지붕만 잔디 위에.
> - ⚠️ 다른 헥스 건물에 쓸 땐 그 모델의 bbox를 보고 값을 다시 잡아야 한다(모델마다 받침 높이가 다름).
> #### 💬 시장·대장간 액션 피드백
> - 교환·수송·연구 시작에 **`_spendFx`+`_spendFloat`**(무엇이 얼마나 나갔는지 뜨는 팝업) 추가, 연구는 **대장간 3D 링 펄스**까지.
> - ⚠️ **버그 같이 수정**: 교환 토스트가 `RES[give]`(=**아이콘 키**)를 그대로 찍어 「wood1,000 → clay900 교환」처럼 글자가 새고 있었다 → `RES_NM`으로 교체. (CLAUDE.md가 경고하던 «RES는 이모지가 아니라 키» 함정에 실제로 걸려 있었음)
> #### 🔎 지도 축소 시 이름표 → 점
> - `CAM.zoom < LABEL_DOT_ZOOM`(1.10)이면 라벨 호스트에 `.far` 클래스 → CSS가 이름표를 **13px 점**으로(내 성은 더 크게 금색). 탭 히트테스트는 화면좌표 기반이라 그대로 동작.
> - 실측: zoom 0.59에서 라벨 폭 51→13px·겹침 10쌍→2쌍, 확대하면 원래 이름표 복귀.
> #### 검증
> 문법 3/3 · PC/모바일 `pageerror` 0 · 도움말 7탭 전부 렌더(문구에 `undefined`/키 누출 없음·모바일 2줄 랩) · 마을/지도 회귀 없음 · 영웅 정복 흐름(충성도 100→0) 유지.
> ⏭️ **미결**: 적 AI/멀티(사장님이 이번 범위에서 제외) · 대장간 지붕이 파란색이라 RTS 목조 건물 사이에서 살짝 튐(원하면 tint) · 도움말 문구는 상수 변경 시 동기화 필요.

### 🏛️ 룬테라 전쟁 v0.8 — 카드 영웅 되돌림, 「신전·영웅」 이름만 유지 (2026-08-03·원격web·작업브랜치 `claude/update-check-jz6251`·main+브랜치 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님: 「카드로 영웅 하는 게 생각보다 안 어울린다 · **3성 카드를 영웅으로 쓰는 걸 없애고** · 귀족→영웅 이름은 그대로 · 기존 부족전쟁처럼 **영웅을 계속 보내 충성도를 깎아 정복**하는 방식 동일」.
> - **되돌린 방법**: 카드 시스템을 손으로 걷어내지 않고 **`git checkout 89b24d2 -- 룬테라원정-프로토타입.html`(v0.7 직전)로 통째 복원 후 이름만 다시 입힘** — 찌꺼기 0(`HERO_SKILL`/`summonHero`/`heroBuff`/`runeGrantCard`/`S.cards`/`v.heroes`/동행 선택 줄/도감 기술 패널/전투·행군 훅 전부 없음).
> - **남긴 것 = 이름뿐**: `BLD.academy.nm='신전'` · `TROOP.noble.nm='영웅'` · desc「영웅 훈련(마을 정복)」. **동작은 원래 정복 유닛 그대로** — 신전에서 일반 훈련(＋1/＋5)으로 뽑고, 보내서 이긴 만큼 충성도 −20~−35씩 깎아 0이 되면 함락.
> - ⚠️ **되살릴 일이 생기면** v0.7 커밋 `c95d2a8`에 카드 소환·기술 18종·동행 선택·도감 기술 패널이 통째로 들어 있다(설계 메모는 아래 v0.7 항목).
> - **검증**: 문법 3/3 · `pageerror` 0 · 신전 시트에 「영웅 ＋1/＋5」 훈련 카드 복귀 · 영웅 1명씩 4회 보내 충성도 **100→69→45→18→0 → 함락**(내 마을 2개) 확인.
> ⏭️ **미결**: 도움말 모달 · 대장간 모델 어색 · 시장/대장간 액션 피드백 · 지도 최대 축소 시 이름표 겹침 · 적 AI/멀티.

### 🎴 (되돌림) 룬테라 전쟁 v0.7 — 신전·영웅(카드 소환) 시스템 — ⛔ **v0.8에서 제거됨**(사장님: 안 어울림). 복구가 필요하면 커밋 `c95d2a8` 참조
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. 사장님 지시: 「귀족 만드는 곳을 **신전**으로 · 귀족을 **영웅**으로 · **3성 이상 카드**를 가진 챔피언만 · 자원은 귀족과 동일 · 챔피언 특성에 맞는 기술(예: 나피리는 사냥개라 정찰 갈 때 빨라짐) · **도감으로 기술 틀을 보여줄 것**」.
> #### ✅ 사장님 결정
> - **영웅은 정복에 쓰면 사라진다**(귀족과 동일). **동시 보유 제한 없음**(비용이 이미 엄청나서 자연 제한).
> #### 🎴 카드 → 영웅
> - **도감 영웅 18종 = 가챠 챔피언 18종과 정확히 같다**(룰루·베인·나피리…). 그래서 카드 연동이 그대로 물린다.
> - `HERO_MIN_STAR=3` · 카드는 **소모되지 않는다**(= 부를 자격). 소환 비용/시간/인구는 기존 정복 유닛 그대로(4만/5만/5만·120초·인구100).
> - **카드 보유는 목업**(`S.cards={slug:star}`) — 프로토타입은 독립 파일이라 앱 가챠(Firebase)를 못 읽는다. **`bindActive`의 `S.cards` 한 곳만 갈아끼우면** 실제 가챠와 붙는다. 테스트=🧪패널 「🎴 3성 카드 전부」 또는 `runeGrantCard('all',3)` / `runeGrantCard('Naafiri',3)`.
> #### 🏛️ 신전(academy) · 소환
> - 건물명 **영주관→신전**, 병종 **귀족→영웅**(파일 내 「귀족」 0). 일반 훈련 경로는 막고(`recruit`서 noble 차단) **신전 시트의 카드 목록에서만** 소환.
> - 영웅은 개수가 아니라 **'누구'** 라서 `v.heroes=['Naafiri',…]`로 따로 들고, `v.troops.noble`은 그 길이로 맞춘다 → **전투·정복 계산(기존 `units.noble`)은 손대지 않고 그대로 동작**. 훈련 큐 항목에 `hero` 슬러그를 실어 완성 시 `v.heroes`에 push.
> #### ⚔️ 기술 18종(`HERO_SKILL`) — 챔피언 특성에서 뽑음
> - 종류(k)와 적용 지점: `speed`(sendCommand 행군시간·mode 조건) · `atk`/`defcut`(twBattle `heroAtk`/`heroDef`) · `wallIgnore`(전투에 넣는 성벽값 감소) · `wall`(승리 시 성벽 추가 파괴) · `loot`(약탈 carry) · `cata`(발전도 추가 파괴) · `guard`/`revive`(이긴 싸움/진 싸움 손실 감소) · `hold`(마을에 두면 수비 전투력↑·`resolveDefense`).
> - 예: 나피리=**정찰 행군 −20%**(mode:'scout') · 람머스=모든 행군 −12% · 갱플랭크=약탈 +25% · 뽀삐=성벽 2단계 추가 파괴 · 베인=성벽 방어 절반만 받음 · 문도=아군 손실 −18% · 아무무/멜=마을 수비 +20%/+15%.
> - ⚠️ **새 기술을 넣을 땐 반드시 위 k 목록 안에서** — 새 종류는 적용 지점(행군·전투·약탈)을 같이 손봐야 한다.
> #### ⚠️ 이번에 밟은 함정
> - **영웅을 데려가면 행군이 오히려 느려졌다**: 영웅 speed가 35분/칸(정복 유닛)이라 `slow=max(...)`가 35로 잡혀 나피리의 −20%가 자기 발에 걸렸다(정찰 10.2초→31.8초). → **영웅은 속도 계산에서 제외**하고, 영웅만 단독으로 보낼 때만 영웅 속도를 쓴다.
> - **도감 카드(`.lor`)는 자식이 절대배치**라 기술 패널을 카드 '안'에 끼우면 레이아웃이 깨진다 → 카드 **아래**에 붙였다.
> - 테스트 헬퍼 `runeArmy(n)`은 **정찰병 10·파성추 6 고정**(n과 무관) — 이걸 모르고 연속 명령을 돌리면 3번째부터 '병력 부족'으로 실패한다(코드 버그로 오인했음).
> #### 검증
> 문법 3/3 · `pageerror` 0 · 소환 큐(`noble:Naafiri`)→완성→`v.heroes` 등록 확인 · 행군: 정찰 10,206→나피리 8,165(−20%)·람머스 8,981(−12%), 공격은 나피리 무변화(정찰 전용)·람머스 −12%, 영웅 단독 39,691 · 전투(운 고정·같은 수비대): 진 공격력 9,040→10,158 · 갱플 약탈 1,608→2,013 · 뽀삐 성벽 4→2 · 문도 손실 39→32 · 모르가나 수비 3,046→2,619 · 브랜드 파괴 1→2 · 베인 수비 2,716+성벽 1.
> ⏭️ **미결**: 실제 가챠 카드 연동(현재 목업) · 영웅별 기술 확장(지금은 1개씩) · 영웅 초상화는 ddragon이라 헤드리스에선 안 보임(실기 확인 필요) · 도움말 모달.

### 🗺️ 룬테라 전쟁 v0.6 — 🏘️ 마을 3D 실기 검증(원격 헤드리스 성공) + 건물 겹침 근본 수정 + 빈 부지 표시 (2026-08-03·원격web·작업브랜치 `claude/update-check-jz6251`·main+브랜치 배포)
> ⚠️ **index.html 무관** — `룬테라원정-프로토타입.html` 단독. v0.5가 "헤드리스로 마을 전체 3D를 못 띄워 실기 미검증"으로 남긴 항목을 **이 원격 환경에서 실제로 렌더해 검증**하고, 거기서 드러난 배치 버그를 고침. 파일 표기 v0.4(정체) → **v0.6**.
> #### ✅ 이 환경에서 마을/지도 전체 3D 헤드리스 렌더 **된다**(v0.5 메모 정정)
> - Playwright chromium(`/opt/pw-browsers/chromium-1194/chrome-linux/chrome`) + `--use-gl=swiftshader --enable-unsafe-swiftshader` + `python3 -m http.server`. 로드 후 **5~9초 대기**(GLB 다수)면 마을·월드맵 다 찍힌다. PC 1366 / 모바일 390 모두 `pageerror` 0·요청 실패 0.
> - ⚠️ **마을은 `_vilBuilt` 가드 때문에 재방문으로 안 다시 그려진다** — 건물 레벨을 바꿔 확인하려면 `window.rune()`으로 `S.v.blds` 수정 → 3초 대기(자동 저장) → **페이지 리로드** → `go('village')`.
> #### 🏘️ 건물 겹침 = 링 각도 계산 버그였음(근본 수정)
> - 예전: `half=RING.length/2`(=6.5)로 각도를 쪼개고 인덱스 홀짝으로 안/바깥을 나눔 → **안쪽이 7칸인데 간격은 6.5칸분** → 첫 건물과 마지막 건물의 중심 간 거리가 **1.87**(건물 폭 2.5라 확실히 겹침). v0.5의 폭·반지름 미세조정은 증상 완화였고 원인은 이 각도식.
> - 지금: 링별로 개수를 세어 각자 2π 균등 분할(`RING.filter(s=>s.in)`), 상수 `RING_RI=3.55·RING_RO=6.15·RING_OFF=0.07`(바깥 링을 살짝 비틀어 반지름 정렬 회피). **최소 중심 간 거리 1.87 → 2.6+**. 반지름은 (건물 폭 2.5·본진 폭 3.4·성벽 7.4)에 물려 있으니 하나만 바꾸면 겹치거나 성벽을 뚫는다 — 주석으로 박아둠.
> - **안쪽 링 = 초반에 짓는 것(자원 4종·창고·집결지)**, 바깥 = 군사/행정(병영·마구간·공방·대장간·대학·시장·지하창고). 처음엔 군사 건물을 안쪽에 뒀다가 렌더해보니 **초반 마을 중앙이 텅 비고 가장자리에만 건물이 서서** 뒤집음. 지금은 마을이 중심부터 차오르고 발전하면 바깥까지 넓어진다.
> #### 📍 GLB 원점 치우침 보정(`instFit`)
> - 다수 GLB가 원점이 모델 한쪽에 치우쳐 있어(성벽 조각과 같은 문제) 슬롯 좌표에 그대로 놓으면 **건물이 밀려나 이름표와 어긋나고 잔디 밖으로 삐져나갔다**. → 홀더 그룹을 슬롯에 놓고 모델은 그 안에서 **바닥 중심**이 원점에 오도록 보정(`-c2.x / -b2.min.y / -c2.z`), 회전은 홀더가 담당해 제자리 회전. `instFit` 반환값을 쓰는 호출부는 없어 안전(전부 fire-and-forget).
> #### 🏗️ 빈 부지 표시(`_plotMark`)
> - 안 지은 자리는 이름표만 허공에 떠 있어 어디를 눌러야 할지 몰랐음 → 바닥에 **터 닦은 흙 원 + 밧줄 + 말뚝 4개**. `userData.bldKey`를 심어 **부지를 직접 클릭해도 건물 창이 열린다**(레이캐스트가 부모까지 훑음). 빈 부지 이름표는 y 2.1→1.25로 낮춰 지면에 붙임.
> #### 검증
> 문법 3블록 `node --check` 3/3 · PC/모바일 `pageerror` 0 · 초반 마을(개척지)·성곽 도시(907점·성벽 8) 둘 다 렌더해 **겹침 없음·전 건물 성벽 안**·빈 부지 클릭→`barracks` 시트 열림 확인.
>
> #### 🔤 용어 정리(같은 세션 후속) — 사장님 «범위 선호 없음» + «소환사 잔재는 지운다»
> - **「소환사」 10곳 제거**(라이벌 AI 삭제 때 남은 잔재·현재 0): 이름 접두어를 떼던 `.replace('소환사 ','')` **7곳**은 이제 그 접두어를 붙이는 코드가 없어(마을 owner는 `me`/`barb`뿐) **죽은 문자열 처리**라 삭제 · `'적 소환사'`→`'알 수 없는 적'` · 보호 안내 「이 소환사는」→「이 마을은」 · 사기 주석. (「신규 보호 중」 배너는 이미 중립 표기였음)
> - **범위 A(기능과 어긋난 것만) 최소 개명 3건**: 공방→**공성 작업장**(파성추·투석기 제작소인데 뭘 만드는지 안 보였음) · 대학→**영주관**(귀족을 배출하는 곳이라 학교 이름이 안 맞음) · 지하창고→**숨은 곳간**. 이름은 **`BLD.nm` 한 곳**이라 팔레트·건물창·3D 명패·큐가 전부 자동 반영(코드 주석 표기만 같이 손봄). **진영·지역·롤 컨셉 명칭은 일절 안 붙임**(두 번 되돌린 전례).
> - ⚠️ 병종 이름은 **안 건드렸다** — 바꾸면 `TROOP_ROLE`(11줄)이 이름을 전제로 쓴 설명문이라 같이 고쳐야 함.
> ⏭️ **미결**: 도움말 모달(용어가 정리됐으니 이제 착수 가능). 대장간이 광장형 monument라 여전히 어색(팩 한계) · 시장 거래/대장간 연구 등 다른 액션 피드백 · 지도 최대 축소 시 이름표 겹침 · 적 AI/멀티.

### 🗺️ 룬테라 전쟁 v0.5 — 🏛️ RTS 에셋 건물 대개편 + PC UI 게임화 + 클릭/피드백 (2026-08-03·이 PC win32·main 직접 배포)
...
>   - ⚠️ 점검 중 삽질 2건(다음 작업자 시간 절약): ①`world` 엔트리 필드는 `type`이 아니라 **`owner:'barb'`** ②`.map-wrap canvas`로 잡으면 **숨은 2D 폴백 `.grid-canvas`(0×0)** 가 먼저 걸려 boundingBox가 null → **`.map-wrap > canvas`** 로 잡을 것.
> - **🗺️ 지도 시작 배율 `zoom 8 → 1.6`**(사장님 지시): 켜자마자 **내 성 하나가 화면을 꽉 채워** 주변 마을·길·안개가 안 보였다(처음 들어오면 뭘 할지 알 수 없음). 시작(2173행)·`resetCam`(2514행) 두 곳에 흩어져 있던 `8`을 **모듈3 상단 `CAM_START_ZOOM` 상수 하나**로 묶음. 휠로 직접 내려보며 잡은 값이 1.6(내 성 + 마을 40여 개 + 길 + 가장자리 구름이 한 화면).
>   - 검증: 문법 3/3 · PC·모바일 초기 진입과 **리셋 버튼까지 전부 zoom 1.6** · **이름표 겹침 0쌍**(화면 안 마커 PC 43/모바일 21) · `pageerror` 0.
>   - ⚠️ **최대 축소(zoom 0.84 이하)에서는 이름표가 여전히 겹친다(19쌍)** — 시작 배율만 바꾼 거라 그 구간은 그대로. 줌별 실측 = 3.24→0쌍 / 1.65→0쌍 / 0.84→19쌍. 거슬리면 축소 시 이름표를 점으로 바꾸는 처리 필요(미적용).
> - ⏭️ **이어받을 것**: **용어 변경 범위(A/B/C) 답 듣기 → 반영** → 도움말 모달. 나머지 미결(적 AI 부재·Town3D 라벨 pointer-events·헥스 바닥색·드로우콜 병합·영웅 도감 이모지)은 그대로.
...
> - **🌫️ 페넘브라(반음영) 지형**: 공개 헥스 주변 **맨해튼 4칸**까지 타일을 실제로 배치하되 구름으로 덮음 → 구름 밑에 땅이 있다는 게 보이면서도 마을 위치는 안 새어나감. 공개되면 페넘브라 타일 제거하고 정규 타일로 승격.
> - **🖐️ 지도 팬 반응성**: 매 프레임 도는 `proj()`가 `getBoundingClientRect()`(레이아웃 강제)와 `new Vector3()`를 라벨 수만큼 반복 → 벡터 재사용 + rect 400ms 캐시 + **값이 바뀔 때만 style 기록**. ⚠️**`camMoved` 게이트를 넣었다가 마을 라벨 11개가 전부 사라진 적 있음**(초기 1회도 스킵돼 원점에 뭉침) → 게이트는 넣지 말 것.
> - **🏰 마을 3D 명패**: 검은 박스/테두리 제거 → **3D 위에 새겨진 글자**(외곽 텍스트 그림자). 표기 순서 **「아이콘 이름 레벨」** 통일, 빈 부지는 레벨 자리만 `＋`.
> - **⏳ 작업 게이지 = 이름표 아래로 통합**: 떠다니던 `.work-gauge` 박스(아이콘·Lv·바·시간)를 **삭제**하고, 이름표 안에 `.wgm/.wgr`(폭 30px 얇은 바 + `0:37`, 훈련은 `×3 0:06`)를 붙임 → 이름이 안 가려져서 **게이지 뜰 때 명패 숨기던 처리도 제거**. 3D 바닥 펄스 링은 유지. **성벽은 RING 슬롯이 아니라 이름표가 없어 게이지가 안 떴음** → `mkNamePlate()`를 `add()`에서 분리해 정문 앞(6.9,0)에 **성벽 이름표만 따로** 세움(모델은 `buildWallRing` 담당). 마을 3D 아래 큐는 **바 없는 한 줄**(`.q-line`)로 축소 — 3D로 못 보여주는 **완료 시각·대기열**만 남김(연구/수송은 `.q-row` 유지).
> - **🧱 하단 건물 그리드**: 세로 스택 → **한 줄(아이콘·이름·레벨)**, 높이 132→**83px**. 건설 가능 표시를 좌상단 초록 점(`::after`)에서 **버튼 자체 초록 인셋 글로우**로(점이 정렬을 흔들었음). 레벨 배지 `1+1` → **`1▸2`**.
> - **✏️ 이름·번역투**: 점토 채취장→**점토굴** · **은신처→지하창고**(실제 역할=약탈당해도 자원 일부 보존) · 기계식 `을(를)/이(가)`를 받침 판별 조사 헬퍼 **`jo(단어,'을')`** 로 교체.
> - **🎨 이모지 전면 폐지 → 인라인 SVG 아이콘 세트(`ICO`/`I(key,cls)`)**: 사장님 지적 두 가지 —①톱니바퀴·드럼통·졸업모자 같은 **현대 물건이 중세 배경과 안 맞음** ②이모지는 **기기(아이폰/안드로이드/윈도우)마다 글리프가 달라** 같은 화면이 다르게 보임. → **24×24 실루엣 패스 38종**(건물 15·병종 12·자원/모드/UI 11)을 직접 그려 `fill:currentColor` 인라인 SVG로 교체. 글자 색을 그대로 따라가고 어디서나 동일 렌더. `BLD.ic`/`TROOP.ic`/`RES`가 **이모지 → 아이콘 키**로 바뀜(그대로 출력하면 "wood" 같은 키가 새니 반드시 `I()`로 감쌀 것). ⚠️**모듈(Town3D/Map3D)은 `I()` 스코프 밖** → `build({icon:I,...})`로 주입받아 `_icon`/`_mIcon`으로 사용. `toast()`는 `textContent`라 SVG 불가 → 토스트에서는 아이콘을 빼고 문구만. 야만인 마을=`hut`·라이벌=`tower` 전용 아이콘(지하창고 항아리 재활용하던 것 정정). 남은 문자는 `★`(발전도)·`✕`(닫기) 같은 **텍스트 기호**뿐 — 이건 이모지가 아니라 플랫폼 무관. **2차 다듬기**: 목재=통나무 단면 3개(나이테)·철=주괴 3단 적재·병영=진짜 교차 검(`I()`가 `<g transform=rotate>` 원시 마크업도 받도록 확장)·대장간=모루(윗판+뿔/허리/받침/굽)·대학=두루마리(창고 나무통과 헷갈려서 분리)·마구간=편자+못구멍·창=세운 창(대각선이라 횃불처럼 보이던 것)·활=활대+시위+깃·투석기=평형추+투척주머니·도끼=날 허리. **제목바 아래 성벽 총안(`repeating-linear-gradient`) 제거**(격자 무늬로 보임)·`.btn.no::after` ⚠ 제거.
> - **🧹 인라인 팁 전부 제거**(설명은 모달로 갈 예정) · 상단바 질감 벽돌→석재.
> - **📦 자원바 = 정통 전략(부족전쟁) 규칙 반영**: 이 게임은 **저장고가 차면 생산이 그냥 버려지는** 구조인데 수량만 보여주고 용량은 title(툴팁)에만 있었음 → 칩을 2행 그리드로 재구성해 **[아이콘] 수량 / 용량 +분당생산 + 저장 게이지**를 항상 노출. 85%=경고(호박)·100%=가득(적색+테두리 펄스+"생산이 버려지고 있어요" 툴팁). 인구 칩도 동일 포맷(가득 시 훈련·건설 막힘 안내). 좁은 화면은 **생산량 → 용량 순으로 접고 수량·게이지는 끝까지 유지**(수량이 먼저 잘리던 것 수정: `em`을 `flex:0 0 auto`로). ⚠️**부족전쟁 원본 그래픽은 복제 금지**(InnoGames 저작권) — 자원 종류·표시 규칙만 참고하고 아이콘은 전부 직접 그림.
> - **⚔️ 병종 평가(`TROOP_ROLE`) + 방어 3종 라벨**: 원작(부족전쟁)은 유닛 정보에 「기병에 강하다」 정도의 **한 줄 역할 서술**과 수치표·모의전투를 주고, **병과 비율대로 전투가 쪼개진다**는 계산 구조나 조합 전략은 안 알려준다(위키 지식). 사장님 방침도 같음 — **조합은 유저가 알아내는 영역**이라 **수치에서 바로 나오는 사실만** 한 줄로 적는다(예: 창병='대기병 45로 인구 1당 최고치, 공격 10은 사실상 없는 값'). `TROOP_ROLE`(11종)+`troopStatHtml()`로 훈련 카드·건물 시트·PC 표에 노출. 설명은 **「무엇에 특화되고 무엇에 약한가」** 한 줄(예 창병='기병을 막는 데 특화. 보병 상대나 공격에는 힘을 못 쓴다.') — 강점 뒤에 반드시 약점을 붙여 트레이드오프가 보이게. ⚠️위 스탯에 이미 나온 숫자를 문장에서 다시 읊지 말 것(사족). **방어 3종은 줄이지 말 것** — `보/기/궁`이 아니라 **「공격 / 보병 방어 / 기병 방어 / 궁병 방어」 2×2 격자**(`.ts-row`)로 그대로 표기(그동안 `10/5/10`이라 뜻을 알 수 없었음). 속도·약탈량도 비용줄에 표기.
> - **⛔ 야만인 습격 비활성화**: 타이머 기반이라 인과가 없어 인위적이었음 → `maybeRaid` 호출 제거(스폰·방어전투 코드는 멀티용 보존). 마을 병력 섹션의 습격 경고줄(`raid-st`)을 **`defenseStatusHtml`(`def-st`)** 로 교체 — 습격 예고가 아니라 **「이 마을이 얼마나 버티는가」**를 **보병/기병/궁병 3종으로 나눠** 표시(성벽 배율·기본방어·연구·주둔 지원군 합산). ⚠️원작은 합산 전투력을 아예 안 보여주지만(개수만+모의전투), 방어 3종을 이미 카드에 노출한 이상 자연스러운 연장이라 남김.
> - **🏘️ 마을 밀도 상향(700→1800)+집 근처 보장 배치**: 초기 공개 범위(반경 7) 안에 마을이 6~11개, 반경 4엔 2개뿐이라 초반에 털 곳이 없었음. `BARB_TOTAL=1800`(밀도 9%→23%) + **`BARB_NEAR=12`**(집에서 2.5~7칸 고리에 무방비 1~2★를 먼저 깔아 시작 지점 파밍 대상 보장). 결과: 반경4 2→**12**(무방비 7) · 반경6 6→**27**(19) · 반경8 11→**49**(34). ⚠️지도 마커는 **공개된 헥스에만** 생성되므로(`!rev[key] continue`) 개수를 늘려도 DOM 부담 없음. **`SAVE_VER 10`**(구 세이브 폐기 — 안 그러면 옛 월드가 그대로 남음).
> - **🖐️ 지도 제스처가 가끔 씹히던 원인 = 마커가 포인터를 가로챔**: `.v3-label`이 `pointer-events:auto`라 손가락이 **마을 마커 위에서 시작하면 그 포인터가 캔버스(OrbitControls)에 아예 도달하지 않음** → 팬 무반응, 핀치는 한 손가락만 인식돼 확대/축소 실패. 마을 밀도를 올린 뒤(화면에 마커 40개) 체감이 커짐. → 지도 라벨 호스트에 `map-lab` 클래스 부여 후 **`.v3-labels.map-lab .v3-label{pointer-events:none}`**, 탭은 `bindTap(el)`이 **화면좌표 히트테스트**(이동<10px·500ms 이내·`_sx/_sy`는 `proj()`가 매 프레임 저장·라벨 rect ±6px)로 직접 판정하고 `VLAB[].onTap` 호출. ⚠️**Town3D(마을 씬) 라벨은 여전히 `pointer-events:auto`라 같은 문제가 남아 있음**(건물 명패 위에서 드래그 시작 시 팬 안 먹음) — 필요하면 같은 패턴 이식.
> - **🎯 확대/이동 중 지도가 기울거나 튀던 원인 = 마을 성장이 전체 재구성을 유발**: `wsig`(월드 시그니처)에 `size`가 들어 있어 **야만인 마을이 자랄 때마다(4★→5★) 전체 재구성**이 돌고, 그 끝의 `CT.target.set(0,0,0)`이 카메라 타깃만 원점으로 되돌림. 카메라 **위치는 팬한 자리 그대로**라 시선 방향이 꺾여 「기울기가 바뀌고 어딘가로 튀는」 것처럼 보였음. 마을을 1800개로 늘린 뒤 성장 이벤트가 잦아져 체감이 커짐(계측: 3.7초 드래그 중 재구성 2회). → ①`wsig`에서 `size` 제외(성장은 라벨 글자만 바뀜) ②성장·보호상태 변화는 `_mkMarkHtml()`로 **보이는 마커 innerHTML만 제자리 갱신** ③재구성이 일어나도 **카메라를 건드리지 않음**(`_camPlaced`로 최초 1회만 집 위치 정렬·리셋 버튼은 그대로). 검증: 팬(2.49,-1.14) → 강제 성장 → (2.54,-1.17) 유지·jump 0·rebuild 0·라벨 6★ 갱신 확인.
> - **🌱 야만인 마을 성장 상한(`barbMaxSize`)**: 성장이 `size<6`까지 무제한이라 **평균 2시간마다 +1★ → 약 10시간이면 월드 전체가 6★로 수렴**, 초반에 깔아둔 무방비 마을이 전부 사라져 파밍터가 없어짐(사장님 지적). → 마을마다 성장 상한 `mx` 부여. ⚠️**「내 집에서의 거리」로 정하면 안 됨**(처음에 그렇게 만들었다가 정정) — 나한테 먼 지역은 **거기 스폰한 팀원에겐 집 앞**이라 그 사람만 시작부터 5~6★ 밭에 갇힘. **마을 고유의 무작위 속성**으로: `barbMaxSize()` 가중치 2★30%·3★22%·4★20%·5★16%·6★12%(2★까지는 수비대 없음=영구 파밍터). 시작 크기도 `1..mx` 무작위라 세계가 자라며 성숙. `BARB_NEAR` 시작 파밍터만 **`mx:2` 고정**(스폰 보조 — ⚠️멀티 붙이면 스폰 지점마다 돌려야 함). ⚠️**TDZ 함정 재발**: 가중치 표를 모듈 최상위 `const`로 뺐다가 `let S=load()`(상단)가 genWorld를 즉시 부르며 ReferenceError로 스크립트 전체 사망 → **함수 안에 둘 것**(`node --check`로 못 잡음·`window.rune` 부재로 발견). 구 세이브는 **폐기 없이 `_ensureBarbCaps()`가 거리 기준으로 backfill**(이미 자란 마을은 줄이지 않고 그 자리에서 정지). 검증(전부 상한까지 성장 + 임의 스폰 12곳 반경6): 무방비 마을 최소 4·최대 12·평균 8.1 — 어디에 스폰해도 만만한 밭이 확보됨. 상한 분포 2★565·3★403·4★346·5★278·6★208.
> - **🖼️ 보고서→지도 복귀 시 지도가 위쪽만 그려지던 버그**: `resize()`가 `EL.clientWidth||420` 폴백을 쓰는데, **보고서 화면에 있는 동안 `#view-map`이 `display:none`이라 0** → window resize(모바일 주소창 여닫힘)가 오면 캔버스가 **420×400으로 굳음**. 돌아와도 `mount`가 early-return이라 재조정 안 됨. ⚠️**이건 직전 커밋의 회귀** — 예전엔 잦은 전체 재구성이 `resize()`를 같이 불러 증상이 가려져 있었는데, 재구성을 없애면서 드러남. → ①`resize()`는 **크기가 0이면 아무것도 안 함**(엉뚱한 값으로 굳지 않게)+같은 크기면 스킵+`_rc` 캐시 무효화 ②`ResizeObserver`로 컨테이너 크기 변화를 직접 감시(뷰 전환·주소창 대응). 마을 씬 `resize()`의 같은 폴백도 동일 처리. 검증: 보고서 왕복+뷰포트 높이 변경 656→572→712에서 캔버스가 매번 컨테이너와 정확히 일치.
> ⏭️ **미결**: 도움말 모달 미착수(우선순위=전투 규칙›야만인 습격›지도›집결지›정찰›정복›마을›시장/대장간) · 헥스 타일 바닥색이 지면색과 살짝 안 맞음 · 습격 빈도/강도 실플레이 튜닝 · 지형 타일이 개별 드로우콜(~300+)이라 병합하면 추가 성능 여지 · **영웅 카드 도감(`HEROES`/`REGIONS`/`ROLE_TRAIT`)은 아직 이모지**(도감 진입 버튼을 이전에 제거해 현재 도달 불가 화면이라 후순위).

### 🗺️ 룬테라 전쟁 — 🚨 3D 지도가 안 뜨던 근본 원인 수정 + three.js 자체 호스팅 (2026-07-30·원격web·작업브랜치 `claude/continue-update-bnfsry`·main+브랜치 배포)
...
> ⛔ **이 습격 시스템은 이후 비활성화됨(2026-07-30 같은 날)** — 사장님 지적 「이유 없이 계속 쳐들어온다·원작은 안 그렇다」. 실제로 순수 타이머였고(발전도로 간격만 단축) 규모가 내 병력에 비례해 **발전할수록 벌 받는** 구조였음. 원작 야만인 마을은 **먼저 공격하지 않고**, 파밍 난이도는 그 마을의 **수비대·성벽**(`barbGarrison` 3★+ 창병×6·검사×2·궁병×2 / `barbWall` 4★+ / 90분 재생 / 6★까지 성장)이 만든다. → `tick()`에서 `maybeRaid` 호출만 제거(스폰 코드·`resolveDefense`는 실팀원 멀티용으로 보존). 아래 내용은 그때 설계 기록.
...
> - **동작**: 발전도(건물 레벨 합) **16** 넘으면 시작(개척지 무공격) · 간격 55분→22분(발전도 따라 단축) · **최소 4분 예고**(지원 회수·대비 가능·기존 행군 목록/토스트로 표시) · **도끼병+창병만**(기병·공성 없음 → 성벽 파괴·정복 불가, 병력/자원만 손실) · 14% 대규모(×1.6) · **오프라인 밀린 습격은 최대 1회만 즉시 정산**(복귀 몰살 방지·`RAID_OFFLINE_MAX`).
> - **난이도 = 습격 규모 `max(발전도 하한, 내 병력 방어력 × 0.12~0.34)`**. ⚠️**전투 공식상 패자 전멸·승자는 (약한쪽/강한쪽)^1.5 손실**이라 비율 0.3만 돼도 방어 성공 시 16%를 잃음 → 비율을 낮게 유지해야 "성가시지만 회복 가능"이 됨.
> - ⚠️**설계 함정 3개(시뮬로 발견·전부 수정함)**: ①습격 규모에 **성벽을 포함하면** 성벽 올릴수록 습격도 세져 투자가 상쇄 → `villageTroopPow`(병력만) 사용 ②**발전도에 성벽·은신처를 넣으면** 방어 건물이 자기 발목 → `villageDev`에서 제외 ③**하한이 고정이면** 후반에 '병력 0'이 오히려 안전(약탈량이 carry 제한이라 미미) → `raidFloor(dev)=50+(dev-16)×5`로 발전도 비례 = 최소 상비군 요구선.
> - **시뮬 검증**(scratchpad `raid_sim.mjs`·실제 `twBattle` 복제·시나리오 8종 × 20,000회): 정상 플레이(병력+성벽) **방어 100% 성공·손실 9~15%**(대규모 16~27%) / 병력 방치 시 뚫려 약탈(dev60 병력0 → 239 자원·**은신처가 대응 수단**) / 최소 상비군 dev30 창병~20 → dev60 창병~30으로 상승 / 성벽 투자는 순수 손실 감소(창병20 기준 15.2%→11.2%).
> - **UI**: 마을 병력 섹션 상단 `raid-st` 3단계(**약탈 위험 / 막지만 손실 큼 / 대비됨**) + `🛡️ 방어 N / 권장 M · 창병 +K` + 다음 습격 카운트다운. **권장선 = 습격이 '하한'에서 '병력 비례'로 넘어가는 교차점**(`raidFloor/raidRatio`) — 이 위부터 손실률이 ~10%로 안정되는 지점이라 이걸 기준으로 잡음(처음엔 "대규모를 겨우 버티는 선"으로 잡았다가 그 선에서도 거의 전멸이라 재정의). 보고서에 `🔥 야만인 습격`/`대규모` 표기(`r.raid`/`r.big`).
> **검증**: 3블록 `node --check` 3/3 + Playwright(모바일 390) 초반 무공격→스폰→4분 예고→도착→약탈/방어성공→보고서, 상태 3단계 렌더, **pageerror 0**.
> ⏭️ **미결**: 습격 빈도·강도는 상수만 고치면 조절됨(`RAID_INT_*`/`RAID_RATIO_*`/`RAID_FLOOR_*`/`RAID_BIG_*`) — 실플레이 체감 후 튜닝 · 지원(다른 마을 원군)으로 막는 플레이가 실제로 유효한지 실기 확인 · 실팀원 멀티(Firebase)는 여전히 미착수.

### 🗺️ 룬테라 전쟁 v0.4 — 🏘️ 마을 콘텐츠 채우기(시장 상인 수송·대장간 연구 시간/게이트·다음 레벨 미리보기) (2026-07-30·원격web·작업브랜치 `claude/continue-update-bnfsry`·main+브랜치 배포)
...
> - **로컬 확인(3D 필수)**: `python3 -m http.server 8899` 후 `http://127.0.0.1:8899/<URL인코딩된 한글파일명>` — **`file://`은 CORS로 GLB 못 부름**. 한글 파일명은 `urllib.parse.quote`로 인코딩.
> - **검증 레시피**: ①인라인 `<script>` 3블록(classic 1 + module 2·importmap 제외) 추출해 `node --check`(module은 `.mjs`) ②헤드리스 브라우저(이 환경=Playwright chromium `/opt/pw-browsers/chromium-1194`, 사장님 PC=Edge `--headless=new`)로 **PC 1366px·모바일 390px 둘 다** 스크린샷+`page.evaluate` 흐름 실행. ⚠️**`let S`(상태)는 evaluate에서 직접 접근 불가**(classic script top-level) → **`window.rune()`** 으로 접근. `window.*` 함수(go/build/recruit/openBldSheet/runeArmy…)는 직접 호출 가능.
> - **테스트 헬퍼**: `runeArmy(n)`(병력+집결지 지급·⚠️인구 상한 걸리면 훈련 실패는 정상 동작) · `runeGrantNoble(n)` · `runeReset()` · `runeSaveCheck()` · `rune()`(상태) · `onVillageInfo(id)`/`goVillage(id)`(마을 패널). 즉시 도착 테스트=`rune().marches.push({fromV:'home',to:<id>,units:{...},dep:Date.now()-20,arriveAt:Date.now()-5,mode:'attack'})`.
> - **최종 상태(커밋 `aefaaf7`)**: 새 세이브→건설/훈련 큐→정찰→공격→보고서→귀족 정복→집결지/보고서 화면까지 **PC·모바일 양쪽 전 흐름 통과·pageerror 0·저장 정상** 확인함.
> - ⚠️**다음 작업자 주의 3가지**: ①**컨셉/명칭(진영·지역·롤 컨셉)을 임의로 붙이지 말 것** — 두 번 되돌렸음(아래 정정 항목 필독), 붙이려면 사장님과 설정 설계부터 ②**부족전쟁 원본 그래픽 복제 금지**(InnoGames 저작권 — 수치·메커니즘은 사실이라 재현 OK, 아트는 CC0 키트로만) ③마을 발전 단계 공식이 **classic `vilStage`와 Town3D `build`의 tier 계산 두 곳에 중복** — 하나 고치면 반드시 같이 고칠 것.
> ### 🧭 코드맵(커밋 `aefaaf7` 기준·줄번호는 근사 앵커)
> **구조 = `<script>` 3블록**: ①**classic L375~1452**(게임 전체: 데이터·상태·전투·UI) ②**module Town3D L1453~1629**(마을 3D) ③**module Map3D L1630~1784**(지도 3D). ⚠️module의 top-level 변수는 window에 안 오름 → 콘솔/테스트는 `window.rune()`.
> - **데이터**(classic): `BLD` L415 · `TROOP` L437(cls/atk/def·defC·defA/speed/carry) · `UNIT_GROUPS` L450 · `RES`/`RES_NM` L407 · `SAVE_VER`(=8) L472 · `HCOLS/HROWS` L412 · `vilStage`+`STAGE_NM` L550(발전단계 45/25/10) · `HEROES`/`REGIONS` L383/396(도감 전용).
> - **전투/명령**: `wallBasicDef` L559 · **`twBattle` L560**(부족전쟁 공식 본체) · `applyLoss` L579 · `moraleVs` L584 · `pushReport` L586 · **`sendCommand` L588**(공격·지원·정찰 공용 진입점) · `conquerVillage` L601 · `processMarches` L612(도착시각 순) · `resolveAttack` L628 · `resolveScout` L658 · `resolveDefense` L682 · `recallSupport` L708.
> - **UI**: `buildNav` L736 · `showView` L745 · `renderTop` L764 · `renderSidebar` L777(PC) · `renderMap` L804 · `openVillageInfo` L926 · `openAttack` L981 · `renderVillage` L1033 · `updateVillageDynamic` L1087(PC=테이블/모바일=아이콘 그리드 분기) · `renderRecruit` L1147 · **`openBldSheet`/`renderBldSheet` L1182/1189**(모바일 바텀시트) · `renderRally` L1264 · `updateRallyLive` L1286 · `reportCard` L1313 · `renderReports` L1353.
> - **Town3D**: `buildWallRing` L1485(성벽 레벨별 외형) · `buildTier` L1509(단계별 광장/소품) · `instFit` L1542(GLB bbox 높이 정규화) · `placeBuilding` L1550(본진 집→저택→성) · `decorate` L1573 · `build` L1597(**tier 공식 L1602 — classic `vilStage`와 동일해야 함**) · export L1628.
> - **Map3D**: `MCOL` L1676(모드별 경로선 색) · `updMarchLines` L1678(아치 경로선) · `biome` L1640(지형 노이즈) · `computeRoads` L1761(마을 간 길 타일) · `mount/build/resetCam` L1714/1720/1781 · `dbg()` 카메라 디버그.
> - **window 노출 18종**: go·build·recruit·openBldSheet/closeBldSheet/sheetBuild/sheetRecruit·switchVil·openMarket·openSmithy·rallyFind·recallSupport·showCard·openDex·onVillageInfo/goVillage + 디버그(rune·runeReset·runeArmy·runeGrantNoble·runeSaveCheck).
> - **의도적 보존(현재 도달 불가)**: `rivalGarrison`·`allocSpawn` = 라이벌 AI 제거로 미사용이나 **실팀원 멀티 붙일 때 재사용**할 코드(파일에도 주석 명시). `buildIso`/`FX`/`fillBiomeFx`는 **살아있는 폴백**(three.js CDN 실패 시 2D 정착지 씬) — 지우지 말 것.
> - **⚔️ 부족전쟁 전투 엔진 신설(`twBattle`)**: 유닛에 병과별 방어 3종(`def`/`defC`/`defA`=대보병/대기병/대궁병·실제 부족전쟁 수치)+공격 병과 `cls`(inf/cav/arch) → **공격 병과 비율대로 부분 전투 가중 합산** · 🍀 운 ±25%(공격자) · 사기(`moraleVs`=수비점수/공격점수×3+0.3, 0.3~1.0·대 라이벌만) · 🧱 성벽=기본방어 `20×1.25^(lv-1)`+배율 `1.037^lv` · **손실=승자 (열세/우세)^1.5·패자 전멸**(부족전쟁 공식) · 🐏 파성추=전투 중 성벽 약화+승리 시 파괴 확정/패배 시 절반 · 💥 투석기=승리 시 마을 발전도(size★) 파괴 · 방어 패배 시 수비 전멸+약탈(은신처 보호). 공격/방어/정찰 전부 이 엔진 경유(`resolveAttack`/`resolveDefense`/`resolveScout`).
> - **🧭 정찰 시스템**: 적 마을 수비/자원/충성도 **기본 은폐(❓ 정찰 필요)** — 정찰병만 보내 수비 정찰병×2를 뚫으면 spy 스냅샷(`S.spy[id]`={res,troops,wall,loyalty,def,at}) 획득, 마을 정보 패널·공격 모달에 "최근 정찰(N분 전)" 표시. 실패=정찰대 전멸. 부족전쟁 핵심 정보전 재현.
> - **🚩 집결지(rally) 건물 신설**: 모든 명령(공격/지원/정찰)에 집결지 Lv1 필요(부족전쟁 동일·건물 테이블에 하이라이트 유도). **집결지 화면**=좌표로 마을 찾기(`(x|y)` 입력)+부대 총괄(마을별)+진행 중 명령(습격 인입 최상단 빨강)+주둔 지원 회수.
> - **🛡️ 지원(원군)**: 내 마을 간 병력 지원(`mode:'support'`→도착 시 `v.support[]` 스택)·방어 전투에 합류·집결지/마을 화면에서 회수(`recallSupport`). 원 마을 인구(농장) 소모 유지(부족전쟁식).
> - **📜 보고서 시스템**: 공격/방어/정찰 전 건 리포트(`pushReport`·최근 30)—운%/사기/공수 전투력/유닛별 파병·손실 칩/전리품/성벽 변화/충성도/정찰 결과. 전멸 시 수비 정보 은폐(부족전쟁 동일). 보고서 탭+미확인 배지.
> - **👑 정복 개선**: 함락된 마을은 크기 기반 개발 상태(본진/자원/농장/창고/성벽 레벨)로 인수(부족전쟁: 건물 유지). 야만인 마을 성장(+수비대 90분 재생)·소형(1~2★)=무방비 파밍 대상(원본과 동일)·대형(3★+)=수비대. `processMarches` 도착 시각 순 처리(정렬) — 같은 틱 다중 도착 순서 보장.
> - **🎨 비주얼 재구성 = 부족전쟁 클래식 양피지 테마**: 다크+골드 → 양피지(#efe2bd)+목재 헤더+브라운 테이블. 자원바=저장고 게이지(80% 노랑/97% 빨강)+인구까지 4칩.
> - **🖥️📱 PC/모바일 완전 분리(핵심)**: **PC(≥900px)=부족전쟁 데스크탑** — 상단 가로 메뉴(지도/마을/집결지/보고서+시장/대장간)+**우측 고정 사이드바**(마을 개요·자원 게이지·건설/훈련 큐·부대 이동 인입 빨강)+건물/유닛 **테이블 UI**(비용·시간·병과별 방어 컬럼). **모바일(<900px)=부족전쟁 모바일앱** — 하단 탭바 4개(배지)+카드 UI+터치 스텝퍼. `isPC()` matchMedia로 렌더 분기+전환 시 재렌더.
> - 유지: 3D 헥스 지도(Map3D·행군 마커에 지원🛡️/정찰🧭 아이콘 추가)·3D 마을(Town3D 그대로)·2D 폴백(격자 25×31로 수정)·뉴비 보호 15일/선공 해제 경고·시장/대장간/도감·디버그(`runeArmy(n)`=병력+집결지 신설·`rune()`·`runeReset` 등).
> - **🎭 룬테라 컨셉 리스킨(같은 세션 후속·사장님 지시 "게임성은 그대로, 우리 컨셉을 껍데기처럼")**: 수치·로직 무변, 명칭/로어/표기만 교체. ①자원: 점토→**🔹 마정석**(RES_NM 신설·resbar title에 이름) ②건물: 본진→**💠 넥서스**·공방→**헥스텍 공방**·대장간→**오른의 대장간**(사이트 S2 연결)·대학→**총독부**·집결지→**지휘소**(탭/문구 전체)·목재소→벌목장 ③병종: **공성 미니언**(ic 🐏→👾)·**헥스텍 투석기**·귀족→**총독** ④월드: 야만인 마을→**버려진 마을**(정보 패널에 전쟁 로어 한 줄)·뉴비 보호→**신규 소환사 보호** ⑤디버그 노출 `window.onVillageInfo`/`goVillage`. 검증=Playwright(명칭·로어·정찰 게이트·모바일 오버플로 전부 통과·에러 0).
> - **⚠️⚠️ v0.3 컨셉 전면 원복 + 라이벌 제거(사장님 2차 피드백 — 필독)**: "라이벌 시스템 자체가 의미 없다 · 공성 미니언/헥스텍 투석기/오른의 대장간 같은 것도 빼라, 컨셉을 잘 못 잡는다" → **컨셉 리네이밍 전부 원복**(본진/공방/대장간/대학/시장/집결지/목재소/점토/파성추/투석기/귀족·야만인 마을·뉴비 보호 문구). **라이벌 AI 시스템 통삭제**(genWorld player 스폰·maybeRaid 습격·보호 시드·runeRaid — 방어/지원/성벽 로직은 향후 실팀원 멀티용으로 유지). `SAVE_VER 8`. **교훈: 컨셉/명칭 붙이는 건 전부 사장님과 설계 먼저, 임의로 붙이지 말 것.** 같은 커밋에 UX 3종: ①**건설/훈련 큐 부족전쟁식**(진행바 `.qbar`+남은시간+완료시각 `fmtClock`+대기 전체시간·훈련은 유닛별 집계 `🔱×4 🪓×2`·큐 head에 `dur` 기록) ②모바일 건물카드 버튼에 ⏱소요시간·훈련카드 ⏱초 ③**Map3D 행군 경로선**(`updMarchLines`·모드별 색 점선 아치 QuadraticBezier·마커가 아치 따라 이동·공격/귀환/지원/정찰 지도 전체에 표시=부족전쟁식) ④**Town3D 마을 퀄 개선**: 조립식(벽+지붕·다 비슷) → **건물별 전용 GLB**(hex 키트 building-castle/archery/sheep/watermill/smelter/wizard-tower/market/cabin/mine/farm/house·`instFit` bbox 높이 정규화·레벨업 시 커짐·집결지 슬롯 추가) — ⚠️**부족전쟁 원본 그래픽 복제는 저작권상 불가**(수치는 사실이라 OK, 아트는 CC0 키트로 재현) — 사장님께 안내함. ⚠️새 마을 3D·경로선은 실기 미검증(환경 CDN 차단).
> - **⚠️🎭 리스킨 정정(사장님 피드백 — 다음 작업자 필독)**: 처음에 유닛에 진영명(데마시아 창병·녹서스 도끼병·아이오니아 궁수·프렐요드 궁기병·요들 정찰대), 시장에 빌지워터, 대륙 K##에 룬테라 지역명(KMAP/regionOf/crdR·지도 라벨), 라이벌에 팀원 닉네임까지 붙였다가 **전부 되돌림**. 이유: **"데마시아와 녹서스는 적대 관계 — 한 군대에 섞어 붙이면 설정 붕괴. 진영/지역 설정은 나중에 별도로 제대로 설계해서 넣을 영역"**(사장님). → 유닛=순수 직업명, 시장=시장, 좌표=K##만. ⚠️**이때 잠시 남겼던 진영 중립 컨셉(넥서스·헥스텍 공방·오른의 대장간·총독·마정석·공성 미니언·버려진 마을 등)도 곧이어 v0.3에서 전부 원복됨 — 현재 코드에 컨셉 리네이밍은 하나도 없음**(전부 부족전쟁 기능명). 도감(`HEROES`/`REGIONS`)의 챔피언·출신 지역만 롤 자산으로 잔존(공식 로어). **앞으로 진영/지역/컨셉 명칭은 반드시 사장님과 설정 설계부터.**
> - **📱 v0.3.1 모바일 마을 UX(부족전쟁 모바일식) + 성 밖 대지(사장님 피드백)**: ①모바일 마을 화면의 스크롤 왕복(건물/훈련 섹션이 길게 나열) 해소 — 부족전쟁 모바일 앱 패턴(건물 탭→전용 화면) 적용: 모바일=**건물 아이콘 그리드(4열 `.bt2`·Lv배지·🟢=건설가능·🔒잠김) + 탭하면 바텀시트(`openBldSheet`/`renderBldSheet`/`.bsheet`)**에서 업글(비용·⏱시간)+병력 훈련(+1/+5)+시장/대장간/집결지 진입. 3D 마을 건물 명패 탭도 모바일=시트/PC=즉시 업글. 훈련 섹션 4박스는 모바일 마크업에서 제거(시트로), PC는 기존 테이블 유지(`_vilPC`로 레이아웃 전환 시 재빌드). 씬 높이 모바일 310px(씬+큐+그리드 한 화면) ②**성 밖 공중에 뜬 문제**: Town3D에 90×90 대지 평면(`earth`)+성 밖 수풀/바위 스캐터 22개+씬 배경 어두운 남색→낮 하늘(BIOME.demacia.sky)+비네트 완화. ⚠️3D 실기 확인 필요.
> - **🏘️ v0.3.2 마을 발전 단계(사장님 피드백 "초반 약한 마을→점점 발전하는 모습 필요")**: 발전도(건물 레벨 합)로 4단계 — 🏕️개척지(<10: 방벽X·흙마당·천막)→🛖마을(10+: 흙길·벤치)→🏘️소도시(25+: 돌광장·분수·좌판)→🏰성곽 도시(45+: 가로등·풍성). **성벽 외형=성벽 건물 레벨 연동**(0=없음·1~4=나무 울타리 `fence`·5~9=돌벽+망루·10+=높은 성벽+**해자**) — 부족전쟁처럼 성벽을 지어야 벽이 생김. **본진=Lv1~4 집(`building-house`)→5~9 저택(`building-village`)→10+ 성(`building-castle`)**, 깃발도 5/10부터. 데코 스캐터 단계별 밀도(10/22/34/48). 코드=Town3D `buildTier`(광장/소품 `tgroup`)·`buildWallRing` 재작성·central 분기, classic `vilStage`/`STAGE_NM`(씬 타이틀에 단계명·⚠️Town3D build 내 공식과 동일 유지). ⚠️3D 실기 미검증(울타리 fence 접선 방향 ry=-a+P2 근사 — 어긋나면 튜닝).
> - ⏭️ **미구현/이월(다음 작업 후보)**: 🌫️ **전장의 안개**(사장님 승인·미착수 — 먼 지역 가리고 정찰/점령으로 증분 공개 → 성능+게임성 동시 해결) · **적 AI 부재**(라이벌 제거 후 현재 공격받는 일 없음 — 방어/지원/성벽/`resolveDefense`/`mode:'raid'` 로직은 전부 살아 있으니 **실팀원 멀티**(Firebase) 또는 재설계된 AI를 붙이면 즉시 동작) · 부족(길드)·시장 상인 이동·프리미엄(부족전쟁 잔여 격차) · **3D 실기 확인**(마을 발전 단계·울타리·해자·성 밖 대지·지도 행군 경로선 — 개발환경 CDN 차단으로 미검증) · 투석기 목표 건물 선택(현재 발전도 파괴로 단순화) · 가챠 챔피언=특수 지휘관 합류(도감에 예고만).

### 🗺️ (구) 룬테라 전쟁 프로토타입 — 지도/마을 대개편 (2026-07-29·이 PC win32·작업브랜치 `claude/continue-previous-work-b6dd2h`·main+브랜치 배포)
...
- **🎓 교장 유미 3D 반영**: 3D가 `yuumi-idle/dig/rest.glb`(일반)만 로드하고 `yuumi-principal.glb`(교장·7MB·v626서 커밋됐으나 미사용)는 안 씀 → 변신 후에도 일반 유미가 뒤짐. **수정**: `ensureYuumi3d`가 `_yuumiIsUnique()` 읽어 교장이면 **principal GLB 1회 로드 → SkeletonUtils(`three/addons/utils/SkeletonUtils.js`) 상태별 클론**(지오메트리·텍스처 공유=저비용). 클립: 대기=`Idle01`/뒤지기=`Dance_Loop`/휴식=`Recall`(principal 전체 클립 보유). 일반은 기존대로 파일 3개. `_buildState(model,clip,st)`/`_fit(_r)` 헬퍼로 공통화.
- **교장↔일반 전환 지원**: 변신은 세션 중 발생 가능(`yuumiTransform`→`_renderYuumiPanel`→`_setYuumi3d`→`ensureYuumi3d`). `_yuumi3dInit` 가드에 `_yuumi3dUnique` 비교 추가 — 같으면 재사용, 다르면 `_teardownYuumi3d`(옛 렌더 루프 `_yuumi3dGen++`로 자가정지·렌더러 `dispose`+`forceContextLoss`·캔버스 제거) 후 재빌드. 렌더 루프 최상단에 `if(_gen!==_yuumi3dGen)return;` 가드로 옛 루프 확실히 정지(WebGL 컨텍스트 누수 방지).
- **검증**: 인라인 7블록 `node --check` 7/7 · principal GLB git 추적 확인 · 배포 success. ⚠️**교장 프레이밍·dig 클립(`Dance_Loop`) 실기 미검증**(3D 못 돌리는 환경) → 크기·위치 어색하면 `YUUMI_POS`(x/y/scale) 또는 dig 클립명만 조정하면 됨. 눈 수정은 claw의 검증된 패턴이라 저위험.
- ⏭️ **미해결(이월)**: 교장 유미 실기 프레이밍 튜닝 · claw.html 미러(`인형뽑기-물리-목업.html`) 미동기 · 인형뽑기 물리/hold/럼블 3D 실기 미검증 · 투기장 엔드게임 코인 sink · 기타 기존 미결.

### v2.45.632 (2026-07-27·이 PC·`main` 직접) — 🎟 복권 Hub 카드 상단 빈공간 제거·무료권/불운 얇은 띠지화
> 사장님: 티어 카드(실버·골드·프리즘) 상단이 무료권/불운 없을 때 휑함(v582 `min-height:36px` 예약 블록). → **예약 블록 제거 + 있을 때만 상단 풀폭 얇은 띠지**. CSS `.lh-tier-badges`: column+min-height:36px → `flex-flow:row wrap`+full-bleed `margin:-12px -10px 9px`+`border-radius:10px 10px 0 0`+subtle bg+하단보더(카드 상단을 두르는 띠지). 렌더=`(freeCnt>0||pityPct>0)`일 때만 wrapper 출력(없으면 이름부터=빈공간0). 칩(.lh-free/.lh-pity-badge) 살짝 얇게(padding 2px6·radius7·free glow 제거). 모바일 규칙(`.lh-tier-grid .lh-tier-badges{display:contents}`)=그대로 inline 칩(밴드 무효). 블랙 복권은 별도 배지(미변경). Edge 목업 실측(빈=compact 정렬·있음=띠지 두름). index 7/7.

### v2.45.631 (2026-07-27·이 PC·`main` 직접) — 🐱 유미 수집 검증 + grantYuumi(n) 테스트 헬퍼
> 사장님 "유미 1마리 이상 정상적으로 나오는 거 맞지(3마리 있어야 변신)" 확인 요청. **코드 검증 완료 — 정상**: 뽑기 게이트 `canYuumi=!_yuumiIsUnique(data)`(교장 변신 전까진 계속 나옴·NOT `!_ownsYuumi`라 1~2마리 보유해도 또 나옴)·`gotYuumi`시 `yuumiCount:_yuumiCount(data)+1`(0.5%/뽑기·재조합 8%도 동일)·구보유자 secretYuumi=1 하위호환·count≥3서 `_yuumiCanTransform`→변신버튼. 실기 검증 쉽게 **`grantYuumi(n)`에 개수 인자 추가**(secretYuumi+yuumiCount=n 세팅, 기본1, `grantYuumi(3)`=변신 가능). index 7/7.

### v2.45.630 (2026-07-27·이 PC·`main` 직접) — 🔥 럼블 챔피언 추가(인형뽑기·투기장·슈퍼 갤럭시 스킨)
> 사장님: 바탕화면 `럼블.glb`(기본)·`슈퍼_갤럭시_럼블.glb`(스킨) 활용해 인형뽑기+투기장에 럼블(8번째 챔피언) 추가. Explore로 기존 7종 등록지점 전수조사 후 반영.
- **GLB**(리포 루트·변환 리스크 0으로 원본 복사·스킨/애니 보존 검증): `rumble.glb`(인형뽑기 미니=원본)·`rumble_full.glb`(투기장 풀=원본 복사)·`rumble_skin1.glb`(슈퍼 갤럭시=원본). `.gitignore` 3줄 화이트리스트. 클립: 기본=Attack1/Idle1/Run/Death/Spell2·3·4(⚠️Spell1 없음)·Idle1~3·Run2·Dance / 스킨=Idle1_Base/Run_Base/Spell1~4/Death/Rumble_Skin03_*.
- **⚠️ 게이트 결정**: 인형뽑기 컬렉션 완성=**8종**(claw `CHAMP_TOTAL` 자동 파생) but **투기장 입장·버디·stock accrual은 7 고정 유지**(`_arenaEligible`/`_buddyOnPull`/`coll>=7` 미변경 — CLAUDE.md "진입장벽 안 부풂" 설계). → **기존 7종 완성자(애긔/울퉁쓰/맹독/빛나는언즈/ap렉사이)는 claw 2단계(무제한 플레이)가 7/8로 잠깐 풀렸다가 럼블 1마리 잡으면 복구**·투기장 접근은 유지. (사장님이 claw 완성도 7 유지 원하면 claw `_collComplete`/`CHAMP_TOTAL` 고정 필요 — 현재는 8.)
- **claw.html**(BUILD 171→**172**): `PRIZE_FILES`에 `{file:'rumble.glb',target:2.89}` · `TROPHY_MAX 7→8` · `_DOLL_KR`에 `rumble:'럼블'`. (완성/바/패널은 `CHAMP_TOTAL` 파생이라 자동 8).
- **index.html**: `ARENA_CHAMPS`(+rumble)·`CHAMP_ELEM`(rumble:venom)·`CHAMP_SKILLS`(화염방사기/고철방패/전기작살)·`ARENA_SKILL_CLIPS`(rumble:Spell2/3/4)·`CHAMP_BATTLE_POS`(rumble.glb+skin·추정값)·`CHAMP_CLIPS`(rumble.glb=Attack1/Idle1/Death·skin=Spell1/Idle1_Base/Death)·`CHAMP_SKINS`(슈퍼 갤럭시 럼블 400코인)·`CHAMP_PREVIEW`(rumble_full/skin1·추정)·`_BUDDY_MODEL_ADJ`(rumble.glb·추정)·`_PLAZA_RUN/WALK/IDLE`(rumble)·하드코딩 로스터 6곳(clawCompleteAll·buddyTunePreview champs·_buddyHomePromptHtml roster·buddy select inline·_bhRoster·_buddyOpenSelect). 가챠(GACHA_CHAMPS)는 **별개 세트라 무관**(미편집).
- 검증: index 7/7·claw 1/1·GLB glTF 파싱 OK(skin/anim 보존). ⚠️**3D 실렌더·전투 배치(CHAMP_BATTLE_POS/PREVIEW/BUDDY_ADJ)·클립 적합성은 실기 미검증**(추정값) → 실기서 어색하면 목업(투기장-전투-목업.html·스킨-위치조정-목업.html)·콘솔(skinPrevTune/buddyTunePreview)로 튜닝. claw.html 미러(인형뽑기-물리-목업.html)는 여전히 미동기.

### v2.45.629 (2026-07-27·이 PC·`main` 직접) — 🔀 컬렉션 일괄 합성 버튼(파편·GOLD 한 번에)
> 사장님: 컬렉션 합성에서 남는 3성 재조합 버튼처럼, 합성 가능한 파편/2성을 버튼 하나로 한 번에 합치기. `window.doGachaMergeAll(tier)` 신설(tier 1=파편→GOLD·2=GOLD→PRISM): 챔프별 `floor(count/3)`만큼 s{tier}→s{tier+1} 일괄(수동 doGachaMerge 여러 번=1버튼·무보호=수동과 동일)·gachaLog에 op:'merge',bulk:true 누적·baseline 박제·낙관적 로컬반영+renderGachaTab 재렌더·`_ctrlGuard`·`_gSfx.mergeImpact`(typeof 가드). 버튼=`renderGachaTab`서 gachaStar 1/2 탭에 `mergeAllHtml`(합성가능 sets>0일 때·초록 `cz-merge-action`·"⬆ 합성 가능한 {파편/GOLD} 한 번에 합치기 · N종 → {GOLD/PRISM} M장")·concat에 recombineHtml 뒤 삽입. GACHA_TIER_LABEL={1:파편,2:GOLD,3:PRISM}. index 7/7. ⚠️실기 미검증(가챠 카드 필요)·무보호라 2성 3개 정확보유 챔프는 합성 시 2성0/3성+1(=상위로·의도).

### v2.45.628 (2026-07-27·이 PC·`main` 직접) — 🎓 교장 유미 업그레이드 배너 한 줄로 간소화 + ? 모달
> 사장님: 진행도 배너 여러 줄 글이 줄 어긋나 별로 → 한 줄로 간단히·자세한 설명은 ?모달로. `_renderYuumiPanel` 진행도 `_tf`를 `.yuumi-slim` 한 줄(tag "🎓 교장 유미 업그레이드" + action "N/3 🐱⚪⚪ [?]")로 재작성. ? 버튼=**기존 `window.showYuumiDetail()`**(유미 상세 모달·교장 유미 업그레이드 설명 li 보강: 효율 5배·700~1100G·일반 130~230·휴식 없음) 재사용. Edge 헤드리스 목업 실측(한 줄·「유미 SECRET·보내기」행과 톤 일치). index 7/7.

### v2.45.627 (2026-07-27·이 PC·`main` 직접) — 🐱 유미 휴식(쿨다운) 완전 제거·강철심장 유미효과 통합 + 🎓 변신 진행도 안내
> 사장님: ①교장 유미는 휴식 없이 파견으로 효율(연속 파견) ②일반 유미 휴식도 제거 → 강철심장 '유미 휴식'(yuumiCool)을 '유미 파견'(yuumiCut)으로 통합, 교장도 동일 적용 ③둘 다 파견 불가(=이미 단일 슬롯이라 구조상 충족) ④쓰레기통/유미에 "유미 3개로 교장 업그레이드" 안내(기존 보유자 동기부여).
- **쿨다운 전면 제거**: `collectYuumi`가 항상 `f={}`(startAt·cooldownUntil 없음) → 수령 후 `_yuumiState`=ready 즉시 재파견(일반·교장 공통). `YUUMI_COOLDOWN_MS`·`YUUMI_MIN_COOLDOWN_MS`·`_yuumiCooldownMs` 삭제. 정산 sub 문구=전원 "🐾/🎓 휴식 없이 바로 다시 파견". 상세모달 ③=「바로 재파견(쿨다운 없음)」. ⚠️레거시 `cooldownUntil` 남은 유저는 만료까지 1회 cooldown 표시(무해).
- **강철심장 걸작 통합**: `EMBLEM_EFFECTS.yuumiCool` 삭제·`EMBLEM_EFFECT_POOL`에서 'yuumiCool' 제거(→ 신규 롤 안 나옴). 옛 `yuumiCool` 줄은 `emblemSanitizeLines`(EMBLEM_EFFECTS[eid] 없으면 rnd)로 대장간 열 때 자동 치유. `yuumiCut`(유미 파견=파견시간 단축·`_yuumiForageMs`)만 남음 — deploy 시점 적용이라 **교장 유미도 동일 수혜**(코드상 이미 unique 무관 적용). 걸작 표시/합산(`emblemEffectsOf`)은 pool 기준이라 안전(잔여 yuumiCool 줄은 치유 전까지 0 기여·무크래시).
- **변신 진행도 안내**: `_renderYuumiPanel`에서 유미 보유·`_yuumiCount()<3`·미변신이면 `_tf`에 진행도 배너("유미 N/3 · 3마리 모으면 교장 유미=쓰레기통 효율 5배+휴식없음!" + 🐱×c⚪×남은). count≥3=변신버튼(기존). 상세모달에도 교장 유미 li(v626~).
- **동시 파견 불가**: `yuumiForage` 슬롯 1개뿐(교장이든 일반이든 동일 슬롯) → 구조적으로 항상 한 마리만. 코드 변경 불필요(확인).
- 검증: index 7/7 · yuumiCool 코드참조 0(주석만). ⚠️교장/걸작 실보상은 유니크·엠블럼 계정 필요라 실기 미검증(로직 additive·치유 기존검증). 경제: 유미 무캡+무휴식이라 이론상 faucet↑(일반 ~180G/시간·교장 ~900G/시간, 온라인 상주 시)이나 교장=0.5%×3 극희귀 엔드게임. 과하면 파견시간↑·일일캡으로 조절 여지.

### v2.45.626 (2026-07-27·이 PC·`main` 직접) — ❌ 「뒷골목으로 나가기」 미니게임 제거 + 🎓 교장 유미=쓰레기통 고효율(약 5배)
> 사장님: "뒷골목 게임 별로다, 빼자. 유니크 유미만 남기고, 쓰레기통에서 교장 유미 효율을 매우 높게." → **v621~625 뒷골목 미니게임 전면 제거**, 교장 유미 변신(Phase 1)은 유지, 기존 쓰레기통 파견(deploy/collect)에 **고효율만 부여**.
- **제거**: index.html 뒷골목 블록 통삭제(`ALLEY_*` 상수·`ALLEY_STAGES`·`_alleyState/_alleyTodayGold/_alleyDailyRemaining/_alleyCooldownLeft`·`_injectAlleyCss`·`ensureAlleyYuumi3d`/`_evacuateAlleyYuumi3d`/`_ALLEY_POS`/`_alleyY3dTune`·게임상태 vars·`openAlleyRun`/`alleyPreview`/`_alleyBuild`/`_alleySpawnOne`/`_alleyLandPuff`/`_alleyTick`/`_alleyCollectOne`/`closeAlleyRun` 전부). `?alleypreview` URL 훅 제거. `_renderYuumiPanel`서 뒷골목 버튼/쿨다운타이머(`_alleyBtn/_alleyCdEnd/_alleyCdTimer/_alleyState`) 참조 제거 → `_tf`(변신) prepend만 복원. grep로 잔여 alley 코드참조 0 확인(변경내역 문자열만 이력으로 잔존).
- **교장 유미 고효율**(`collectYuumi`): `const uniq=_yuumiIsUnique(data)`; base = uniq ? **700~1100G** : 130~230G(약 5배). 지급 경로·구경보너스(≤60)·무료권(하루3장 공유)·쿨다운·trashGold 버킷·역산로그(ev:'yuumi'에 `uniq` 추가) **전부 그대로**. `deployYuumi` 토스트·`_renderYuumiPanel`(슬림태그 `🎓 교장 유미 고효율`·보내기버튼 `💰고효율` 배지)·변신 리빌 문구·유미 상세모달(② 아래 교장 유미 안내 li)만 교장 유미 반영.
- **경제**: 유미 파견은 **일일캡 없음**(기존 설계 유지)·1시간 파견+1시간 쿨다운(걸작으로 최소 30분씩 단축 가능). 교장 ~900G/사이클(~2시간) → 이론상 하루 최대 ~9000G지만 **교장 유미=유미 3마리(0.5% 뽑기) 필요=사실상 현재 보유자 0명 엔드게임**이라 사장님 요청대로 고효율 유지(캡 미추가). 필요 시 나중에 일일캡/쿨다운 조정 여지.
- ⚠️ **`assets/pets/Yuumi/3d/yuumi-principal.glb`(7MB) 현재 코드 미사용**(뒷골목 3D 제거로). 삭제 안 함 — 교장 유미 모델이라 향후 쓰레기통 3D에 교장 유미 표시 등에 재활용 가능. 커밋 유지.
- 검증: index 7/7 · 잔여참조 0. ⚠️교장 유미 실보상은 유니크 계정 필요라 실기 미검증(로직 additive).
- ⏭️ (선택) 쓰레기통 3D를 교장 유미(principal GLB)로 표시·교장 전용 연출·유미 파견 일일캡 검토.

### v2.45.625 (2026-07-27·이 PC·`main` 직접) — 🌃 「뒷골목으로 나가기」 탑다운 시점 전환 + 뒷골목 배경 재조정 [❌ v626서 제거됨]
> 사장님: "하늘에서 떨어지는 느낌(측면 중력·최하단=바닥)이 아니라, **상단에서 누군가 던진 쓰레기가 필드에 떨어져야** 하고, 뒷골목 느낌이 전혀 안 남." → **측면뷰→탑다운 필드**로 전환.
- **물리(측면 중력 → 탑다운 착지)**: 쓰레기가 바닥(H-16)으로 쌓이던 것 폐기. `_alleySpawnOne`이 상단 **문(W*0.5)에서 출발**(y=H*0.1), 좌우로 던져(vx ±180) 각자 **`landY`(H*0.30~0.92, 필드 곳곳 깊이)** 로 낙하 → `t.y>=landY`서 착지(landed·먼지 `_alleyLandPuff`)·남은 vx로 조금 구르다 정지. 낙하 중 `sc=0.68~1.0`(다가올수록 커짐=탑다운 깊이감). `ALLEY_GRAV 1500→1050`(완만 궤적). 흡입은 낙하/착지 무관 동일.
- **배경(측면 벽/바닥 → 탑다운 뒷골목 바닥)**: `.alley-bg-wall`/`.alley-bg-ground` 제거. 신규 `.alley-topwall`(상단 얇은 담벼락)+`.alley-door`(불 켜진 문=쓰레기 출처·글로우)+아스팔트 바닥(균열 repeating-gradient·가로등 빛무리 radial·비네트 inset shadow)+`.alley-puddle`(웅덩이 sheen)×2+`.alley-manhole`(맨홀)+💡가로등+🗑️쓰레기통×2+📦. 데코 위치는 `_alleyBuild` 인라인 style.
- 데이터/경제/스크롤잠금/프리뷰/쿨다운·일일캡 전부 유지. index 7/7 · Edge 헤드리스 목업 실측(문·담벼락·빛무리·웅덩이·맨홀·쓰레기통·낙하중(작게)/착지(크게) 쓰레기 OK). ⚠️3D 유미·실제 착지 궤적 체감은 `?alleypreview` 실기 확인.
- ⏭️ 후속(Phase 3) 동일: 아이템/스킬 상점·다중 스테이지·무료복권 드롭·3D 유미 튜닝.

### v2.45.624 (2026-07-27·이 PC·`main` 직접) — 🌃 「뒷골목으로 나가기」 드래그 시 화면 스크롤/튐 수정(스크롤 잠금)
> 사장님 제보: 가끔 잘 되다가 드래그가 스크롤과 겹치면 화면 전체가 막 움직임(모바일 페이지 팬/러버밴딩·주소창 토글로 무대 vh 변동 추정). 원인=오버레이가 `body.overflow:hidden`만 걸고 터치 스크롤/오버스크롤을 안 막아, 드래그가 페이지 팬으로 새거나 주소창 토글로 `min(58vh,460px)` 무대 높이가 변해 좌표 어긋남.
- **수정**: ①`.alley-overlay`에 `touch-action:none`+`overscroll-behavior:contain`+`user-select:none`+`.alley-overlay *{touch-action:none}` ②오버레이에 **비패시브 `touchmove` preventDefault**(el.addEventListener passive:false) — 페이지 스크롤/러버밴딩 원천 차단(포인터 드래그는 그대로) ③`stage.onpointermove`에 `e.preventDefault()` ④**resize 핸들러 `_alleyOnResize`** — 주소창 토글 등으로 무대 크기 바뀌면 `_alleyStageRect` 갱신+유미 위치 클램프(좌표 어긋남 방지), closeAlleyRun서 리스너 제거. index 7/7. ⚠️실기 재확인 필요.

### v2.45.623 (2026-07-27·이 PC·`main` 직접) — 🌃 「뒷골목으로 나가기」 게임성 대개편(뒷골목 배경·낙하 물리·스테이지 쓰레기·반응성·프레이밍)
> 사장님 프리뷰 피드백 반영: ①유미 머리 잘림 ②드래그 반응성 부족 ③게임성 방향(뒷골목 배경·위에서 떨어져 뒹구는 쓰레기·종류 제한·아이템/스킬 확장). 다섯 가지 손봄.
- **①프레이밍**: 카메라 여유 `*1.72→*2.2`·`_ALLEY_POS` {y:-0.06,scale:0.92}·마운트 104×138(↑)·이동 클램프 `_ALLEY_YHEAD=82`/`_ALLEY_YFOOT=12`(무대 overflow:hidden에 머리/발 안 잘리게). 여전히 어긋나면 콘솔 `_alleyY3dTune(x,y,scale,rot)`.
- **②반응성**: 고정속도 lerp → **지수 스무딩 추적**(`k=1-exp(-ALLEY_FOLLOW·dt)`·FOLLOW=15) = 손가락에 딱 붙음. drag setT가 target 갱신, tick이 pos를 target으로 빠르게 수렴.
- **③배경(2D 뒷골목)**: `.alley-bg-wall`(위 벽돌담)+`.alley-bg-ground`(아래 아스팔트+퍼들)+데코 `.lamp`(가로등 글로우)/`.moon`/`.dumpster`/`.box`. 기존 단색 아스팔트 대체.
- **④낙하 물리**: 쓰레기 사전배치 폐기 → **스포너**(`_alleySpawnOne`·`ALLEY_SPAWN_MS=380`·`ALLEY_MAX_CONCUR=20`)가 위(y=-22)에서 떨어뜨림. tick에 중력(`ALLEY_GRAV=1500`)·바닥 바운스(감쇠 0.4)·굴러가기(마찰 exp)·벽 반사·회전. 흡입은 낙하/지면 무관 `ALLEY_VAC_R=74` 내 자동.
- **⑤쓰레기 종류 제한(스테이지)**: `ALLEY_STAGES`(현 1개=뒷골목 초입: 🥫캔10·🍾병20·💵지폐55, 가중치). `_alleyStageFor(data)`(현 스테이지0 고정)·`_alleyValMult(type,data)`(현 1 고정) = **아이템/스킬 확장 골격**(⏭️나중에 스테이지 진행+아이템으로 새 쓰레기 해금·특정 효율↑). 잡다한 12종 이모지 랜덤 → 스테이지 3종으로.
- **경제/종료**: 예산=`min(일일남은, 1000±)`을 스포너가 다 뿌리면(`_alleySpawnVal≥budget`) + 남은 쓰레기 0 → 종료. 55초 타이머 백스톱. 수집은 `_alleyCollectOne`서 목표(budget) 초과 안 함. 정산·쿨다운·일일캡·trashGold·프리뷰 우회 전부 유지(v621~622).
- **검증**: index 7/7 · Edge 헤드리스 2D 목업 실측(벽돌담·바닥·가로등·달·쓰레기통·낙하/지면 쓰레기 3종·유미자리 OK). ⚠️**3D 유미 프레이밍·낙하 물리 체감·드래그 반응성은 실기기 미검증** → `?alleypreview`로 확인. 어색하면 `_alleyY3dTune`·`ALLEY_FOLLOW`·`ALLEY_GRAV`·`ALLEY_VAC_R`·`ALLEY_SPAWN_MS` 조정.
- ⏭️ **후속(Phase 3)**: 아이템/스킬 상점(새 쓰레기 해금·특정 쓰레기 효율↑·흡입 반경/이동속도 업글)·다중 스테이지(스테이지별 쓰레기·난이도·목표)·무료복권 드롭·사운드/연출. 3D 유미 실기 튜닝.

### v2.45.622 (2026-07-27·이 PC·`main` 직접) — 🌃 「뒷골목으로 나가기」 프리뷰 모드(게이트·저장 우회)
> 사장님 "데이터 문제 없이 내가 확인할 수 있게" → 교장 유미 없이·실데이터 0영향으로 미니게임 실플레이 확인용 프리뷰. **URL `?alleypreview`**(모바일) 또는 콘솔 **`alleyPreview()`**. `_alleyPreview` 플래그: `_alleyBuild(getMyGoldData()||{})`로 게이트 우회(닉네임/유니크/쿨다운/캡 무시·`_alleyState` 안 탐), `closeAlleyRun`이 프리뷰면 **Firebase write·trashGold·쿨다운·일일캡 전부 스킵**하고 "+NG (저장 안 됨)" 토스트만. 제목에 "· 미리보기" 배지. 진입점(`?alleypreview` at ~L25128 옆·`window.alleyPreview`). ⚠️3D 교장 유미 프레이밍/드래그/흡입 체감을 이걸로 실기 확인 → 어색하면 `_alleyY3dTune`/`ALLEY_SPEED`/`ALLEY_VAC_R` 조정. index 7/7.

### v2.45.621 (2026-07-27·이 PC·`main` 직접) — 🌃 「뒷골목으로 나가기」 미니게임 신설 (교장 유미 Phase 2)
> 유니크(교장) 유미 전용 액티브 미니게임. **드래그로 유미 이동 → 근처 쓰레기 자동 흡입(Garbage Growth식) → 골드**. Phase 1(v619 수집·변신)의 후속.
- **렌더=하이브리드**: 게임플레이(이동·흡입·쓰레기·HUD·파티클)는 **2D DOM/rAF**(리스크↓·모바일 안정), 캐릭터는 **3D 교장 유미**(`yuumi-principal.glb`·Idle01/Run01)를 위치추적 **오버레이 캔버스**로. 월드공간 3D 이동 대신 **CSS transform으로 마운트 이동 + scaleX 좌우 반전** → 3D는 정면 정지 렌더만(ensureYuumi3d 패턴 복제). ⚠️모델 프레이밍 실기 미검증 → 콘솔 `_alleyY3dTune(x,y,scale,rot)`로 조정 가능(`_ALLEY_POS` 기본 {0,-0.12,1,0}).
- **코드맵**(index.html·`_renderYuumiPanel` 직전 블록·~L29216 부근): 상수 `ALLEY_COOLDOWN_MS`(90분)·`ALLEY_DAILY_CAP`(2500)·`ALLEY_RUN_TARGET`(1000)·`ALLEY_SPEED/VAC_R/COLLECT_SPD/TIME_MS`. 상태 `_alleyState`(locked/capped/cooldown/ready)·`_alleyTodayGold`/`_alleyDailyRemaining`/`_alleyCooldownLeft`. 3D `ensureAlleyYuumi3d`(캔버스 `alley-yuumi3d`·마운트 `alley-yuumi3d-mount`·홀더 대피 `_evacuateAlleyYuumi3d`·`window._alleyY3d.setMoving/reframe`). 게임 `window.openAlleyRun`→`_alleyBuild`(무대 `#alley-stage`·rAF로 크기확정 후 쓰레기 스폰·드래그 바인드)→`_alleyTick`(이동 lerp·흡입 `ALLEY_VAC_R`내 자동수집·HUD/타이머)→`_alleyCollectOne`(floating +NG·코인SFX 스로틀)→`window.closeAlleyRun(discard)`(정산). `_injectAlleyCss`(`.alley-*`). 진입 버튼=`_renderYuumiPanel`서 `_yuumiIsUnique()`면 `_alleyBtn`(ready=버튼/cooldown=라이브카운트다운 `_alleyCdTimer`/capped=안내) 패널 맨 위 prepend.
- **경제**: 한 판 쓰레기 값 합계 ≈ `min(ALLEY_RUN_TARGET±, 오늘 남은 캡)`. 정산 시 `min(주운값, 일일남은)`을 **`trashGold`(sField)** 에 지급(calcPlayerGoldEarned 합산·L25458)+`alleyGoldDay`/`alleyGoldDate`(일일캡 추적, 합산 안 함)+`alleyLastAt`(쿨다운)+`trashLog` ev:'alley'(역산). **90분 쿨다운+하루 2500G 캡**으로 무한파밍 차단. 수동 뒤적 250G 캡과 **별개 버킷**(alleyGoldDay ≠ trashGoldDay). ⚠️유니크 유저는 기존 유미 파견(idle)도 그대로 사용 가능 → 총 faucet 다소 큼(단 교장 유미=3마리 수집 필요=0.5% 가챠라 현재 보유자 사실상 0명, 엔드게임).
- **검증**: index 7/7 · **Edge 헤드리스 2D 목업 실측**(HUD·놀이터·쓰레기 코인·유미자리·수령버튼·드래그안내 OK·출력 Temp/HTML file:/// see [[reference_edge_headless_screenshot]]). ⚠️**3D 교장 유미 프레이밍·드래그 이동·흡입 체감은 실기기 미검증**(three.js 물리/게임플레이 헤드리스 곤란) → 사장님 확인 필요. 어색하면 `_alleyY3dTune`·`ALLEY_SPEED`/`ALLEY_VAC_R` 조정.
- ⏭️ **미해결/후속(Phase 3)**: 3D 교장 유미 프레이밍 실기 튜닝 · 뒷골목 무료 복권권 드롭(현재 골드만) · 배경/연출/사운드 폴리시(knock 등) · 업그레이드/스킬트리 · claw.html 미러 미동기 · 투기장 엔드게임 sink · 기존 미결.

### v2.45.620 (2026-07-27·이 PC·`main` 직접) — 🌃 유니크 유미 해금 콘텐츠 이름 「뒷골목으로 나가기」로 변경
> 사장님: "고급 쓰레기통 말고 **「뒷골목으로 나가기」**(유니크 유미 획득 시)로." → v619의 유저노출/주석 문구를 전부 개명. 기존 **뒷골목 쓰레기통**(=복권 밑 수동 뒤적 미니게임)과 컨셉 구분: 통을 뒤지는 게 아니라 **유미와 함께 뒷골목으로 나가 흩어진 걸 줍기**(Garbage Growth식). 변경=변신 리빌 `showBingoReward`(🌃 「뒷골목으로 나가기」 곧 열려요·보상칩 아이콘 🗑️→🌃)·CHANGELOG v619 텍스트·주석 2곳(L28972·L29200 근처). 데이터/로직(`yuumiUnique` 플래그·수집·변신) 무변경=순수 리네이밍. index 7/7. Phase 2 미니게임 명칭도 앞으로 「뒷골목으로 나가기」로 진행.

### v2.45.619 (2026-07-27·이 PC·`main` 직접) — 🐱🎓 유미 수집 & 교장 유미 변신 (「뒷골목으로 나가기」 Phase 1)
> 사장님 유미 대형 업데이트(투입해두고 돈 벌어오는 고급 쓰레기통·유미 3마리 모아 교장 변신·바탕화면 `전투사관학교_유미_교장선생님.glb`). AskUserQuestion: **①1단계=유미 수집→교장 변신+개방** ②변신은 가챠서 3마리 모아 ③한 번에 한 통만 ④파견 미니게임=클릭 아닌 **끌어서 이동(드래그)·Garbage Growth식**(통 밖 흩어진 쓰레기 줍기). → **이번 배포=Phase 1(수집·변신·해금 플래그)만**. 고급 쓰레기통 미니게임 자체(Phase 2)는 미구현.
- **GLB**: 바탕화면 원본 13.5MB → `gltf-transform`(webp 1024·meshopt) **7MB**로 압축해 `assets/pets/Yuumi/3d/yuumi-principal.glb` 커밋(`.gitignore` 화이트리스트 `!assets/pets/*/*/*`라 추적됨). Phase 1은 **미로드**(변신 리빌은 2D `showBingoReward`)·Phase 2 고급 통서 3D 데뷔 예정.
- **데이터/헬퍼**(index.html `_ownsYuumi` 뒤 L28968~): `YUUMI_TRANSFORM_NEED=3` · `_yuumiCount(data)`(신규 `yuumiCount` 필드·**구 보유자=`secretYuumi` true면 1마리로 하위호환**) · `_yuumiIsUnique`(`yuumiUnique` 플래그) · `_yuumiCanTransform`(3마리&미변신).
- **가챠 수집**: 뽑기 경로(pull gate)·재조합(`doGachaRecombine3`) 둘 다 게이트를 `secretYuumi` 1회성 → **`!_yuumiIsUnique(data)`(교장 변신 전까지 계속 나옴)**로 바꾸고, 획득 시 `yuumiCount+1` 증가(update+로컬반영)+`N/3` 진행 토스트. 변신(unique) 후엔 더 안 나옴.
- **변신**: `window.yuumiTransform`(닉/데이터·이미변신·3마리 미달 가드 → `yuumiUnique:true` update+로컬반영 → `_trashSfx.free()` + `showBingoReward('🎓 교장 유미 변신 완료!', ...·고급 쓰레기통 준비 중)` + `_renderYuumiPanel` 재렌더). `_renderYuumiPanel`(L29197~)에 `_yuumiCanTransform`이면 상단 변신 버튼(`_tf`) 주입(`el.innerHTML = _tf + el.innerHTML`).
- **검증**: index 7블록 node --check 7/7 · 식별자·백워드호환(구 유미 보유자=1마리 인정, 2마리만 더 모으면 됨) 확인. ⚠️가챠 실경로(뽑기 확률·재조합)는 로그인+가챠 데이터 필요라 헤드리스 미검증(로직은 additive·backward-compat).
- ⏭️ **미해결(이월·Phase 2/3)**: 🗑️**고급 쓰레기통 미니게임**(Garbage Growth식·드래그로 유미 이동→통 밖 흩어진 쓰레기 자동 흡입→골드·~1000G/사이클·한 번에 한 통·`yuumiUnique` 게이트·3D 교장 유미 사용·별도 필드/일일캡) + 유미 파견(~1000G) · Phase 3=GLB 추가 최적화/업그레이드/사운드(`knock()`=두드려 여는 연출) · claw.html 미러 미동기 · 투기장 엔드게임 sink · 기존 미결.

### v2.45.618 (2026-07-27·이 PC·`main` 직접) — 🤖 자동로봇 캐릭터 CSS아트 고퀄 리디자인
...
> - **증상**: 이 세션 중 **GitHub Pages 클래식 자동 트리거(=`pages build and deployment` 워크플로우)가 멈춤** — main에 여러 번 push해도 2시간+ 빌드 run이 아예 생성 안 됨(직전 커밋까지는 정상). GitHub 전체 장애 아님(githubstatus 정상)·특정 repo 트리거만 stall. 라이브가 오래도록 옛 BUILD로 남았던 원인.
> - **해결**: `.github/workflows/pages-rebuild.yml` 신설 — `on: push(paths: index.html·claw.html·워크플로우) + workflow_dispatch`, `permissions: pages:write`, 스텝에서 `curl -X POST .../repos/${repo}/pages/builds`(GITHUB_TOKEN)로 **클래식 Pages 재빌드 요청**(= Settings→Pages 재저장과 동일·**배포 방식은 안 바꿈**·HTTP 201 확인). **커스텀 워크플로우는 push로 정상 트리거**되므로(클래식 Pages 트리거만 stall) 이게 배포를 대신 걸어줌. 이후 모든 배포가 이걸로 정상화됨.
> - ⚠️ **삭제 금지**(자동 트리거 또 멈추면 이게 유일한 자동 배포 경로). ⚠️ **MCP github `run_workflow`(dispatch)는 403**(Resource not accessible by integration)이라 dispatch 불가 → **push로 트리거**해야 함. `.gitignore`가 `*`+화이트리스트라 워크플로우는 `git add -f` 필요.
> - 배포 확인: `mcp__github__actions_list`(list_workflow_runs·event=dynamic)로 head_sha 완료/성공 확인(결과 토큰초과→파일저장→python json 파싱). 새 push가 이전 in-progress 빌드를 **cancelled**로 만드는 건 정상(concurrency).
>
> ### 🕹️ 인형뽑기(claw.html) BUILD 164~171 — 치명타 보너스집게 + 상단 UI 대개편
> ⚠️ **구조**: 실제 게임=**`claw.html`**(iframe·three.js+cannon-es). 라이브는 claw.html 로드. index.html=래퍼(브라우저 창 모달 `openClawTest3D`·`_claw3dInit`/`_claw3dSave` 브릿지). 미러 `인형뽑기-물리-목업.html`은 치명타 집게 기능 자체가 없음(미동기·라이브 무관).
> - **BUILD164**: 치명타 연쇄 **보너스 집게가 완전 랜덤 위치→남은 인형(prizes 중 !won) 겨냥**(±0.55 흔들림). `_startBonusDrop`. 출구 구멍엔 인형 없어 헛집게 되던 것 방지.
> - **BUILD165**: `BONUS_SPDX 1.95→2.6`(하강·복귀·닫기↑) · 보너스 중 집게(`steel` 재질=집게 전용) **무지개 발광 펄스**(loop서 hue순환·종료 시 은색 복귀·`_clawBonusGlow`/`_steelBaseColor`) · 첫 치명타 즉발→**`BONUS_WAIT`(0.65초) 상태** 추가(집게 빛나며 「⚡보너스 집게!」 예고+두근호버 후 출동).
> - **BUILD166~168 상단 UI**: **BGM 조정 안 되던 근본원인=`#hud`가 `pointer-events:none`이라 슬라이더/버튼 클릭 막힘** → BGM을 **컬렉션 바 우측 🔊 아이콘**으로 이동+탭 팝오버(`_bgmPopToggle`)·`#hud` display:none(화면 안 가림) · 컬렉션 **7/7 완성 후 스트립→「🧸컬렉션 ▾」 버튼접기+수량 팝오버**(`_collPopToggle`/`_collPopRender`·`_dollStock`은 TDZ 회피 위해 팝오버 탭 시점에만 렌더) · **코인 상단 골드칩**(`#cb-coin-top`, 하단은 무제한이면 표기 안 함) · 톤 **투기장(검정+골드)** 통일 · 조작(시점/들이대기/집기 배지)→**화면 위 반투명 좌(조이스틱)·우(집기+돋보기)·시점◀▶ 좌상 은은**(터치 READY, `body.is-touch #controls:not(.idle)` 절대배치·joystick/버튼 ID 바인딩이라 재구성 안전) · 카메라 상단 인셋 터치 ×0.35.
> - **BUILD169**: index.html 래퍼 **브라우저창 느낌 제거**(가짜 주소창 `아수라장-인형뽑기.gg/play`·최대화 버튼 삭제·`.claw-win*` 투기장 톤) · **돋보기 버튼 글자 안 맞던 원인=`toggleCloseUp`이 innerHTML을 「🔭 물러서기」 텍스트로 바꾸는데 아이콘 원형버튼이라 넘침** → **라벨 알약**(`.vbtn.zoom` 「🔍 돋보기」↔「🔭 물러서기」) · 치명타 「⚡」만→**「⚡ 치명타」 라벨칩**(`.claw-crit-mini`·장착 시만·미보유 시 구매CTA 배너).
> - **BUILD170**: BGM 팝오버에 숨어 안 보이던 **BUILD 표시→좌상단 작은 고정 라벨**(`#build-pill` position:fixed·3탭=직원 튜너 유지).
> - **BUILD171**: 치명타 판정(`_onGrabResolved`) 시 `_SPDX=BONUS_SPDX`+발광 즉시 적용(그 판 복귀 빠르게·보너스 예고) · 빈손(실패) 복귀 올리기 ×1.8·RETURN 2.2→3.2배.
>
> ### 🐾🛡️ index.html 변경 (v609~615)
> - **v609**(BUILD166 동반): 인형뽑기 상단 톤·컬렉션·코인·치명타 칩(위 claw와 짝).
> - **v611 🐾 포로 위로코인 10→50**(`ARENA_PORO_COIN`·`_clawPoroGrant`·`_claw3dInit`의 `poroCoin`으로 게임내 문구도 자동). 참여율↑용·하루 플레이 5회 고정이라 무한파밍 없음(최대 ~250/일). 사장님이 "50" 지정.
> - **v612 🛡️ 자동로봇 강철심장 당첨금 보너스 누락 수정**: 진단=`lottoTkt`(해골 감소) 엠블럼은 ①해골출현 감소(`rollScratch`가 `getMyGoldData` 읽어 롤시점 적용=로봇도 O) ②당첨금 보너스(`_myLottoPrizeBonus`)인데 **②가 로봇 정산서 누락**(수동만). → `autoBotReserve`가 당첨티켓에 **예약 시점 장착 기준** `t.eb=_myLottoPrizeBonus(data,i)` 저장, `autoBotClaim`이 `emBonus` 합산·정산팝업/영수증/미리보기 claimTotal/로그 전부 반영(구버전 큐 `eb` 없어 0). 예약시점 기준=수령 때 재장착 불필요(사장님 요청).
> - **v613 🧾 영수증 투여금액 항목별**: `autoBotReserve`가 종류별 지불골드 `spentBy`+무료권 `useFreeBy`를 큐 `q.spent/q.free`에 저장 → 영수증 패널 「💸 투여 금액(종류별)」+「🎟 개봉 결과」 분리, 전량수령 시 정산팝업에도 투여 항목(부분수령 땐 영수증만). 구버전 큐는 mix×가격 폴백.
> - **v614 🤖 로봇 기본선택 실버1→전부0**: `_autoBotSelCnt` 기본 `[1,0,0]→[0,0,0]`(3곳)+`openAutoBotModal` 열 때마다 `[0,0,0]` 리셋. 원하는 복권 직접 선택.
> - **v615 🔁 수량 +/- 꾹눌러 가속**: `autoBotAdjLot`이 매번 `_autoBotRefresh`(버튼 교체)라 **오버레이 pointerdown 위임 + window pointerup/cancel 정지**(재렌더에 강함)·0.4초 후 반복 시작 110→45ms 가속(`_abotHold`/`_abotHoldStop`)·버튼 onclick→`data-abstep/idx/dir`·`autoBotAdjLot` 변화없으면 재렌더 스킵+bool반환·`.abot-qbtn` touch-action:none.
>
> ### ⏭️ 미해결/이월
> - ⚠️ **인형뽑기 물리·hold 실기 미검증**(개발환경서 three.js 물리·터치 hold 못 돌림) — 실기기 확인 필요(치명타 연쇄 느낌·조작 버튼 위치·+/- 꾹누르기 속도).
> - ⚠️ **claw.html 미러 `인형뽑기-물리-목업.html` 미동기**(치명타 집게 등 v605~171 미반영·라이브 무관하나 향후 미러서 재편집 시 유실 주의).
> - 기존 미결(투기장 엔드게임 +20 이후 코인 sink·오버레이 실기·보안 공개링크·복권 해골감소 인플레 등) 유지.

### v2.45.608 (2026-07-24·이 PC·`main` 직접) — 🗡️ 투기장 배틀 코인 ~+55% 상향(강화 병목 완화)
...
### v2.45.606 (2026-07-24·이 PC·`main` 직접) — 🎯 치명타 밸런스 튜닝(CRIT_Q 0.5→0.7·연쇄 5→4)
...
- **index.html**: `_claw3dInit` 반환에 `critClaw:(items에 claw_crit 보유)` 추가(iframe init 전달). 상점 아이템 `{id:'claw_crit', type:'claw_tool', price:600}`(S1_SHOP_ITEMS L10655)·buyItem `claw_tool` 분기(영구소유·골드차감·purchaseLog)·shop 카드 `isClawToolOwned`(보유중 상태). claw_tool=`isConsumeType` 아님→**장비 탭** 노출(scratch_tool처럼 hide 아님).
- **밸런스 주의**: grabQ 공식상 성공 집기 대부분 ≥0.5라 치명타 잦음(연출 잦음·장비시 보너스도 잦음). 사장님 지정 50%라 유지, 잦으면 `CRIT_Q`↑ 튜닝.
- **검증**: index 7/7·claw.html 1/1 문법 OK · Edge 목업 치명타 연출(번개 파박) 실측. ⚠️**물리 실동작(보너스 드롭·연쇄·속도) 실기기 미검증**(헤드리스로 물리 못 돌림) → 실기 확인 필수.
- ⚠️⚠️ **미러 `인형뽑기-물리-목업.html` 미동기화** — 라이브 무관하나 향후 목업서 재미러 시 claw.html 변경 유실 위험. **동일 편집 적용 필요(claw.html 기준)**.
- ⏭️ **미해결(이월)**: 인형뽑기 치명타 실기 검증·미러 동기화·(원하면)진짜 병렬2집게 폴리시 · 기존.

### v2.45.604 (2026-07-24·이 PC·`main` 직접) — 🖤 블랙 팟 교차 적립(실버/골드/프리즘 유료 구매금 5%)
...
- **경로 보존**: 조회/수령/교체=홈 단짝 클릭→단짝창 · 프로필→단짝창 열기 · 수령=대시보드 알림(`buddyClaimPrompt`)도 그대로. `_buddySpaceHtml`은 dev 프리뷰(L38790)서만 잔존.
- **3D 마운트**: 아케이드 `buddy-space-mount` 사라짐 — 후처리 `_buddyHomeAttach`(L27357)는 try/catch+"보이는 마운트만" 부착이라 무해.
- **검증**: `aram-check` 7/7 · 식별자(window 접두 포함) 확인. ⚠️Edge 헤드리스 스샷 이 PC 계속 미작동 → 시각 미검증. **라이브서 아케이드/홈 단짝 클릭→단짝창/교체 흐름 확인 권장**.
- ⏭️ **미해결(이월)**: 아케이드·단짝창 시각/흐름 확인(v595~597) · 자동로봇 실기(v593~594) · 기존 미결.

### v2.45.596 (2026-07-23·이 PC·`main` 직접) — 🕹️ 아케이드 정리 2탄(단짝 스택 제거·진입버튼 이모지 제거·색띠 구분)
> 사장님: "단짝 인형 효과(내전 연승/연패 스택)가 투기장이랑 헷갈림. 해당 부분(연승/연패 스택) 빼고 아케이드는 아케이드답게. 아까 추가한 진입버튼 이모지 과하니 빼고 디자인으로 구별." (AskUserQuestion 프리셋 대신 커스텀 지시).
- **핵심 안전 확인**: 단짝 스택 보상은 `_buddySpaceHtml`(아케이드 전용·+개발프리뷰)에만 있던 게 아니라, **홈 대시보드 알림**(`buddyClaimPrompt` L23185)+**홈 단짝카드→`_buddyOpenPanel` 모달**(L38822·스택/보상/보상표/설명 전부)에서 온전히 수령 가능 → 아케이드서 빼도 기능 손실 0. 내전 맥락(홈)에 그대로 남음.
- **수정**: ①`_buddySpaceHtml`(L38688)=스택 pill·보상라인·수령버튼·보상표 details 전부 제거 → 3D+이름+별칭+함께N회+교체만(프로필 쇼케이스). 대신 `.bsp-buddy-note` 한 줄("내전 연승·연패 보상은 홈 화면에서 받아요"). ②`_arcadeEntriesHtml`=진입 3버튼 이모지(🎪🗡️🏘️)·`.ae-ic` 제거. ③CSS `.arcade-entry`=이모지 대신 **좌측 색 액센트 바**(claw 금#f0c860·arena 주황#e4804c·plaza 초록#7CFC9A)로 구별·배경 뉴트럴화.
- **죽은코드**: `buddyClaimInline`(L38750)·`_buddySpaceHtml`의 `st`인자 미사용 됐으나 무해(유지). `_buddyRewardAt` 등은 홈/모달서 계속 사용.
- **검증**: `aram-check` 7/7. ⚠️**Edge 헤드리스 스크린샷 이 PC서 계속 미작동**(png 미생성·엔터프라이즈 정책 추정) → 목업/실기 시각검증 못 함. CSS 저위험이나 **라이브 아케이드 탭 눈으로 확인 권장**.
- ⏭️ **미해결(이월)**: 아케이드 탭 시각 확인(v595~596) · 자동로봇 실기(v593~594) · 기존 미결.

### v2.45.595 (2026-07-23·이 PC·`main` 직접) — 🕹️ 아케이드 탭 UX 정리(투기장 3버튼→1진입점·진입 3개 나란히)
...
- **불운(천장) 스택 = 수동과 같은 `lotteryPity_s2` 공유**: 예약 N장을 순서대로 굴리며 `stack` 읽고→conversion→`win?0:min(20,stack+1+skulls*0.5)` 갱신, 마지막 값 저장. `_rollScratchPity`(27651) 로직을 순차 threading으로 복제(로봇은 그 함수 직접 안 씀). 예약 순간 스택 즉시 반영.
- **해골**: 로봇은 전 칸 공개 → `slots`의 `id==='skull'` 전부 카운트(수동은 회피 가능하나 로봇은 불가·사장님 감수). net = 당첨 − 해골수×`skullPenalty`(골드·프리즘 둘 다 10G) + 로봇보너스. net 음수 가능(영수증·버튼·정산 팝업 전부 부호 처리).
- **밸런스**: 골드 복권 회수율 ~74%인데 로봇은 해골 전부 공개라 더 낮음 + 보너스 +8~12 미미 → 여전히 골드 sink(무한증식 없음). 시뮬 별도 안 함(구조상 명백).
- **백워드 호환**: 배포 전 예약된 실버 큐는 `lidx`/`pen`/`sk` 없음 → `??0`/`||0`으로 실버(패널티0) 동작 유지 ✅.
- **검증**: `aram-check` 7/7 · 식별자 grep 대조 · 머니 math 일관성(예약버튼 미리보기 claimTotal === claim total) 확인. ⚠️**실기 상호작용 미검증**(모달이 로그인+골드데이터+로봇 보유 필요라 헤드리스 확인 곤란) — 라이브 첫 사용 시 예약→수령 흐름 눈으로 확인 권장.
- ⏭️ **미해결(이월)**: 자동 로봇 골드/프리즘 실기 검증(예약·해골 패널티·불운 스택 반영·net 음수 표시) · 기존 미결 그대로.

### v2.45.592 (2026-07-20·이 PC·`main` 직접·Pages success·라이브 v2.45.592) — 🖤 블랙 당첨률 24%→31%(프리즘급) + 팟 30%→20% 상쇄 + mig20 소급 재작성
> **v591에서 미결로 남겼던 「블랙 당첨률 vs 팟 크기」 결정**을 이 세션 시작 시 사장님께 다시 물어 **옵션 ①(당첨 31% + 팟 20%)** 확정. 세션 시작 루틴대로 양쪽 리포 fetch/pull 먼저(홈 6커밋 behind→pull, 오버레이 최신) + 라이브 v591 배포 확인 후 착수.
- **파라미터**: `BLACK_TIER.winChance 0.24→0.31` · `potRate 210→140`(구매금 20%). 코드=index.html `BLACK_TIER`(~L29673~29684).
- **⚠️ "프리즘급"의 정확한 의미(다음 작업자 주의)**: 프리즘 당첨률 30.7%는 **체감(실 rollScratch)** 값인데, 블랙은 티켓 10%가 해피 블랙(당첨 1%)으로 변신하므로 **체감 = 0.9×winChance + 0.1×0.01**. winChance 0.31 → **체감 28.0%**(프리즘에 근접). 체감 31%를 맞추려 `winChance 0.345`로 올려 시뮬한 결과 **최악 회수율 101.4% = 돈복사**라 **불가** → **0.31이 팟 20% 하에서의 상한**. 팟을 더 낮추지 않는 한 이보다 못 올림.
- **시뮬 재검증**(리포 `블랙복권-시뮬.mjs`·상수 동기화 완료·`node 블랙복권-시뮬.mjs`로 재실행): 300만장 기본 **83.5%** · 불운 스택 89.3% · 강철심장 max 88.8% · **최악(둘 다) 94.1% <100% ✅**(인플레 불가). 100장 손익(기본) −11,544G.
- **🔁 소급 마이그레이션 재작성 (`_blackPotMigrate30` → `_blackPotMigrate20`)** — 이번 작업의 함정:
  - 최종 목표 = 원본(70G/장 누적)의 **×2**. 그런데 **v591의 mig30(×3)이 이미 라이브에서 실행됨** — 배포 전 REST 실측 `season2/blackPot = {amount:420, mig30:true}`(원본 140 × 3).
  - → 새 트랜잭션: `if(node.mig20) return node;` / `amt = round(node.mig30 ? cur*2/3 : cur*2)` / **`{mig20:true, mig30:true}` 둘 다 세움**(아직 v591 캐시를 쓰는 stale 클라가 mig30(×3)을 재적용하는 것을 차단).
  - **수렴 검산(node)**: `420(mig30) → 280` ✅ / `mig20 먼저 140→280 후 stale mig30 재적용 → 280` ✅ / `mig20 두 번 → 280` ✅ / `팟 0 → 0 + 플래그만` ✅. 트랜잭션 순서 무관.
  - ⚠️ **실행 시점 = 라이브 첫 유저가 복권 Hub를 열 때**(`_blackPotEnsureSub`서 호출) → 배포 직후엔 아직 420. **사장님이 팟이 280으로 바뀌었는지 확인 권장**(또는 `curl .../season2/blackPot.json`).
- **UI/문구**: "30%"→"20%" 5곳(허브 카드 `.lh-black-potsub`·`?` 도움말 `scard-help-row`·구매확인 문구·모달 팟바 `.blk-pb-lbl`·블록 상단 주석). **과거 CHANGELOG/CHANGELOG_MAJOR의 30%는 이력이라 유지**(정책).
- **CHANGELOG_MAJOR 1줄 추가**: v591이 "팟 3배" 공지로 이미 나갔으므로 정정 성격의 팀 공지가 필요하다고 판단(자잘한 수치조정이 아니라 직전 공지를 뒤집는 변경).
- **검증**: `aram-check` 7블록 node --check **7/7** · 시뮬 재실행 · 마이그레이션 수렴 검산 · Pages run success · 라이브 curl `v2.45.592` 확인.
- ⏭️ **미해결(이월)**: 블랙 팟 소급 실반영 확인(위) · 기존 미결(오버레이 실기 미검증·투기장 엔드게임 코인 인플레·보안 공개링크·복권 해골감소 인플레·레벨 패스 S1식 등) 그대로.

### v2.45.583 (2026-07-16·이 PC·홈페이지·Pages success·라이브 v2.45.583) — 🖤 블랙 복권(TIER 4·스피또) + 블랙 팟(공유 잭팟·해피 블랙) 정식 통합
> 사장님 요청으로 목업(`블랙복권-목업.html`)을 index.html에 정식 통합. 여러 차례 반복(개념→예시이미지[스피또1000]→확률→인플레검증→블랙컨셉+팟→통합). 검증=aram-check 7블록 node --check 7/7 + 로컬서버(`_clawserve.mjs`:8731)+Edge 헤드리스 **실사이트 iframe 390px 고정** 스샷(PC/모바일 허브·모달·해피 전부 OK).
- **무엇**: 프리즘 위 700G 등급 **스피또식** 복권. 행운문양 1칸 + 나의문양 8칸(4×2·전부 유니크=중복문양 없음·💀2칸-20G). 행운문양과 같은 그림 1개면 그 칸 당첨금(최고 5만G). **매치 즉시 당첨 확정**(이후 칸은 구경용·감점X)·「🖐 이만 확인」=해골 회피 조기종료.
- **🖤 블랙 팟(신규 핵심)**: 구매금 10%(70G)를 `season{N}/blackPot.amount`에 **runTransaction 적립**(전 팀원 공유·`onValue` 실시간·오도미터 자릿수 롤링). 🍀**해피 블랙**(1.5% 변신) 티켓이 **당첨**되면 칸 당첨금 + **팟 전액**. 지급도 **runTransaction**(동시당첨 이중수령 불가)·`season{N}/blackPotLog` 감사 push·팟 **쭈르륵 0 드레인** 연출.
- **⚠️ 팟 적립률=10% 설계근거(중요·재론 대비)**: 이 게임은 구매=소각(goldSpent)·당첨/팟지급=신규발행(faucet)이라 **팟 적립률이 회수율에 그대로 +로 더해짐**(팟=낸 돈에서만 지급이지만 지급 시 새로 찍힘). 일반 당첨만 회수율≈**73.5%**(당첨32%) → +팟10% = 83.5%. **팟률↑ 하려면 일반 당첨을 그만큼 낮춰 총합<100% 유지해야 함**(예: 팟50%면 일반당첨률 32→~15%로·일반회수율33.5%·총 83.5% 동일). **50%도 인플레 아님**(사장님 지적 반영)—단 "흔한 당첨↓·잭팟↑" 성격 선택일 뿐. 사장님 결정=**일단 10% 유지**. 나중에 잭팟 키우면 팟%+"해피=무조건당첨"(현재 해피도 32% 매치 필요라 2/3 허무) 재조정 후 재시뮬.
- **EV(scratchpad `black_sim2.mjs`·3백만장)**: 당첨32%·기본+팟83.5%·불운89.2%·강철심장max89.6%·**최악94.2% <100%**=인플레불가. 팟 지급 평균 1/205장·~1.4만G·최대 16만G 목격.
- **기존 시스템 완전 연동**: 불운스택(`lotteryPity.black`·`LOTTERY_PITY_KEY[3]='black'`·`_rollBlackPity`)·강철심장 해골감소(해골칸→문양 치환·`_lottoSkullReduce`)+당첨보너스(`_myLottoPrizeBonus(...,3)`)·무료권`freeScratch[3]`·보류/이어긁기(`pendingScratch` **black:true 분기**·`_lhResumeScratch`가 `_lhResumeBlack`로 라우팅)·버리기·**긁기전 취소 환불(골드+불운+팟적립 전부 원복)**·긁개도구(brushMult/threshold 동일 시맨틱)·`lotteryHistory`(happy/potWin/potAdd/pity 필드)·지출로그(`[70,200,400,700]`/`['실버','골드','프리즘','블랙']`)·버디통계/우편라벨.
- **코드맵**(index.html·`window._lhResumeScratch` 뒤 ~L29627+ 한 블록): `BLACK_TIER`(idx3·price700·winChance0.32·happyChance0.015·potRate70·symbols11·prizePool)·`_blackPickPrize`/`_blackShuffle`·`rollBlackScratch`·`_rollBlackPity`·`_blackPot{Ref,Add,Claim,EnsureSub}`(`_blackPotSubbed`/`_blackPotAmt`)·`_blkOdometer`·`_lhBlackCardHtml`(허브 카드·`_injectBlackScratchCss` 먼저 호출)·`window._lhBuyBlack`(구매확인)·`window._buyBlackScratch`(실구매)·`_blackHandlers`(onComplete/onSetAside/onDiscard 공용)·`window._lhResumeBlack`·`_injectBlackScratchCss`(`#blk-css`·`.blk-*`/`.lh-black-*`·모바일 ≤480 축소)·`showBlackScratchModal`(윈도우레벨 pointermove로 원 밖 드래그 끊김 방지)·`window.blackScratchPreview({happy,win})`(실데이터·팟 무수정=청구/기록/재구매 차단). 허브 삽입=`openLotteryHub`서 `<div class="lh-tier-grid">` 뒤 `${_lhBlackCardHtml(data, myGold)}`.
- ⚠️**실기 미검증**: 실구매→보류→이어긁기→해피 당첨(팟 지급) 1사이클은 실계정 확인 권장(개발환경에 Firebase 실흐름 없음). 문법·로직·헤드리스 렌더만 검증. 목업 파일(`블랙복권-목업.html`)은 리포에 유지(설계 레퍼런스).

### 🃏 챔피언 카드게임 (복권 신규·2026-07-13~·기획+목업 진행중·아직 미통합)
> 사장님: 복권 섹션에 **컬렉션 챔피언 카드로 하는 카드게임** 추가하고 싶다. **목업만 있고 index.html 미통합**(홈 배포 무영향). 목업 파일=리포 루트(gitignore 화이트리스트)·GitHub Pages로 플레이 테스트.
- **방향 확정(여러 번 뒤집힘 주의)**:
  1. 처음=성급(실버/골드/프리즘) 족보 포커 → "비직관적"이라 폐기.
  2. 다음=**진짜 트럼프 포커**(52장·♠♥♦♣+숫자, 챔피언은 카드 그림/스킨). 카드=하얀 바탕+모서리 인덱스(♥♦빨강/♠♣검정)+가운데 챔피언 초상화 액자(성급=프레임색). → `챔피언포커-목업.html`(vs 딜러 5카드 드로우·레이즈/폴드).
  3. **최종 방향=텍사스 홀덤 vs 봇들**(사장님: "봇 넣고 심리싸움". 실유저는 사람 없으면 못 하니까). → `홀덤-목업.html`(현재 주력).
- **🃏 홀덤 확정 설계**: **단판**(한 판=한 게임·토너먼트 아님) · **나 vs 봇 3명(4인)** · 참가비 고정(예 200G)=그 판 스택, **가짜 칩으로 베팅**(진짜 골드는 참가비만·손실상한=참가비) · **팟 레이크 5%**=sink · **RTP ~85% 목표**(스킬게임이라 복권76%보다 후하게)+일일 판수 제한(인플레 방지) · 봇 성격은 캐릭터별(바위=타이트/불도저=어그로/허풍쟁이=블러핑/호구=콜링스테이션/프로=균형)로 "읽기" 재미. 딜러=TF 3D(진행자·플레이어 아님).
- **📁 파일**: `홀덤-목업.html`(주력·1단계 완성)·`챔피언포커-목업.html`(구 5카드·참고보존)·`dealer_tf.glb`(TF 딜러 3D·5MB·리포 루트·`!dealer_tf.glb` 화이트리스트·⚠️미커밋이면 404). TF 프레이밍 확정값=크기(DIST)15·높이(LOOKY)1.0·좌우(XOFF)0.16·각도(ELEV)0.8·**frameCam=구면좌표 고정반경**(각도 바꿔도 크기 불변)·29개 클립(딜=Spell1·셔플=Channel 등·모션테스터로 선택 예정).
- **✅ 1단계 완성(홀덤-목업.html)**: 홀덤 엔진(홀2+커뮤5·프리플롭/플롭/턴/리버 베팅·블라인드5/10·best7 판정·팟/올인 런아웃)+기본 봇AI+4인 테이블 UI+**초보 도움(코치=내 패 힘/추천액션·토글)+30초 인트로+족보/용어 도움말**. RTP/역추적/사이드팟은 미구현(단판 동일스택이라 사이드팟 불필요).
- **✅ 2단계 완성(2026-07-13·원격web·작업브랜치 `claude/resume-work-another-computer-1rnw9x`)**: 🎭**봇 성격 5종 시스템** — `PERSONAS`(바위🪨신중/불도저🐗저돌/허풍쟁이🎭뻥카/호구🐟콜쟁이/프로🎯균형·각 `{tight,aggro,bluff,sticky,size}` 파라미터). 매 판 5종 중 **랜덤 3명**이 좌석에 앉음(distinct). `botDecide(p)` 전면 재작성=`p.persona` 파라미터로 폴드 문턱/공격성/블러핑/끈기 분기(판단 노이즈 ±0.08 추가·내 레이즈=`big` 플래그로 반응). **읽기 재미**: 좌석 이름 밑 성격 태그(`.phint`)+도움말 모달에 성격 범례. ⚠️**같이 고친 잠복버그(1단계 엔진 결함)**: `best7(cards)`이 "2장 제거→5장 남김" 방식이라 **정확히 7장일 때만 동작** → 플롭(2+3=5장)·턴(2+4=6장)에서 항상 null 반환 → 봇이 `strength` 계산 중 크래시(판 hang). **임의 N(≥5)장에서 최고 5장 조합**을 고르도록 재작성(조합 재귀). 이걸로 플롭/턴 쇼다운·내 패 표시 전부 정상화. **검증(Playwright 헤드리스·scratchpad `holdem_*.mjs`)**: 12판 무오류 완주·쇼다운(트리플/투페어) 정상·성격별 액션분포 4000회 집계로 차별화 확인(호구 폴드2%·콜55% / 허풍쟁이 레이즈26% / 바위 폴드24%·레이즈4% / 불도저 레이즈19% / 프로 균형). ⚠️봇 750ms 딜레이라 대량표본은 `botDecide` 직접호출로 검증.
- **⏭️ 다음 단계**: 3단계=참가비/상금·**RTP 시뮬 재튠**·일일캡 → 4단계=TF 3D 딜러 연동·봇 얼굴·모션 → **홈 복권 탭 정식 통합**(진짜 골드·역추적 로그·오버레이 임베드). 시뮬 스크립트=scratchpad `poker_real.mjs`(진짜 포커 RTP)·`poker_sim2.mjs`(구 성급버전). ⚠️3단계 RTP 시뮬 시 새 봇 성격(특히 호구=콜쟁이가 하우스/스킬 유저에게 유리)을 반영해 재계산 필요.
- ⚠️ **사장님 반복 강조**: 절대 인플레 없어야 함(돈 버는 구조 X·복권처럼 sink)·딜러/하우스가 항상 유리·돈 없는 사람도 즐길 수 있게(저스테이크). 홀덤 목업 URL=`https://sohada2.github.io/aram/홀덤-목업.html`.

### v2.45.576~578 (2026-07-09·이 PC) — 🥊 막고라 결과 오버레이 주입 + 🎮 오버레이 안내 팝업 + ⚔️ 승급전 표시 명확화
> 이 세션 = **이 PC**(`C:\Users\sbs_n\Desktop\aram`·`main` 직접). **주 작업은 오버레이(별도 리포 aram-overlay·아래 그 리포 CLAUDE.md 참조)** — 헤더 재설계·로비 배너·막고라·클라종료 다이얼로그·자동로그인 제거·1열 지갑·최소화 버튼 등. 홈페이지는 3건. 배포검증=Pages success·라이브 curl v2.45.578. 인라인 `<script>` 7블록 `node --check` 7/7.
- **v578 ⚔️ 승급전 표시 명확화**(사장님: "승급전 도달 시 「승급전 0-0」이 애매") — `updateMyInfoBar`(~L25349) 티어 pill에 `_promo`/`_promoBadge` 추가 → 비만렙 승급전이면 **「⚔️ 승급전 N승 N패」 금빛 펄스 배지**(`.mib-promo`·`mibPromoPulse` 키프레임 ~L2002) + 승급 중 LP 텍스트 숨김. 만렙(LV50) 영역(`d2.promoActive` ~L25455)도 「⚔️ 승급전 N승 N패」로. **오버레이도 동일 적용**(aram-overlay v0.1.48: 사이드패널 프로필 `.pf-promo`·인게임 `promoCtx`).
- **v577 🎮 오버레이 안내 팝업**(아이디당 1회) — 「내전 오버레이」가 뭔지 안내(롤 클라 위 도킹·홈 연동·「방장」 체크=예전 라이브계정 역할·홈 최하단서 다운로드). `maybeShowOverlayNotice`/`showOverlayNotice`/`window.closeOverlayNotice`(`window.showS2Tutorial` 직후 ~L24366)·트리거=`_goldUiRefresh`+`selectMyName` 끝(setTimeout 500)·`.ovn-*` CSS(~L5186)·seen=`gold/{key}.overlayNoticeSeen`(Firebase)+`localStorage overlayNoticeSeen_{key}`.
- **v576 🥊 막고라 결과 오버레이 주입** — 오버레이 방장이 막고라 결과(승자/요건/증강)를 네이티브로 입력 → 홈 `window.overlayMagollaResult(matchId,winner,cond,aug)`(`window.overlayForceSettle` 직후 ~L17953)가 숨은 라이브 창(liveWin)에서 실행: liveMode+`status==='closed'` 검증 후 `mgSettleMatch(matchId,winner,cond,aug,d)`. 반환 'ok'/'err:...'. (오버레이 v0.1.45~47이 이 함수 주입)
- ⏭️ **미해결(이월)**: 오버레이 실기 미검증(개발환경에 롤 클라·실계정 없음)·기존 투기장/보안/코인 인플레 등.

### v2.45.573~575 (2026-07-08·다른 PC) — 🖥️ 오버레이 정산 발행 + 🔒 계정 조작 잠금(B안) + 🪟 시즌팝업 스크롤
> 다른 컴퓨터(작업브랜치 무관·`main` 직접)가 올린 3건. **전부 오버레이(별도 리포 aram-overlay 연동)/계정 관련이라 투기장과 무관**. 이 세션(원격web)은 pull로 받아 동기화만 함(v572까지가 이 세션 작업). node --check 7/7·문서는 이 커밋에서 최신화.
- **v575 🖥️ 오버레이 정산 발행(`overlayForceSettle`)** — 전원 투표해야 `autoConfirm`이 돌아 정산(`lastSettlement`)이 발행되는데, 오버레이만으로 진행 시 미투표자가 있으면 '건너뛰기' 버튼이 숨은 라이브계정(`liveWin`)에만 있어 아무도 못 눌러 정산창이 영영 안 뜨던 문제. `window.overlayForceSettle` 노출 → 오버레이 방장이 executeJavaScript로 호출, 지금까지 표로 확정(`autoConfirmMvp`)→저장→`finalizeVotes`→`lastSettlement` 발행.
- **v574 🔒 계정 조작 잠금(B안)** — 같은 계정 동시 쓰기 사고(last-write-wins 증발) 방지. `accountSession/{fbKey}={deviceId,app,label,at,hb}`(보기는 자유·조작 권한만 한 기기). `_ctrlGuard()`=비어있음/stale(75s)/내 기기면 자동 클레임 통과, 다른 기기 조작 중이면 확인 후 인계(takeover). 개인 경제 뮤테이터 **15곳** 게이트(`buyItem`·`toggleItemActive`·`quick*`·`doGachaPull/Merge`·`toggleGachaSynergy`·`emblem` 7종·`claimS2PassLevel`). 하트비트 25s·`onDisconnect` 자동 반납·**liveMode 제외**(진행 자동화 보호). 강제력 없는 사고방지용(신뢰 모델 동일).
- **v573 🪟 시즌 안내 팝업 작은 화면 스크롤** — 시즌 안내 팝업(`showS2Tutorial` 등)이 작은 화면(오버레이 홈창)서 잘려 확인 버튼 접근 불가 → 스크롤 가능하게 수정.
- ⚠️ 상세 코드맵은 커밋 메시지(78a255c/46379be/2e7b39b) 참조. 오버레이 쪽 상세는 `aram-overlay` 리포 CLAUDE.md.

### v2.45.572 (2026-07-08·원격web·작업브랜치 `claude/continued-update-1srahn`) — 🛡️ 투기장 방어 순서를 강화창에도 노출
> 사장님: "내 방어 순서가 안쪽(배틀 화면)에만 들어있는데 바깥쪽에도 표시해주면 좋겠다." CLAUDE.md 미해결 항목("방어 순서 설정이 배틀 화면에만 노출—강화창/프로필 검토")과 일치. 강화창에 추가.
- **위치**: `openArenaForge` render(~L35766)의 `arena-picknote` 다음, `_arenaTabs` 앞에 `.arena-defset` 블록 삽입(배틀 화면과 동일 CSS·클래스 재사용).
- **주인**: 방어 순서는 공격받을 때 쓰는 **전투(장착) 인형** 기준 → `const eqF = SB? active : _arenaActiveFighter(getMyGoldData())`. 강화창서 다른 인형을 미리보기(`active`) 중이어도 방어는 장착 인형이라, 라벨에 `${_buddyChampName(eqF)}가 이 순서로 막아요`로 명시하고 스킬명도 `_champSkills(eqF)` 사용.
- **순서값**: `const defOrder = SB? null : _arenaDefOrder(getMyGoldData()||{}, key)`(미설정 시 키 해시 기본 permutation). 슬롯 3개 `.ads-slot data-i`.
- **탭 핸들러**(`!SB` 가드): `_arenaDefOrder`로 현재값 읽어 `_nxt={Q:'W',W:'E',E:'Q'}`로 순환→`_arenaSetDefOrder(cur)`. **전체 `render()` 대신 그 슬롯의 `<b>`/`<span>` textContent만 갱신** → `_forge3dShow` 재호출 안 해 3D 인형 깜빡임/재로드 없음(배틀 화면은 full render라 3D 없음이라 무관했지만 강화창은 3D라 제자리 갱신 필수).
- **검증**: `.ads-lbl`/`.arena-defset`/`.ads-slot` CSS 기존 존재(배틀 화면 공유). 인라인 `<script>` 7블록 `node --check` 7/7. ⚠️실기 확인=사장님(강화창 하단에 방어 순서 뜨고 탭 시 3D 안 깜빡이며 바뀌는지).

### v2.45.571 (2026-07-08·원격web·작업브랜치 `claude/continued-update-1srahn`) — 🗡️ 투기장 배틀 중도 이탈=패배 처리 + 확인 알림
