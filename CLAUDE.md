...
- **다음 작업/미결정**: ① 보상·경제 구조(챔피언별 보상·포로 패널티·횟수충전) Firebase 서버검증 ② 27마리 성능 ③ 배경 평면 그림 위치/톤 튠 ④ **앱 통합 런칭 시** `claw.html`에 Firebase 연결(닉네임=localStorage 공유, 해금/일일/보상 검증). **출시 전 디버그 플래그 false·`ENFORCE_DAILY_LIMIT=true`.**

## 🃏 가챠 시너지 재설계 (작업 예정 · 다른 컴퓨터 핸드오프) ★구현 대기
> S1 실데이터(286경기·승/패 각 1069표본)로 보정한 시너지 **조건 개편안**. **아직 코드 미반영** — `GACHA_SYNERGY_GROUPS`(index.html ~30761) 조건 교체 + 조건 평가기 확장 필요. 목표: "거의 다 무조건 발동"을 **다양한 스탯 조건(≈50% 달성)**으로 바꿔 재미↑. **보상값은 현재 유지(LP 인플레 방지). 연승/연패 효과는 제거**(알아차리기 어려움).
- **조건 평가기 확장 필요** (현재 `pData[cond.stat] >= cond.min` 단일 비교만, index.html ~16845): ①스탯 ≥(킬·어시·딜량, **데스도 ≥로 사용**) ②**KDA 파생**((k+a)/max(1,d)) ③**복합 AND**(킬6+ & 데스4+) ④**MVP**(매치레벨 `m.mvpWinner`) ⑤lp_block(강철·여명)은 **패배 참가자 스탯**으로 판정. participants 없는 수동저장 경기는 미발동(기존 안전장치 유지).
- **확정 조건·보상 (2성/3성, 달성률=S1 실측)**:
  ...
  | 나는 왕이다(shurima) | MVP 승리(26%) **※연승 제거** | +2 LP | +3 LP |
  | 강철 심장(tank) | 데스 8+ 패배(52%) | LP 3 방어 +골드 30 | LP 5 방어 +골드 50 |
  | 여명의 의지(demacia) | KDA 2.2+ 패배(49%) | LP 1 방어 | LP 2 방어 |
  | 해적의 보물(bilgewater) | 승리(무조건) **※연승 제거→flat** | 40 G | 60 G |
  | 신성한 개입(support) | 어시 15+ 승리(51%) | 승 40G / 패 위로금 15G | 승 60G / 패 위로금 25G |
  | 유레카(mage) | 딜량 25,000+(51%) | 승 +8 / 패 −8 LP | 승 +10 / 패 −8 LP |
  | 공허 균열(void) | 패배(도박) | 전액 방어 / 실패 −6 | 전액 방어 / 실패 −4 |

- **구현 전 사용자 확인 필요(미결정 3)**: ①신성한 개입 "어시15+"가 **패배 위로금까지 막음** → 위로금 무조건으로 둘지 ②유레카 도박 조건이 **패배 −8까지 막음** → 승리 보너스에만 조건 걸지 ③해적의 보물 연승 빼서 flat됨 → 정체성용 스탯조건(골드/CS 등) 추가할지.
- **데이터 산출법**: Firebase `<DB>/matches.json` REST로 받아 S1(season:1)·participants 보유 경기의 킬·데스·어시·딜량·KDA 분포 분석(임시 node 스크립트). ⚠️ 멀티킬(double/triple…)은 데이터 전부 0이라 조건 불가. MVP는 구조상 ~26%(팀당 1명) 최대.
